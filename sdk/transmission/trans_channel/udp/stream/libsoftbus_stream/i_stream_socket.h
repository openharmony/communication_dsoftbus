...
public:
    ...
    virtual void OnStreamStatus(int32_t status) = 0;
    virtual int32_t OnStreamHdrReceived(std::unique_ptr<char[]> header, int32_t size) = 0;
...
public:
    ...
    virtual bool CreateClient(IpAndPort &local, int32_t streamType,
        std::pair<uint8_t*, uint32_t> sessionKey) = 0; // socket + bind
    virtual bool CreateClient(IpAndPort &local, const IpAndPort &remote, int32_t streamType,
        std::pair<uint8_t*, uint32_t> sessionKey) = 0;
    virtual bool CreateServer(IpAndPort &local, int32_t streamType, std::pair<uint8_t*, uint32_t> sessionKey) = 0;
    ...
    virtual bool SetOption(int32_t type, const StreamAttr &value) = 0;
    virtual int32_t SetMultiLayer(const void *para) = 0;
    virtual StreamAttr GetOption(int32_t type) const = 0;
...
protected:
    ...
    virtual int32_t CreateAndBindSocket(IpAndPort &local, bool isServer) = 0;
    virtual bool Accept() = 0;

    virtual int32_t EpollTimeout(int32_t fd, int32_t timeout) = 0;
    virtual int32_t SetSocketEpollMode(int32_t fd) = 0;
    virtual std::unique_ptr<char[]> RecvStream(int32_t dataLength) = 0;
