...
static int32_t TransTdcProcessData(int32_t channelId)
{
    ...
    SoftBusLog(SOFTBUS_LOG_TRAN, SOFTBUS_LOG_INFO, "channelId[%d] data has all received, dataLen = %u", channelId, dataLen);
...
static int32_t TransTdcProcAllData(int32_t channelId)
{
    while (1) {
        ...
        if (bufLen < DC_DATA_HEAD_SIZE) {
            SoftBusLog(SOFTBUS_LOG_TRAN, SOFTBUS_LOG_WARN, "ChannelId[%d]: head[%d] not enough, recv biz head next time.", channelId, bufLen);
            SoftBusMutexUnlock(&g_tcpDataList->lock);
