...
void AddSocketResource(const char *sessionName, const ChannelInfo *channel)
{
    ...
    if (ClientGetSessionIdByChannelId(channel->channelId, channel->channelType, &newItem->socketId) != SOFTBUS_OK) {
...
void DeleteSocketResourceByChannelId(int32_t channelId, int32_t channelType)
{
    ...
    int32_t socketId;
    if (ClientGetSessionIdByChannelId(channelId, channelType, &socketId) != SOFTBUS_OK) {
