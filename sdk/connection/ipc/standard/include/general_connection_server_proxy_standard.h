...
#ifndef INTERFACES_INNERKITS_CONNECTION_SERVER_PROXY_STANDARD_H_
#define INTERFACES_INNERKITS_CONNECTION_SERVER_PROXY_STANDARD_H_

#include "if_softbus_server.h"

namespace OHOS {
class ConnectionServerProxy : public IRemoteProxy<ISoftBusServer> {
public:
    explicit ConnectionServerProxy(const sptr<IRemoteObject> &impl)
        : IRemoteProxy<ISoftBusServer>(impl) {}
    virtual ~ConnectionServerProxy() = default;
    ...
    int32_t GetNodeKeyInfo(const char *pkgName, const char *networkId, int key, unsigned char *buf,
        uint32_t len) override;
    int32_t SetNodeKeyInfo(const char *pkgName, const char *networkId, int key, unsigned char *buf,
        uint32_t len) override;
...
private:
    static inline BrokerDelegator<ConnectionServerProxy> delegator_;
};
} // namespace OHOS

#endif // !defined(INTERFACES_INNERKITS_CONNECTION_SERVER_PROXY_STANDARD_H_)