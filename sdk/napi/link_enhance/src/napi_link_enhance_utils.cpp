...
void NapiCallFunction(napi_env env, napi_ref callbackRef, napi_value *argv, size_t argc)
{
    ...
    if (isExist) {
        ...
        COMM_LOGD(
            COMM_SDK, "napi_get_and_clear_last_exception status: %{public}d, exception: %{public}p", status, exception);
    }
}
...
int32_t ConvertToJsErrcode(int32_t err)
{
    switch (err) {
        ...
        case SOFTBUS_CONN_BLE_GENERAL_CREATE_CLIENT_MAX:
            return LINK_ENHANCE_CONNECTIONS_EXCEEDS;
        case SOFTBUS_CONN_BLE_GENERAL_CONNECT_TIMEOUT:
            return LINK_ENHANCE_CONNECT_TIEMOUT;
        case SOFTBUS_CONN_BLE_GENERAL_SERVER_NOT_OPENED:
            return LINK_ENHANCE_CONNECT_PEER_NOT_START_SERVICE;
        case SOFTBUS_CONN_BLE_GENERAL_DUPLICATE_SERVER:
            return LINK_ENHANCE_DUPLICATE_SERVER_NAME;
        case SOFTBUS_CONN_BLE_GENERAL_CONNECTION_NOT_READY:
