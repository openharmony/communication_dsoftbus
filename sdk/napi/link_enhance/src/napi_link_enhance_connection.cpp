...
napi_value NapiLinkEnhanceConnection::Create(napi_env env, napi_callback_info info)
{
    ...
    if (status != napi_ok) {
        HandleSyncErr(env, LINK_ENHANCE_PARAMETER_INVALID, "no needed arguments");
...
napi_value NapiLinkEnhanceConnection::On(napi_env env, napi_callback_info info)
{
    ...
    if (status != napi_ok || argc != ARGS_SIZE_TWO) {
        HandleSyncErr(env, LINK_ENHANCE_PARAMETER_INVALID, "expected 2 arguments");
        return NapiGetUndefinedRet(env);
    }
    NapiLinkEnhanceConnection *connection = NapiGetEnhanceConnection(env, info);
    if (connection == nullptr) {
        HandleSyncErr(env, LINK_ENHANCE_PARAMETER_INVALID, "get connection failed");
        return NapiGetUndefinedRet(env);
    }
    ...
    if (status != napi_ok) {
        HandleSyncErr(env, LINK_ENHANCE_PARAMETER_INVALID, "get string failed");
        return NapiGetUndefinedRet(env);
    }
    ...
    if (strcmp(type, "connectResult") == 0) {
        ...
        HandleSyncErr(env, LINK_ENHANCE_PARAMETER_INVALID, "invalid type");
        return NapiGetUndefinedRet(env);
    }
    return NapiGetUndefinedRet(env);
}
...
napi_value NapiLinkEnhanceConnection::Connect(napi_env env, napi_callback_info info)
{
    ...
    if (status != napi_ok || argc > ARGS_SIZE_ZERO) {
        HandleSyncErr(env, LINK_ENHANCE_PARAMETER_INVALID, "arguments err");
        return NapiGetUndefinedRet(env);
    }
    NapiLinkEnhanceConnection *connection = NapiGetEnhanceConnection(env, info);
    if (connection == nullptr) {
    ...
    if (strcpy_s(address.addr.ble.mac, BT_MAC_LEN, connection->deviceId_.c_str()) != 0) {
        HandleSyncErr(env, LINK_ENHANCE_INTERVAL_ERR, "get ble mac failed");
        return NapiGetUndefinedRet(env);
    }
    ...
    if (handle <= 0) {
        ...
        HandleSyncErr(env, errcode, "connect failed");
        return NapiGetUndefinedRet(env);
    }
...
napi_value NapiLinkEnhanceConnection::GetPeerDeviceId(napi_env env, napi_callback_info info)
{
    ...
    int32_t ret = GeneraConnGetPeerDeviceId(handle, cDeviceId, BT_MAC_LEN);
    if (ret != 0) {
        ...
        if (ConvertToJsErrcode(ret) == LINK_ENHANCE_PERMISSION_DENIED) {
            HandleSyncErr(env, LINK_ENHANCE_PERMISSION_DENIED, "get peer deviceId failed");
        }
        return NapiGetStringRet(env, deviceId);
    }
    deviceId = cDeviceId;
...
napi_value NapiLinkEnhanceConnection::SendData(napi_env env, napi_callback_info info)
{
    ...
    if (status != napi_ok || argc != ARGS_SIZE_ONE) {
        HandleSyncErr(env, LINK_ENHANCE_PARAMETER_INVALID, "expected one arguments");
        return NapiGetUndefinedRet(env);
    }
    NapiLinkEnhanceConnection *connection = NapiGetEnhanceConnection(env, info);
    if (connection == nullptr) {
    ...
    if (arrayBuffer == nullptr) {
        HandleSyncErr(env, LINK_ENHANCE_PARAMETER_INVALID, "get arraybuffer failed");
        return NapiGetUndefinedRet(env);
    }
    ...
    if (status != napi_ok) {
        HandleSyncErr(env, LINK_ENHANCE_PARAMETER_INVALID, "fail to get arraybuffer data");
        return NapiGetUndefinedRet(env);
    }
    uint8_t *data = (uint8_t *)SoftBusCalloc(dataLen);
    if (data == nullptr) {
        HandleSyncErr(env, LINK_ENHANCE_INTERVAL_ERR, "fail to calloc mem");
        return NapiGetUndefinedRet(env);
    }
    if (memcpy_s(data, dataLen, bufferData, dataLen) != EOK) {
        SoftBusFree(data);
        HandleSyncErr(env, LINK_ENHANCE_INTERVAL_ERR, "fail to memcpy data");
        return NapiGetUndefinedRet(env);
    }
    ...
    if (ret != 0) {
        ...
        HandleSyncErr(env, errcode, "send data failed");
        return NapiGetUndefinedRet(env);
    }
    SoftBusFree(data);
    return NapiGetUndefinedRet(env);
