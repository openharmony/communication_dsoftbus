...
{
    DISC_LOGI(DISC_BROADCAST, "enter, register type=%{public}d.", type);
    ...
    DISC_CHECK_AND_RETURN_RET_LOGE(interface != NULL, SOFTBUS_INVALID_PARAM, DISC_BROADCAST, "interface is null!");
...
static void BcBtStateChanged(int32_t listenerId, int32_t state)
{
    ...
    for (uint32_t managerId = 0; managerId < SCAN_NUM_MAX; managerId++) {
        DISC_CHECK_AND_RETURN_LOGE(SoftBusMutexLock(&g_scanLock) == SOFTBUS_OK, DISC_BROADCAST, "scanLock mutex err!");
...
{
    DISC_LOGI(DISC_BROADCAST, "enter.");
    if (g_mgrLockInit) {
        return SOFTBUS_OK;
    }
    if (SoftBusMutexInit(&g_bcLock, NULL) != SOFTBUS_OK) {
        DISC_LOGE(DISC_BROADCAST, "g_bcLock init failed");
        return SOFTBUS_NO_INIT;
    }
    if (SoftBusMutexInit(&g_scanLock, NULL) != SOFTBUS_OK) {
        DISC_LOGE(DISC_BROADCAST, "g_scanLock init failed");
...
int32_t InitBroadcastMgr(void)
{
    DISC_LOGI(DISC_BROADCAST, "enter.");
    if (g_mgrInit) {
        DISC_LOGD(DISC_BROADCAST, "mgr already init.");
        return SOFTBUS_OK;
    }
    int32_t ret = BcManagerLockInit();
    DISC_CHECK_AND_RETURN_RET_LOGE(ret == SOFTBUS_OK, ret, DISC_BROADCAST, "lock init fail!");
    ...
    DISC_CHECK_AND_RETURN_RET_LOGE(g_interface[g_interfaceId] != NULL, SOFTBUS_BC_MGR_NO_FUNC_REGISTERED,
        DISC_BROADCAST, "interface is null!");
    DISC_CHECK_AND_RETURN_RET_LOGE(g_interface[g_interfaceId]->Init != NULL,
        SOFTBUS_BC_MGR_FUNC_NULL, DISC_BROADCAST, "function is null!");

    ret = g_interface[g_interfaceId]->Init();
    DISC_CHECK_AND_RETURN_RET_LOGE(ret == SOFTBUS_OK, ret, DISC_BROADCAST, "call from adapter fail!");

    ret = SoftBusAddBtStateListener(&g_softbusBcBtStateLister);
    DISC_CHECK_AND_RETURN_RET_LOGE(ret >= 0, ret, DISC_BROADCAST, "add bt state listener fail!");
...
int32_t DeInitBroadcastMgr(void)
{
    ...
    if (g_btStateListenerId != -1) {
        ...
        DISC_CHECK_AND_RETURN_RET_LOGE(ret == SOFTBUS_OK, ret, DISC_BROADCAST, "RemoveBtStateListener fail!");
...
static void BcStartBroadcastingCallback(int32_t adapterBcId, int32_t status)
{
    ...
    DISC_LOGI(DISC_BROADCAST, "enter. adapterBcId=%{public}d", adapterBcId);
    for (uint32_t managerId = 0; managerId < BC_NUM_MAX; managerId++) {
        int32_t ret = SoftBusMutexLock(&g_bcLock);
        DISC_CHECK_AND_RETURN_LOGE(ret == SOFTBUS_OK, DISC_BROADCAST, "mutex err!");
...
{
    static uint32_t callCount = 0;
    DISC_LOGI(DISC_BROADCAST, "enter. adapterBcId=%{public}d, callCount=%{public}u", adapterBcId, callCount++);
    for (uint32_t managerId = 0; managerId < BC_NUM_MAX; managerId++) {
        int32_t ret = SoftBusMutexLock(&g_bcLock);
        DISC_CHECK_AND_RETURN_LOGE(ret == SOFTBUS_OK, DISC_BROADCAST, "mutex err!");
...
{
    DISC_LOGD(DISC_BROADCAST, "enter.");
    for (uint32_t managerId = 0; managerId < BC_NUM_MAX; managerId++) {
        int32_t ret = SoftBusMutexLock(&g_bcLock);
        DISC_CHECK_AND_RETURN_LOGE(ret == SOFTBUS_OK, DISC_BROADCAST, "mutex err!");
...
static int32_t BuildBcInfoCommon(const SoftBusBcScanResult *reportData, BroadcastReportInfo *bcInfo)
{
    ...
    DISC_CHECK_AND_RETURN_RET_LOGE(ret == SOFTBUS_OK, SOFTBUS_MEM_ERR, DISC_BROADCAST, "memcpy addr fail!");

    ret = memcpy_s(bcInfo->localName, BC_LOCAL_NAME_LEN_MAX, reportData->localName, SOFTBUS_LOCAL_NAME_LEN_MAX);
    DISC_CHECK_AND_RETURN_RET_LOGE(ret == SOFTBUS_OK, SOFTBUS_MEM_ERR, DISC_BROADCAST, "memcpy localName fail!");
...
static bool CheckManufactureIsMatch(const BcScanFilter *filter, const BroadcastPayload *bcData)
{
    ...
    if (filter->manufactureId != bcData->id) {
        DISC_LOGD(DISC_BROADCAST, "manufactureId not match!");
        return false;
    }
...
{
    DISC_CHECK_AND_RETURN_LOGE(description != NULL, DISC_BROADCAST, "description is null!");
    DISC_CHECK_AND_RETURN_LOGE(len != 0, DISC_BROADCAST, "description=%{public}s, len is 0!", description);
    DISC_CHECK_AND_RETURN_LOGE(data != NULL, DISC_BROADCAST, "description=%{public}s, data is null!", description);
...
static int32_t BuildBcPayload(int32_t maxPayloadLen, const SoftbusBroadcastPayload *srcData, BroadcastPayload *dstData)
{
    ...
    if (memcpy_s(dstData->payload, bcDataLen, srcData->payload, bcDataLen) != EOK) {
        DISC_LOGE(DISC_BROADCAST, "memcpy payload fail");
...
{
    DISC_LOGD(DISC_BROADCAST, "enter.");
    DISC_CHECK_AND_RETURN_LOGE(reportData != NULL, DISC_BROADCAST, "reportData is null!");

    BroadcastReportInfo bcInfo;
    int32_t ret = BuildBroadcastReportInfo(reportData, &bcInfo);
    DISC_CHECK_AND_RETURN_LOGE(ret == SOFTBUS_OK, DISC_BROADCAST, "BuildBroadcastReportInfo fail!");
...
static int32_t ConvertBroadcastUuid(const SoftbusBroadcastUuid *uuid, BroadcastUuid *bcUuid)
{
    ...
    if (memcpy_s(bcUuid->uuid, bcUuid->uuidLen, uuid->uuid, uuid->uuidLen) != EOK) {
        DISC_LOGE(DISC_BROADCAST, "memcpy_s err!");
...
static void BcLpDeviceInfoCallback(const SoftbusBroadcastUuid *uuid, int32_t type, uint8_t *data, uint32_t dataSize)
{
    ...
    DISC_CHECK_AND_RETURN_LOGE(ret == SOFTBUS_OK, DISC_BROADCAST, "ConvertBroadcastUuid failed!");
...
int32_t RegisterBroadcaster(BaseServiceType srvType, int32_t *bcId, const BroadcastCallback *cb)
{
    ...
    DISC_CHECK_AND_RETURN_RET_LOGE(ret == SOFTBUS_OK, SOFTBUS_LOCK_ERR, DISC_BROADCAST, "mutex err!");
    ...
    *bcId = managerId;
    ...
    if (ret != SOFTBUS_OK) {
        DISC_LOGE(DISC_BROADCAST, "SoftBusCondInit failed!");
        SoftBusMutexUnlock(&g_bcLock);
        return ret;
    }
...
{
    DISC_LOGI(DISC_BROADCAST, "enter. listenerId=%{public}d", listenerId);
    DISC_CHECK_AND_RETURN_RET_LOGE(CheckMediumIsValid(g_interfaceId), SOFTBUS_INVALID_PARAM, DISC_BROADCAST, "bad id");
    DISC_CHECK_AND_RETURN_RET_LOGE(g_interface[g_interfaceId] != NULL, SOFTBUS_BC_MGR_NO_FUNC_REGISTERED,
        DISC_BROADCAST, "interface is null!");
    ...
    if (g_scanManager[listenerId].isScanning) {
        ...
        if (ret != SOFTBUS_OK) {
            DISC_LOGE(DISC_BROADCAST, "stop scan fail!");
            SoftBusMutexUnlock(&g_scanLock);
            return ret;
        }
    }
...
static void ConvertBcParams(const BroadcastParam *srcParam, SoftbusBroadcastParam *dstParam)
{
    ...
    if (memcpy_s(dstParam->peerAddr.addr, SOFTBUS_ADDR_MAC_LEN, srcParam->peerAddr.addr, BC_ADDR_MAC_LEN) != EOK) {
        DISC_LOGE(DISC_BROADCAST, "memcpy peerAddr fail");
        return;
    }
...
static int32_t BuildSoftbusBroadcastData(const BroadcastPacket *packet, SoftbusBroadcastData *softbusBcData)
{
    ...
    DISC_CHECK_AND_RETURN_RET_LOGE(ret == SOFTBUS_OK, ret, DISC_BROADCAST, "BuildSoftbusBcPayload fail!");
...
static void StartBroadcastingWaitSignal(int32_t bcId, SoftBusMutex *mutex)
{
    ...
    DISC_CHECK_AND_RETURN_LOGE(SoftBusMutexLock(mutex) == SOFTBUS_OK, DISC_BROADCAST, "bcLock mutex err!");
...
{
    static uint32_t callCount = 0;
    DISC_LOGI(DISC_BROADCAST, "enter. bcId=%{public}d, callCount=%{public}u", bcId, callCount++);
    DISC_CHECK_AND_RETURN_RET_LOGE(CheckMediumIsValid(g_interfaceId), SOFTBUS_INVALID_PARAM, DISC_BROADCAST, "bad id");
    DISC_CHECK_AND_RETURN_RET_LOGE(g_interface[g_interfaceId] != NULL, SOFTBUS_BC_MGR_NO_FUNC_REGISTERED,
        DISC_BROADCAST, "interface is null!");
...
int32_t UpdateBroadcasting(int32_t bcId, const BroadcastParam *param, const BroadcastPacket *packet)
{
    ...
    DISC_CHECK_AND_RETURN_RET_LOGE(ret == SOFTBUS_OK, ret, DISC_BROADCAST, "StopBroadcasting fail");
...
static bool NeedUpdateScan(int32_t listenerId)
{
    ...
    DISC_CHECK_AND_RETURN_RET_LOGE(g_interface[g_interfaceId] != NULL, false, DISC_BROADCAST, "interface is null!");
...
NEED_RESET:
    if (isScanning) {
        ...
        if (ret != SOFTBUS_OK) {
            DISC_LOGE(DISC_BROADCAST, "call from adapter fail!, ret=%{public}d", ret);
        }
        return true;
    }
...
{
    static uint32_t callCount = 0;
    DISC_LOGI(DISC_BROADCAST, "enter. listenerId=%{public}d, callCount=%{public}u", listenerId, callCount++);
    DISC_CHECK_AND_RETURN_RET_LOGE(param != NULL, SOFTBUS_INVALID_PARAM, DISC_BROADCAST, "invalid param!");
    DISC_CHECK_AND_RETURN_RET_LOGE(CheckMediumIsValid(g_interfaceId), SOFTBUS_INVALID_PARAM, DISC_BROADCAST, "bad id");
    DISC_CHECK_AND_RETURN_RET_LOGE(g_interface[g_interfaceId] != NULL, SOFTBUS_BC_MGR_NO_FUNC_REGISTERED,
        DISC_BROADCAST, "interface is null!");
...
{
    DISC_LOGD(DISC_BROADCAST, "enter.");
    DISC_CHECK_AND_RETURN_RET_LOGE(scanFilter != NULL, SOFTBUS_INVALID_PARAM, DISC_BROADCAST, "param is null!");
...
bool BroadcastIsLpDeviceAvailable(void)
{
    ...
    DISC_CHECK_AND_RETURN_RET_LOGE(g_interface[g_interfaceId]->IsLpDeviceAvailable != NULL,
        false, DISC_BROADCAST, "function is null!");
