...
#define DISC_ASSIGNER(dataType, filedName, filed)                                                     \
    ...
    {                                                                                                 \
        if (!Assigner##dataType(form.discExtra.filed, &param->value)) {                               \
            return false;                                                                             \
        }                                                                                             \
        param->value.t = type;                                                                        \
...
DISC_ASSIGNER(Int32, Errcode, errcode)
...
static const SoftbusEventParamAssigner g_discAssigners[] = {
...
static inline void ConvertDiscForm2Param(SoftbusEventParam params[], size_t size, SoftbusEventForm form)
{
    for (size_t i = 0; i < size; ++i) {
        SoftbusEventParamAssigner assigner = g_discAssigners[i];
