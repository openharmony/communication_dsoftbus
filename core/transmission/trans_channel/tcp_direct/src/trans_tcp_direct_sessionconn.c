...
SessionConn *GetSessionConnByReq(int64_t req)
{
    ...
    SoftBusLog(SOFTBUS_LOG_TRAN, SOFTBUS_LOG_ERROR, "GetSessionConnByReqId fail: reqId=%u", req);
    return NULL;
}
...
void TransDelSessionConnById(int32_t channelId)
{
    ...
    LIST_FOR_EACH_ENTRY_SAFE(item, next, &g_sessionConnList->list, SessionConn, node) {
        if (item->channelId == channelId) {
            if (item->authId > 0) {
                SoftBusLog(SOFTBUS_LOG_TRAN, SOFTBUS_LOG_ERROR, "close auth conn: authId=%d", item->authId);
