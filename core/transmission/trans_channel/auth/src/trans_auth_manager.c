...
#define SERVICE_FOR_ACTION (sizeof(g_serviceForAction) / sizeof(g_serviceForAction[0]))
...
static int32_t OnRequsetUpdateAuthChannel(int32_t authId, AppInfo *appInfo)
{
    ...
    if (SoftBusMutexLock(&g_authChannelList->lock) != SOFTBUS_OK) {
        TRANS_LOGE(TRANS_SVC, "lock failed, auth=%{public}d", authId);
        return SOFTBUS_LOCK_ERR;
    }
    bool exists = false;
    ...
    if (!exists) {
        ...
        if (item == NULL) {
            TRANS_LOGE(TRANS_SVC,
                "init auth channel info failed, ret=%{public}d, authId=%{public}d", ret, authId);
            (void)SoftBusMutexUnlock(&g_authChannelList->lock);
            return ret;
        }
    ...
    if (CopyPeerAppInfo(appInfo, &item->appInfo) != SOFTBUS_OK) {
        ...
        TRANS_LOGE(TRANS_CTRL, "Copy app failed, authId=%{public}d, channelId=%{public}" PRId64,
            authId, item->appInfo.myData.channelId);
        SoftBusFree(item);
...
static bool CheckForAuthWithParam(const char *sessionName, const LaneConnInfo *connInfo, int32_t *channelId)
{
...
static int32_t TransFillAuthChannelInfo(AuthChannelInfo *channel, const LaneConnInfo *connInfo, int32_t *channelId)
{
