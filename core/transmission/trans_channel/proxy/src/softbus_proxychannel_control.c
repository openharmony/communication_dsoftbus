...
int32_t TransProxyHandshake(ProxyChannelInfo *info)
{
    ...
    if (TransProxyTransSendMsg(info->connId, dataInfo.outData, dataInfo.outLen, CONN_HIGH) != SOFTBUS_OK) {
...
int32_t TransProxyAckHandshake(uint32_t connId, ProxyChannelInfo *chan)
{
    ...
    cJSON_free(payLoad);
    if (TransProxyTransSendMsg(connId, dataInfo.outData, dataInfo.outLen, CONN_HIGH) != SOFTBUS_OK) {
...
int32_t TransProxyResetPeer(ProxyChannelInfo *info)
{
    ...
    cJSON_free(payLoad);
    if (TransProxyTransSendMsg(info->connId, dataInfo.outData,  dataInfo.outLen, CONN_LOW) != SOFTBUS_OK) {
