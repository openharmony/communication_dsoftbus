...
static void DfxRecordServerRecvPassiveConnTime(const AuthConnInfo *connInfo, const AuthDataHead *head)
{
    ...
    } else if (connInfo->type == AUTH_LINK_TYPE_WIFI || connInfo->type == AUTH_LINK_TYPE_SESSION_KEY) {
        ret = ConvertBytesToHexString(udidHash, SHORT_UDID_HASH_HEX_LEN + 1,
...
int32_t AuthFlushDevice(const char *uuid, AuthLinkType type)
{
    if (type != AUTH_LINK_TYPE_WIFI) {
        AUTH_LOGE(AUTH_CONN, "invalid auth type");
        return SOFTBUS_INVALID_PARAM;
    }
...
{
    AUTH_CHECK_AND_RETURN_RET_LOGE(handle.type >= 0 && handle.type < AUTH_LINK_TYPE_MAX,
        SOFTBUS_INVALID_PARAM, AUTH_CONN, "handle type is invalid");
    ...
    uint32_t connectionId = GetConnId(auth->connId[handle.type]);
...
void DelAuthManagerByConnectionId(uint32_t connectionId)
{
    ...
    for (uint32_t i = 0; i < num; i++) {
        ...
        AuthHandle authHandle = { .authId = authIds[i], .type = GetConnType(connId) };
        if (GetConnType(connId) == AUTH_LINK_TYPE_SESSION_KEY) {
            AuthNotifyDeviceDisconnect(authHandle);
            RemoveAuthManagerByAuthId(authHandle);
        }
    }
