...
static void ConvertMacStrToBinary(char *macStr, const char *delimit, uint8_t *binMac, int32_t binMacLen)
{
    ...
    while (itemStr != NULL) {
        if (i == binMacLen) {
            CLOGE("mac string over max mac bin len.");
            break;
        }
...
static void DumpGroupInfo(WifiP2pGroupInfo *groupInfo)
{
    ...
    CLOGI("group owner %d size %d.", groupInfo->isP2pGroupOwner, groupInfo->clientDevicesSize);
    for (i = 0; i < groupInfo->clientDevicesSize; i++) {
        macStr[0] = '\0';
        ConvertMacBinToStr(macStr, sizeof(macStr), groupInfo->clientDevices[i].devAddr);
        CLOGI("mac dev[%d].", i);
    }
    CLOGI("group interface %s.", groupInfo->interface);
}
...
NO_SANITIZE("cfi") P2pLinkGroup *P2pLinkRequetGroupInfo(void)
{
    ...
    if (groupInfo == NULL) {
        CLOGE("malloc groupInfo fail.");
        return NULL;
    }
    ret = GetCurrentGroup(groupInfo);
    if (ret != WIFI_SUCCESS) {
        SoftBusFree(groupInfo);
        CLOGE("get current group fail[%d].", ret);
        return NULL;
    }
...
NO_SANITIZE("cfi") static void InnerP2pStateChangedProc(P2pState state)
{
    ...
    CLOGI("recv p2p state change %d.", state);
...
NO_SANITIZE("cfi") static void InnerConnResultProc(WifiP2pLinkedInfo info)
{
    ...
    CLOGI("recv conn state change conn %d grp %d", info.connectState, info.isP2pGroupOwner);
...
{
    (void)devices;
    CLOGI("InnerP2pPeersChangedCallback len %d.", len);
}
...
NO_SANITIZE("cfi") int32_t P2pLinkAdapterInit(const BroadcastRecvCb *cb)
{
    ...
    if (cb == NULL) {
        CLOGE("P2pLinkAdapterInit input para illegal.");
        return SOFTBUS_ERR;
    }
...
static bool GetMacAddr(const char *ifName, unsigned char *macAddr, int32_t len)
{
    ...
    if (memset_s(&ifr, sizeof(ifr), 0, sizeof(ifr)) != EOK ||
        ...
        CLOGE("init ifreq failed.");
        return false;
    }
    int fd = socket(AF_INET, SOCK_DGRAM, 0);
    if (fd < 0) {
        CLOGE("create socket failed.");
        return false;
    }

    if (ioctl(fd, SIOCGIFHWADDR, &ifr) < 0) {
        CLOGE("ioctl SIOCGIFHWADDR failed.");
        close(fd);
        return false;
    }
    close(fd);
    if (memcpy_s(macAddr, len, ifr.ifr_hwaddr.sa_data, len) != EOK) {
        CLOGE("memcpy_s macAddr failed.");
        return false;
    }
    return true;
}
...
static bool GetIpAddr(const char *ifName, char *ipAddr, int32_t len)
{
    ...
    if (ioctl(fd, SIOCGIFADDR, &ifr) < 0) {
        CLOGE("ioctl SIOCGIFADDR failed.");
        close(fd);
        return false;
    }
    close(fd);
    ...
    if (inet_ntop(sin->sin_family, &sin->sin_addr, ipAddr, len) == NULL) {
        CLOGE("inet_ntop ip addr failed.");
        return false;
    }
    return true;
}
...
NO_SANITIZE("cfi") int32_t P2pLinkGetP2pIpAddress(char *ip, int32_t len)
{
    ...
    if (ret != WIFI_SUCCESS) {
        SoftBusFree(groupInfo);
        CLOGE("P2pLinkGetP2pIpAddress GetCurrentGroup fail[%d].", ret);
        return SOFTBUS_ERR;
    }
    CLOGI("interface name %s.", groupInfo->interface);

    if (!GetIpAddr(groupInfo->interface, ipAddr, sizeof(ipAddr))) {
        SoftBusFree(groupInfo);
        CLOGE("GetIpAddr fail.");
        return SOFTBUS_ERR;
    }
    SoftBusFree(groupInfo);
    ...
    CLOGI("P2pLinkGet P2p IpAddress.");
    return SOFTBUS_OK;
}
...
NO_SANITIZE("cfi") int32_t P2pLinkSharelinkRemoveGroup(void)
{
    ...
    if (ret != WIFI_SUCCESS) {
        CLOGE(" Hid2dSharedlinkDecrease fail[%d]", ret);
        return SOFTBUS_ERR;
    }
    CLOGI("P2pLinkSharelinkRemoveGroup ok.");
    return SOFTBUS_OK;
}
...
NO_SANITIZE("cfi") int32_t P2pLinkSharelinkReuse(void)
{
    ...
    if (ret != WIFI_SUCCESS) {
        CLOGE("Hid2dSharedlinkIncrease fail[%d]", ret);
        return SOFTBUS_ERR;
    }
    CLOGI("P2pLinkSharelinkReuse ok.");
    return SOFTBUS_OK;
}
...
NO_SANITIZE("cfi") P2pLink5GList *P2pLinkGetChannelListFor5G(void)
{
    ...
    if (ret != WIFI_SUCCESS) {
        CLOGE("get channel 5g list failed, ret = %d", ret);
        return NULL;
    }
    ...
    if (useCnt == 0) {
        CLOGW("get channel 5g list, no list");
        return NULL;
    }
    ...
    for (int32_t i = 0; i < useCnt; i++) {
        ...
        CLOGD("channel 5g list %d : %d.", i, chanList[i]);
    }

    CLOGD("get channel 5g list success.");
    return channelList;
}
...
NO_SANITIZE("cfi") int32_t P2pLinkGetFrequency(void)
{
    ...
    if (ret != WIFI_SUCCESS) {
        CLOGE("get wifi station freq failed, ret = %d", ret);
        return -1;
    }
    CLOGD("get wifi freq success : %d", wifiInfo.frequency);
...
{
    if (freq == NULL) {
        CLOGE("get recommand channel, invalid param.");
    ...
    if (ret != WIFI_SUCCESS) {
        CLOGE("get recommand channel failed, ret = %d.", ret);
        return SOFTBUS_ERR;
    }
    ...
    CLOGE("no suitable 2.4G and 5G recommnad channel.");
    return SOFTBUS_ERR;
}
...
NO_SANITIZE("cfi") char *P2pLinkGetGroupConfigInfo(void)
{
    ...
    if (!GetMacAddr(groupInfo->interface, macAddr, sizeof(macAddr))) {
        CLOGE("get p2p mac by interface name failed.");
        SoftBusFree(groupInfo);
        return NULL;
    }

    ConvertMacBinToStr(macStr, sizeof(macStr), macAddr);
    CLOGD("p2p interface = %s", groupInfo->interface);
    // 4: \n and \0
    ...
    if (groupCfgStr == NULL) {
        CLOGE("calloc group config str fail");
        SoftBusFree(groupInfo);
        return NULL;
    }
    ...
    SoftBusFree(groupInfo);
    if (ret == -1) {
        CLOGE("sprintf_s group config string failed.");
...
NO_SANITIZE("cfi") int32_t P2pLinkConnectGroup(const char *groupConfig)
{
    ...
    if (outNum < GROUP_CONFIG_ITEM_NUM) {
        CLOGE("group config string is not enough.");
        return SOFTBUS_MEM_ERR;
    }
...
#define IP_INDEX_TREE 3
    ...
    if (ret != WIFI_SUCCESS) {
        CLOGE("request ip failed, ret = %d.", ret);
        return SOFTBUS_ERR;
    }
    ...
    if (ret == -1) {
        CLOGE("sprintf_s request ip failed, ret = %d.", ret);
        return SOFTBUS_MEM_ERR;
    }
    CLOGI("P2pLinkRequestGcIp success");
...
static int32_t ConvertIpStringToIntArray(unsigned int dest[IPV4_ARRAY_LEN], const char *src)
{
    ...
    if (ret == -1) {
        CLOGE("sscanf_s failed, src = %s.", src);
        return SOFTBUS_MEM_ERR;
    }
...
NO_SANITIZE("cfi") int32_t P2pLinkConfigGcIp(const char *ip)
{
    ...
    if (ret != WIFI_SUCCESS) {
        SoftBusFree(groupInfo);
        CLOGE("get current group failed, ret = %d.", ret);
        return SOFTBUS_ERR;
    }

    CLOGI("config ip, interface = %s.", groupInfo->interface);
    IpAddrInfo addrInfo;
    ...
    if (ConvertIpStringToIntArray(addrInfo.ip, ip) != SOFTBUS_OK ||
        ...
        SoftBusFree(groupInfo);
        CLOGE("convert ipaddr string to int array failed.");
        return SOFTBUS_ERR;
    }
    ...
    if (ret != WIFI_SUCCESS) {
        CLOGE("config ip failed, ret = %d.", ret);
        return SOFTBUS_ERR;
    }
...
NO_SANITIZE("cfi") int32_t P2pLinkGetSelfWifiCfgInfo(char *cfgData, int32_t len)
{
    if (cfgData == NULL || len == 0) {
        CLOGE("get self wifi config, invalid param.");
        return SOFTBUS_INVALID_PARAM;
    }
    ...
    if (ret != WIFI_SUCCESS) {
        CLOGE("get self wifi config failed, ret = %d.", ret);
        return SOFTBUS_ERR;
    }

    ret = SoftBusBase64Encode((unsigned char *)cfgData, len, &encodeLen, (unsigned char *)wifiCfg, outLen);
    if (ret != 0) {
        CLOGE("softbus encode wifi config data failed, ret = %d.", ret);
        return SOFTBUS_ERR;
    }
...
NO_SANITIZE("cfi") int32_t P2pLinkSetPeerWifiCfgInfo(const char *cfgData)
{
    if (cfgData == NULL || strlen(cfgData) == 0) {
        CLOGW("peer wifi config data is null.");
        return SOFTBUS_OK;
    }
    ...
    if (peerWifiCfg == NULL) {
        CLOGE("calloc peer wifi config data failed.");
        return SOFTBUS_MEM_ERR;
    }
    ...
    if (ret != 0) {
        CLOGE("softbus decode peer wifi config data failed, ret = %d.", ret);
    ...
    if (ret != WIFI_SUCCESS) {
        CLOGE("set peer wifi config data failed, ret = %d.", ret);
        SoftBusFree(peerWifiCfg);
        return SOFTBUS_ERR;
    }
...
NO_SANITIZE("cfi") int32_t P2pLinkReleaseIPAddr(void)
{
    ...
    if (ret != WIFI_SUCCESS) {
        SoftBusFree(groupInfo);
        CLOGE("P2pLinkReleaseIPAddr GetCurrentGroup fail[%d].", ret);
        return SOFTBUS_ERR;
    }
    ...
    if (ret != WIFI_SUCCESS) {
        SoftBusFree(groupInfo);
        CLOGE("P2pLinkReleaseIPAddr releaseIPAddr fail[%d].", ret);
        return SOFTBUS_ERR;
    }
    SoftBusFree(groupInfo);
    CLOGI("P2pLinkReleaseIPAddr ok.");
    return SOFTBUS_OK;
}
...
NO_SANITIZE("cfi") void P2pLinkRemoveGroup(void)
{
    ...
    ret = RemoveGroup();
    if (ret != WIFI_SUCCESS) {
        CLOGE("remove group faul [%d].", ret);
    }
    CLOGI("remove group ok");
...
NO_SANITIZE("cfi") void P2pLinkRemoveGcGroup(void)
{
    ...
    if (ret != WIFI_SUCCESS) {
        SoftBusFree(groupInfo);
        CLOGE("P2pLinkRemoveGcGroup GetCurrentGroup fail[%d].", ret);
        return;
    }
    ...
    if (ret != WIFI_SUCCESS) {
        SoftBusFree(groupInfo);
        CLOGE("P2pLinkRemoveGcGroup removeGcGroup fail[%d].", ret);
        return;
    }
    SoftBusFree(groupInfo);
    CLOGI("P2pLinkRemoveGcGroup ok.");
    if (g_p2pLinkCallBack.enterDiscState != 0) {
        g_p2pLinkCallBack.enterDiscState();
    }
}