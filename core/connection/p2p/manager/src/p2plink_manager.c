...
void P2pLinkLoopDisconnectDev(P2pLoopMsg msgType, void *arg)
{
    ...
    if (P2pLinkIsEnable() == false) {
        CLOGI("in disconn dev p2p state is closed");
        goto END;
    }

    if (P2pLinkIsDisconnectState() == true) {
        CLOGE("p2plink is disconnecting");
        goto END;
    }
...
static void LoopOpenP2pAuthSuccess(P2pLoopMsg msgType, void *arg)
{
    ...
    if (item == NULL) {
        ...
        CLOGE("auth ok no find item %" PRId64, p2pAuthId);
        return;
    }
    ...
    if (ret != SOFTBUS_OK) {
        AuthCloseConn(p2pAuthId);
        CLOGE("send hand shake fail %" PRId64, p2pAuthId);
        return;
    }
    item->chanId.p2pAuthIdState = P2PLINK_AUTHCHAN_FINISH;
    item->chanId.p2pAuthId = p2pAuthId;
    CLOGI("p2plink handshake success id %d authId %" PRIu64, authRequestId, p2pAuthId);
}
...
static void OpenP2pAuthSuccess(uint32_t requestId, int64_t authId)
{
    ...
    CLOGI("auth success id %d authId %" PRIu64, requestId, authId);
...
static void LoopOpenP2pAuthChan(P2pLoopMsg msgType, void *arg)
{
    ...
    if (connedItem == NULL) {
        CLOGE("p2p auth can not find");
        SoftBusFree(arg);
        return;
    }

    if (P2pLinkIsDisconnectState() == true) {
        CLOGE("p2p dev is disconnect");
        SoftBusFree(arg);
        return;
    }
    ...
    if (P2pLinkGetGcPort() <= 0) {
        ...
        if (gcPort <= 0) {
            CLOGE("gc start listen fail %d", gcPort);
            SoftBusFree(arg);
            return;
        }
        CLOGI("gc start listen ip, gcPort=%d ", gcPort);
    ...
    SoftBusFree(arg);
    CLOGI("open p2p auth chan %d", connedItem->chanId.authRequestId);
}
...
static void P2pLinkStartOpenP2pAuthChan(const P2pLinkNegoConnResult *conn)
{
    ...
    if (authInfo != NULL) {
        ...
        if (ret != SOFTBUS_OK) {
            CLOGE("open auth chan loop fail");
            SoftBusFree(authInfo);
        }
    }
}
...
static void P2pLinkNegoSuccess(int32_t requestId, const P2pLinkNegoConnResult *conn)
{
    ...
    if (conningItem == NULL) {
        CLOGE("Nego ok no find item %d", requestId);
        return;
    }
    P2pLinkRole role = P2pLinkGetRole();
    CLOGI("Nego ok port %d, role %d", conn->goPort, role);
    ...
    if (connedItem != NULL) {
        ...
        CLOGI("P2pLinkNegoSuccess ret %d", ret);
        connedItem->chanId.inAuthId = conningItem->connInfo.authId;
    } else {
    ...
    P2pLinkAddMyP2pRef();
    P2pLinkDumpRef();
    CLOGI("add new dev ok role %d", role);
    if (role == ROLE_GC) {
...
static void P2pLinkNegoFail(int32_t requestId, int32_t reason)
{
    ...
    CLOGI("nego fail reqid %d, reason %d", requestId, reason);
    item = P2pLinkGetConningDevByReqId(requestId);
    if (item == NULL) {
        CLOGE("Nego Fail no find item.");
        return;
    }

    if (reason == ERROR_BUSY) {
        CLOGE("peer dev is busy");
...
static void P2pLinkNegoConnected(const P2pLinkNegoConnResult *conn)
{
    ...
    role = P2pLinkGetRole();
    CLOGI("Nego conned port %d role %d",  conn->goPort, role);
...
NO_SANITIZE("cfi") int32_t P2pLinkMagicInit(void)
{
    ...
    if (ret != SOFTBUS_OK) {
        CLOGE("reg broadcast fail.");
        return SOFTBUS_ERR;
    }

    ret = P2pLinkGetP2pIpAddress(myIp, sizeof(myIp));
    if (ret != SOFTBUS_OK) {
        CLOGE("P2pLinkMagicInit no get p2p ip ");
    }
    P2pLinkSetMyIp(myIp);
...
NO_SANITIZE("cfi") int32_t P2pLinkManagerInit(void)
{
    ...
    if (ret != SOFTBUS_OK) {
        CLOGE("P2p magic Link Init fail.");
        return SOFTBUS_ERR;
    }

    ret = P2pLoopInit();
    if (ret != SOFTBUS_OK) {
        CLOGE("P2p loop Init fail.");
        return SOFTBUS_ERR;
    }

    ret = P2pLinkDevInit();
    if (ret != SOFTBUS_OK) {
        CLOGE("P2p Link dev Init fail.");
        return SOFTBUS_ERR;
    }
    P2pLinkInitRef();
    ...
    if (ret != SOFTBUS_OK) {
        CLOGE("P2p Link nego Init fail.");
        return SOFTBUS_ERR;
    }

    ret = P2pLinkMessageInit();
    if (ret != SOFTBUS_OK) {
        CLOGE("P2p Link msg Init fail.");
        return SOFTBUS_ERR;
    }

    CLOGI("P2p Link Init ok.");
    return SOFTBUS_OK;
}
