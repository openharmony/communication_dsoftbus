...
NO_SANITIZE("cfi") int32_t P2plinkPackRepsonseMsg(const P2pRespMsg *response, P2pContentType type, cJSON *data)
{
    ...
    if (!AddNumberToJsonObject(data, KEY_CONTENT_TYPE, response->contentType) ||
        ...
        CLOGE("pack response msg failed.");
        return SOFTBUS_PARSE_JSON_ERR;
    }
...
NO_SANITIZE("cfi") int32_t P2pLinkUnpackRequestMsg(const cJSON *data, P2pContentType type, P2pRequestMsg *request)
{
    ...
    if (type == CONTENT_TYPE_GO_INFO) {
        ...
        if (UnpackGoInfo(goInfo, data) != SOFTBUS_OK) {
            CLOGE("unpack requset goInfo failed.");
            return SOFTBUS_PARSE_JSON_ERR;
        }
    } else if (type == CONTENT_TYPE_GC_INFO) {
        GcInfo *gcInfo = (GcInfo *)(request->data);
        if (UnpackGcInfo(gcInfo, data) != SOFTBUS_OK) {
            CLOGE("unpack requset gcInfo failed.");
            return SOFTBUS_PARSE_JSON_ERR;
        }
    }
...
NO_SANITIZE("cfi") int32_t P2plinkUnpackRepsonseMsg(const cJSON *data, P2pContentType type, P2pRespMsg *response)
{
    ...
    if (type == CONTENT_TYPE_GO_INFO) {
        GoInfo *goInfo = (GoInfo *)(response->data);
        if (UnpackGoInfo(goInfo, data) != SOFTBUS_OK) {
            CLOGE("unpack response goInfo failed.");
            return SOFTBUS_PARSE_JSON_ERR;
        }
    } else if (type == CONTENT_TYPE_GC_INFO) {
        GcInfo *gcInfo = (GcInfo *)(response->data);
        if (UnpackGcInfo(gcInfo, data) != SOFTBUS_OK) {
            CLOGE("unpack response gcInfo failed.");
            return SOFTBUS_PARSE_JSON_ERR;
        }
    } else if (type == CONTENT_TYPE_RESULT) {
        if (!GetJsonObjectInt32Item(data, KEY_RESULT, &(response->result))) {
            CLOGE("unpack response result failed.");
            return SOFTBUS_PARSE_JSON_ERR;
        }
    }
