...
static void DestroyProxyConnectInfo(ProxyConnectInfo **connectInfo);
static void DestroyProxyConnection(struct ProxyConnection *proxyConnection);
...
static BrProxyListener g_listener = { 0 };
...
static void ProxyChannelDereference(struct ProxyConnection *proxyConnection)
{
    ...
    if (destruct) {
        ...
        DestroyProxyConnection(proxyConnection);
    }
}
...
static void ResetReconnectEvent(uint32_t requestId, const char *brMac)
{
    ...
    if (connectingChannel != NULL && connectingChannel->requestId == requestId &&
        ...
        DestroyProxyConnectInfo(&GetBrProxyChannelManager()->proxyChannelRequestInfo);
    }
}
...
static void ProxyChannelRefresh(struct ProxyChannel *channel, uint32_t newRequestId,
    const OpenProxyChannelCallback *callback)
{
    ...
    if (strcpy_s(connectInfo->brHashMac, sizeof(connectInfo->brHashMac), channel->brMac) != EOK) {
        CONN_LOGE(CONN_PROXY, "cpy brMac failed");
        SoftBusFree(connectInfo);
        return;
    }
    connectInfo->result = *callback;
    connectInfo->requestId = newRequestId;
    int32_t ret = ConnPostMsgToLooper(&g_proxyChannelAsyncHandler, MSG_RECONNECT_DEVICE,
        0, 0, connectInfo, 0);
    if (ret != SOFTBUS_OK) {
        CONN_LOGE(CONN_PROXY, "send msg fail, error=%{public}d", ret);
        SoftBusFree(connectInfo);
...
static struct ProxyConnection *CreateProxyConnection(ProxyConnectInfo *connectInfo)
{
    ...
    proxyConnection->proxyChannel.refresh = ProxyChannelRefresh;
...
static uint32_t AllocateConnectionIdUnsafe(void)
{
    ...
    LIST_FOR_EACH_ENTRY(it, &GetBrProxyChannelManager()->proxyConnectionList->list, struct ProxyConnection, node) {
...
static void PostEventByAddr(enum BrProxyLooperMsgType msgType, const char *brMac, uint64_t delayTimeMs)
{
    ...
    CONN_LOGI(CONN_PROXY, "post event=%{public}d, addr=%{public}s, delay=%{public}" PRIu64, msgType, anonymizeAddress,
        delayTimeMs);
    int32_t ret = ConnPostMsgToLooper(&g_proxyChannelAsyncHandler, msgType, 0, 0, copyAddr, delayTimeMs);
    if (ret != SOFTBUS_OK) {
...
{
    CONN_LOGE(CONN_PROXY, "notify open fail reqId=%{public}u, reason=%{public}d",
        connectingChannel->requestId, reason);
...
static void BrChannelConnectSuccess(uint32_t channelId)
{
    ...
    ctx->isSuccess = true;
    int32_t ret = ConnPostMsgToLooper(&g_proxyChannelAsyncHandler, MSG_OPEN_PROXY_CHANNEL_CONNECT_RESULT,
        0, 0, ctx, 0);
    if (ret != SOFTBUS_OK) {
        CONN_LOGE(CONN_PROXY, "send msg fail, error=%{public}d", ret);
        SoftBusFree(ctx);
...
static bool IsNeedWaitCallbackError(uint32_t channelId, int32_t *errorCode)
{
    ...
    CONN_CHECK_AND_RETURN_RET_LOGE(connection != NULL, false, CONN_PROXY,
        "get connection fail, connId=%{public}d", channelId);
...
static void RemoveProxyChannelByChannelId(uint32_t channelId)
{
    ...
    LIST_FOR_EACH_ENTRY_SAFE(it, next, &GetBrProxyChannelManager()->proxyConnectionList->list,
...
static void HandleConcurrentConnect(ProxyConnectInfo *connectingProxyChannel, const ProxyConnectInfo *connectInfo)
{
    if (StrCmpIgnoreCase(connectInfo->brMac, connectingProxyChannel->brMac) != 0) {
        ...
        connectInfo->result.onOpenFail(
            connectInfo->requestId, SOFTBUS_CONN_PROXY_CONCURRENT_OPERATION_ERR, connectInfo->brMac);
        return;
    }
    ...
    if (connectingProxyChannel->isInnerRequest) {
        connectingProxyChannel->result.onOpenFail(connectingProxyChannel->requestId,
            SOFTBUS_CONN_PROXY_CONCURRENT_OPERATION_ERR, connectingProxyChannel->brMac);
    }
...
{
    ProxyConnectInfo *connectingProxyChannel = GetBrProxyChannelManager()->proxyChannelRequestInfo;
    ...
    if (state == PROXY_CHANNEL_CONNECTING) {
        ...
        if (ret != SOFTBUS_OK) {
            CONN_LOGE(CONN_PROXY, "send msg fail, error=%{public}d", ret);
            DestroyProxyConnectInfo(&copyConnectInfo);
            return false;
        }
        return true;
    }
...
static void AddReconnectDeviceInfoUnsafe(ProxyConnectInfo *connectInfo)
{
    ...
    CONN_CHECK_AND_RETURN_LOGI(target == NULL, CONN_PROXY, "already exist, addr=%{public}s", anomizeAddress);
...
static void RemoveReconnectDeviceInfoByAddrUnsafe(const char *addr)
{
    ...
    LIST_FOR_EACH_ENTRY_SAFE(it, next, &GetBrProxyChannelManager()->reconnectDeviceInfos, ProxyConnectInfo, node) {
        if (StrCmpIgnoreCase(addr, it->brMac) == 0 || StrCmpIgnoreCase(addr, it->brHashMac) == 0) {
            char anomizeAddress[BT_MAC_LEN] = { 0 };
...
static void OpenProxyChannelHandler(ProxyConnectInfo *connectInfo)
{
    ...
    GetBrProxyChannelManager()->proxyChannelRequestInfo = CopyProxyConnectInfo(connectInfo);
    CONN_CHECK_AND_RETURN_LOGE(GetBrProxyChannelManager()->proxyChannelRequestInfo != NULL,
...
static int32_t CreateProxyConnectInfo(ProxyChannelParam *param, const OpenProxyChannelCallback *callback,
    ProxyConnectInfo **connectInfo, bool isRealMac, bool isSupportFarField)
{
    ...
    ctx->isSupportFarField = isSupportFarField;
    ...
    const OpenProxyChannelCallback *callback)
{
    ...
    int32_t ret = CreateProxyConnectInfo(param, callback, &connectInfo, isRealMac, isSupportFarField);
...
static int32_t RegisterProxyChannelListener(BrProxyListener *listener)
{
    ...
    CONN_CHECK_AND_RETURN_RET_LOGE(listener->onBrProxyStateChanged != NULL, SOFTBUS_INVALID_PARAM,
        CONN_PROXY, "onBrProxyStateChanged is NULL");
    g_listener = *listener;
    return SOFTBUS_OK;
}
...
static bool IsTargetDeviceAlreadyConnected(char *brAddr)
{
    ...
    CONN_CHECK_AND_RETURN_RET_LOGI(proxyConnection != NULL, false,
        CONN_PROXY, "not exist same proxyConnection");
...
static void AttemptReconnectDevice(char *brAddr)
{
    ...
    CONN_CHECK_AND_RETURN_LOGW(reconnectDeviceInfo != NULL, CONN_PROXY,
        "not exist same addr=%{public}s need to reconnect", anomizeAddress);
    ...
    CONN_LOGI(CONN_PROXY,
        "start reconnect reqId=%{public}u, addr=%{public}s, times=%{public}u, delay=%{public}" PRIu64,
    ...
    if (ret != SOFTBUS_OK) {
        ...
        DestroyProxyConnectInfo(&proxyChannelRequestInfo);
    }
}
...
static void AclStateChangedHandler(ProxyChannelAclStateContext *context)
{
    ...
    if (!reconnectDeviceInfo->isAclConnected) {
        g_listener.onBrProxyStateChanged(reconnectDeviceInfo->requestId, reconnectDeviceInfo->brMac);
        return;
    }
...
static void ProxyResetHandler(void)
{
    ...
    struct ProxyConnection *next = NULL;
    LIST_FOR_EACH_ENTRY_SAFE(item, next, &GetBrProxyChannelManager()->proxyConnectionList->list,
        struct ProxyConnection, node) {
        ListDelete(&item->node);
...
static void ReconnectDeviceHandler(ProxyConnectInfo *reconnectInfo)
{
    ProxyConnectInfo *connectInfo = GetReconnectDeviceInfoByAddrUnsafe(reconnectInfo->brHashMac);
    CONN_CHECK_AND_RETURN_LOGE(connectInfo != NULL, CONN_PROXY, "not exist same addr");
    char anomizeAddress[BT_MAC_LEN] = { 0 };
    ConvertAnonymizeMacAddress(anomizeAddress, BT_MAC_LEN, connectInfo->brMac, BT_MAC_LEN);
    CONN_LOGW(CONN_PROXY, "refresh to connect, reqId=%{public}d, brmac=%{public}s",
              connectInfo->requestId, anomizeAddress);

    connectInfo->result = reconnectInfo->result;
    connectInfo->requestId = reconnectInfo->requestId;
    connectInfo->timeoutMs = INNER_RECONNECT_TIMEOUT_MS;
    OpenProxyChannelHandler(connectInfo);
}

// Message handler wrapper functions
static void HandleOpenProxyChannelMsg(SoftBusMessage *msg)
{
    CONN_CHECK_AND_RETURN_LOGW(msg->obj != NULL, CONN_PROXY, "msg->obj is NULL");
    OpenProxyChannelHandler((ProxyConnectInfo *)msg->obj);
}

static void HandleOpenProxyChannelTimeoutMsg(SoftBusMessage *msg)
{
    OpenProxyChannelTimeout(msg->arg1);
}

static void HandleOpenProxyChannelRetryMsg(SoftBusMessage *msg)
{
    CONN_CHECK_AND_RETURN_LOGW(msg->obj != NULL, CONN_PROXY, "msg->obj is NULL");
    AttemptReconnectDevice((char*)msg->obj);
}

static void HandleOpenProxyChannelConnectResultMsg(SoftBusMessage *msg)
{
    CONN_CHECK_AND_RETURN_LOGW(msg->obj != NULL, CONN_PROXY, "msg->obj is NULL");
    ProxyChannelConnectResultHandler((ProxyChannelNotifyContext *)msg->obj);
}

static void HandleCloseProxyChannelMsg(SoftBusMessage *msg)
{
    CONN_CHECK_AND_RETURN_LOGW(msg->obj != NULL, CONN_PROXY, "msg->obj is NULL");
    ProxyChannelCloseHandler(msg->arg1, (char *)msg->obj);
}

static void HandleCloseProxyDisconnectMsg(SoftBusMessage *msg)
{
    CONN_CHECK_AND_RETURN_LOGW(msg->obj != NULL, CONN_PROXY, "msg->obj is NULL");
    ProxyChannelDisconnectHandler((ProxyChannelNotifyContext *)msg->obj);
}

static void HandleAclStateChangeMsg(SoftBusMessage *msg)
{
    CONN_CHECK_AND_RETURN_LOGW(msg->obj != NULL, CONN_PROXY, "msg->obj is NULL");
    AclStateChangedHandler((ProxyChannelAclStateContext *)msg->obj);
}

static void HandleProxyBtTurnOffMsg(SoftBusMessage *msg)
{
    (void)msg;
    ProxyResetHandler();
}

static void HandleProxyBtTurnOnMsg(SoftBusMessage *msg)
{
    (void)msg;
    ProxyRestoreHandler();
}

static void HandleProxyUnpairedMsg(SoftBusMessage *msg)
{
    CONN_CHECK_AND_RETURN_LOGW(msg->obj != NULL, CONN_PROXY, "msg->obj is NULL");
    ProxyDeviceUnpaired((const char*)msg->obj);
}

static void HandleDataReceivedMsg(SoftBusMessage *msg)
{
    CONN_CHECK_AND_RETURN_LOGW(msg->obj != NULL, CONN_PROXY, "msg->obj is NULL");
...
static void HandleReconnectDeviceMsg(SoftBusMessage *msg)
{
    CONN_CHECK_AND_RETURN_LOGW(msg->obj != NULL, CONN_PROXY, "msg->obj is NULL");
    ProxyConnectInfo *connectInfo = (ProxyConnectInfo *)(msg->obj);
...
static MsgHandlerCommand g_proxyCommands[] = {
    ...
    {MSG_OPEN_PROXY_CHANNEL_RETRY, HandleOpenProxyChannelRetryMsg},
    {MSG_OPEN_PROXY_CHANNEL_CONNECT_RESULT, HandleOpenProxyChannelConnectResultMsg},
    {MSG_CLOSE_PROXY_CHANNEL, HandleCloseProxyChannelMsg},
    {MSG_CLOSE_PROXY_DISCONNECT, HandleCloseProxyDisconnectMsg},
    {MSG_ACL_STATE_CHANGE, HandleAclStateChangeMsg},
    {MSG_PROXY_BT_TURN_OFF, HandleProxyBtTurnOffMsg},
    {MSG_PROXY_BT_TURN_ON, HandleProxyBtTurnOnMsg},
...
static void ProxyChannelMsgHandler(SoftBusMessage *msg)
{
    ...
    size_t commandSize = sizeof(g_proxyCommands) / sizeof(g_proxyCommands[0]);
    for (size_t i = 0; i < commandSize; i++) {
        if (g_proxyCommands[i].cmd == msg->what) {
            g_proxyCommands[i].func(msg);
            return;
        }
    }
...
int32_t BrProxyChannelManagerInit(void)
{
    ...
    CONN_CHECK_AND_RETURN_RET_LOGE(GetBrProxyChannelManager()->proxyConnectionList != NULL, SOFTBUS_CREATE_LIST_ERR,
        CONN_INIT, "create channels list fail");
    ListInit(&(GetBrProxyChannelManager()->reconnectDeviceInfos));
