...
static int32_t BleUpdateConnection(uint32_t connectionId, UpdateOption *option)
{
    BleConnectionInfo *connInfo = GetBleConnInfoByConnId(connectionId);
    if (connInfo == NULL) {
        SoftBusLog(SOFTBUS_LOG_CONN, SOFTBUS_LOG_ERROR, "update ble connection failed, %u not exist", connectionId);
        return SOFTBUS_ERR;
    }
    if (connInfo->state != BLE_CONNECTION_STATE_CONNECTED &&
        connInfo->state != BLE_CONNECTION_STATE_BASIC_INFO_EXCHANGED) {
        SoftBusLog(SOFTBUS_LOG_CONN, SOFTBUS_LOG_ERROR,
            "update ble connection failed, %u current state is %d, only %d or %d support",
            connectionId, connInfo->state, BLE_CONNECTION_STATE_CONNECTED, BLE_CONNECTION_STATE_BASIC_INFO_EXCHANGED);
        return SOFTBUS_ERR;
    }
    if (connInfo->info.isServer) {
        SoftBusLog(SOFTBUS_LOG_CONN, SOFTBUS_LOG_ERROR,
        "update ble connection failed, %u is server side, only client side support", connectionId);
        return SOFTBUS_ERR;
    }
    ...
    switch (option->bleOption.priority) {
        ...
        case CONN_BLE_PRIORITY_LOW_POWER:
            ...
            break;
        default:
            SoftBusLog(SOFTBUS_LOG_CONN, SOFTBUS_LOG_ERROR, "update ble connection failed, %u, unknown priority: %d",
                connectionId, option->bleOption.priority);
            break;
    }
    return SoftbusGattcSetPriority(connInfo->halConnId, &connInfo->btBinaryAddr, priority);
}
