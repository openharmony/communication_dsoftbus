...
static int32_t UpdateBleServerStateInOrder(enum GattServerState expectedState, enum GattServerState nextState)
{
    ...
    if (g_serverState.state != expectedState) {
        CONN_LOGW(CONN_BLE,
            "update server state fail: actualState=%{public}d, expectedState=%{public}d, nextState=%{public}d",
...
static void ResetServerState()
{
    CONN_CHECK_AND_RETURN_LOGE(SoftBusMutexLock(&g_serverState.lock) == SOFTBUS_OK, CONN_BLE,
        "ATTENTION UNEXPECTED ERROR! ble reset server state fail, try to lock fail");
...
int32_t ConnGattServerStartService(void)
{
    CONN_CHECK_AND_RETURN_RET_LOGE(SoftBusMutexLock(&g_serverState.lock) == SOFTBUS_OK, SOFTBUS_LOCK_ERR, CONN_BLE,
        "ATTENTION UNEXPECTED ERROR! ble server start service fail: try to lock fail");
...
static void BleServiceAddCallback(int32_t status, SoftBusBtUuid *uuid, int32_t srvcHandle)
{
    ...
    CONN_CHECK_AND_RETURN_LOGE(ctx != NULL, CONN_BLE,
        "receive gatt server callback, server added handle fail: calloc service add msg context fail");
    ctx->status = status;
...
static int32_t CheckUuidAndSetServiceHandle(const ServiceAddMsgContext *ctx)
{
    ...
    if (ctx->uuid.uuidLen != strlen(SOFTBUS_SERVICE_UUID) ||
        ...
        ret = SOFTBUS_CONN_BLE_UNDERLAY_UNKNOWN_SERVICE_ERR;
        CONN_LOGE(CONN_BLE, "unknow service id, err=%{public}d", ret);
        return ret;
    }
    if (ctx->status != SOFTBUS_OK) {
        CONN_LOGW(CONN_BLE, "underlay returned status is not success, status=%{public}d", ctx->status);
        ret = SOFTBUS_CONN_BLE_UNDERLAY_SERVER_ADD_SERVICE_FAIL;
        return ret;
    }

    ret = SoftBusMutexLock(&g_serverState.lock);
    if (ret != SOFTBUS_OK) {
        CONN_LOGE(CONN_BLE, "try to lock fail, err=%{public}d", ret);
...
static void BleServiceAddMsgHandler(const ServiceAddMsgContext *ctx)
{
    ...
    do {
        ...
        ret = UpdateBleServerStateInOrder(BLE_SERVER_STATE_SERVICE_ADDING, BLE_SERVER_STATE_SERVICE_ADDED);
        if (ret != SOFTBUS_OK) {
            CONN_LOGE(CONN_BLE, "update server state fail, err=%{public}d", ret);
            break;
        }
        ...
        ret = SoftBusGattsAddCharacteristic(ctx->srvcHandle, uuid,
        ...
        if (ret != SOFTBUS_OK) {
            CONN_LOGW(CONN_BLE, "underlayer add characteristic fail, err=%{public}d", ret);
            ret = SOFTBUS_CONN_BLE_UNDERLAY_CHARACTERISTIC_ADD_ERR;
            break;
        }
        ret = UpdateBleServerStateInOrder(BLE_SERVER_STATE_SERVICE_ADDED, BLE_SERVER_STATE_NET_CHARACTERISTIC_ADDING);
        if (ret != SOFTBUS_OK) {
            CONN_LOGE(CONN_BLE, "update server state fail, err=%{public}d", ret);
            break;
        }
    ...
    if (ret != SOFTBUS_OK) {
        ResetServerState();
        ConnRemoveMsgFromLooper(&g_bleGattServerAsyncHandler, MSG_SERVER_WAIT_START_SERVER_TIMEOUT, 0, 0, NULL);
        g_serverEventListener.onServerStarted(BLE_GATT, ret);
    }
}
    ...
    int32_t status, SoftBusBtUuid *uuid, int32_t srvcHandle, int32_t characteristicHandle)
{
    ...
    CONN_CHECK_AND_RETURN_LOGE(ctx != NULL, CONN_BLE, "calloc characteristic add msg fail");
    ctx->status = status;
    ctx->srvcHandle = srvcHandle;
    ctx->uuid.uuidLen = uuid->uuidLen;
    char *copyUuid = (char *)(ctx + 1);
...
static void BleCharacteristicAddMsgHandler(const CharacteristicAddMsgContext *ctx)
{
    ...
    do {
        ...
        ret = IsConnCharacterisicAndSetHandle(ctx, &serverState);
        if (ret != SOFTBUS_OK) {
            CONN_LOGE(CONN_BLE, "IsConnCharacterisicAndSetHandle fail, err=%{public}d", ret);
            break;
        }
        ret = UpdateBleServerStateInOrder(serverState.expect, serverState.next);
        if (ret != SOFTBUS_OK) {
            CONN_LOGE(CONN_BLE, "update server state fail, err=%{public}d", ret);
            break;
        }
        ...
        ret = SoftBusGattsAddDescriptor(
            ctx->srvcHandle, uuid, SOFTBUS_GATT_PERMISSION_READ | SOFTBUS_GATT_PERMISSION_WRITE);
        if (ret != SOFTBUS_OK) {
            CONN_LOGE(CONN_BLE, "underlayer add decriptor fail, err=%{public}d", ret);
            ret = SOFTBUS_CONN_BLE_UNDERLAY_DESCRIPTOR_ADD_ERR;
            break;
        }
        ret = UpdateBleServerStateInOrder(serverState.next, serverState.nextNext);
        if (ret != SOFTBUS_OK) {
            CONN_LOGE(CONN_BLE, "update server state fail, err=%{public}d", ret);
            break;
        }
...
static void BleDescriptorAddCallback(int32_t status, SoftBusBtUuid *uuid, int32_t srvcHandle, int32_t descriptorHandle)
{
    ...
    CONN_CHECK_AND_RETURN_LOGE(ctx != NULL, CONN_BLE, "calloc descriptor add msg fail");
    ctx->status = status;
    ctx->srvcHandle = srvcHandle;
    ctx->uuid.uuidLen = uuid->uuidLen;
    char *copyUuid = (char *)(ctx + 1);
...
static void BleDescriptorAddMsgHandler(DescriptorAddMsgContext *ctx)
{
    ...
    do {
        if (ctx->uuid.uuidLen != strlen(SOFTBUS_DESCRIPTOR_CONFIGURE_UUID) ||
            ...
            ret = SOFTBUS_CONN_BLE_UNDERLAY_UNKNOWN_DESCRIPTOR_ERR;
            CONN_LOGE(CONN_BLE, "unkown desciptor uuid, err=%{public}d", ret);
            break;
        }
        ret = SoftBusMutexLock(&g_serverState.lock);
        if (ret != SOFTBUS_OK) {
        ...
        if (g_serverState.netDescriptorHandle == -1) {
            ...
            ret = SOFTBUS_CONN_BLE_UNDERLAY_DESCRIPTOR_HANDLE_MISMATCH_ERR;
        }
        (void)SoftBusMutexUnlock(&g_serverState.lock);
        if (ret != SOFTBUS_OK) {
            CONN_LOGE(CONN_BLE, "descriptor handle mismatch, err=%{public}d", ret);
            break;
        }
        ret = isConnDescriptor ? BleConnDescriptorAddMsgHandler(ctx) : BleNetDescriptorAddMsgHandler(ctx);
    } while (false);
    if (ret != SOFTBUS_OK) {
        ResetServerState();
        ConnRemoveMsgFromLooper(&g_bleGattServerAsyncHandler, MSG_SERVER_WAIT_START_SERVER_TIMEOUT, 0, 0, NULL);
...
static int32_t BleNetDescriptorAddMsgHandler(DescriptorAddMsgContext *ctx)
{
    ...
    (void)SoftBusMutexUnlock(&g_serverState.lock);
    ret = UpdateBleServerStateInOrder(BLE_SERVER_STATE_NET_DISCRIPTOR_ADDING, BLE_SERVER_STATE_NET_DISCRIPTOR_ADDED);
    if (ret != SOFTBUS_OK) {
        CONN_LOGE(CONN_BLE, "update server state fail, err=%{public}d", ctx->status);
        return ret;
    }
    SoftBusBtUuid uuid = {
    ...
    ret = SoftBusGattsAddCharacteristic(ctx->srvcHandle, uuid,
        ...
        SOFTBUS_GATT_PERMISSION_READ | SOFTBUS_GATT_PERMISSION_WRITE);
    if (ret != SOFTBUS_OK) {
        CONN_LOGE(CONN_BLE, "underlayer add characteristic fail, err=%{public}d", ret);
...
static int32_t BleConnDescriptorAddMsgHandler(DescriptorAddMsgContext *ctx)
{
    ...
    (void)SoftBusMutexUnlock(&g_serverState.lock);
    ret = UpdateBleServerStateInOrder(BLE_SERVER_STATE_CONN_DISCRIPTOR_ADDING, BLE_SERVER_STATE_CONN_DISCRIPTOR_ADDED);
    if (ret != SOFTBUS_OK) {
        CONN_LOGE(CONN_BLE, "update server state fail, err=%{public}d", ctx->status);
        return ret;
    }
    ret = SoftBusGattsStartService(ctx->srvcHandle);
    if (ret != SOFTBUS_OK) {
        CONN_LOGE(CONN_BLE, "underlayer start service fail, err=%{public}d", ret);
        return SOFTBUS_CONN_BLE_UNDERLAY_SERVICE_START_ERR;
    }
    ret = UpdateBleServerStateInOrder(BLE_SERVER_STATE_CONN_DISCRIPTOR_ADDED, BLE_SERVER_STATE_SERVICE_STARTING);
    if (ret != SOFTBUS_OK) {
        CONN_LOGE(CONN_BLE, "update server state fail, err=%{public}d", ctx->status);
        return ret;
    }
...
static void BleServiceStartCallback(int32_t status, int32_t srvcHandle)
{
    ...
    CONN_CHECK_AND_RETURN_LOGE(ctx != NULL, CONN_BLE, "calloc service start status msg fail");
    ctx->srvcHandle = srvcHandle;
    ctx->status = status;
...
static void BleServiceStartMsgHandler(const CommonStatusMsgContext *ctx)
{
    ...
    do {
        if (ctx->status != SOFTBUS_OK) {
            ret = SOFTBUS_CONN_BLE_UNDERLAY_SERVICE_START_FAIL;
            CONN_LOGW(CONN_BLE, "underlayer return status is not success, status=%{public}d", ctx->status);
            break;
        }
        ret = SoftBusMutexLock(&g_serverState.lock);
        ...
        if (serviceHandle != ctx->srvcHandle) {
            ret = SOFTBUS_CONN_BLE_UNDERLAY_SERVICE_HANDLE_MISMATCH_ERR;
            CONN_LOGE(CONN_BLE, "underlayer srvcHandle mismatch, err=%{public}d", ret);
            break;
        }
        ret = UpdateBleServerStateInOrder(BLE_SERVER_STATE_SERVICE_STARTING, BLE_SERVER_STATE_SERVICE_STARTED);
        if (ret != SOFTBUS_OK) {
            CONN_LOGE(CONN_BLE, "update server state fail, err=%{public}d", ret);
            break;
        }
...
static void BleConnectServerCallback(int32_t underlayerHandle, const SoftBusBtAddr *btAddr)
{
    ...
    if (connection == NULL) {
        CONN_LOGI(CONN_BLE, "create connection fail,disconnect this connection,address=%{public}s", anomizeAddress);
...
static void BleMtuChangeCallback(int32_t underlayerHandle, int32_t mtu)
{
    ...
    ConnBleConnection *connection = ConnBleGetConnectionByHandle(underlayerHandle, CONN_SIDE_SERVER, BLE_GATT);
    if (connection == NULL) {
        CONN_LOGE(CONN_BLE, "mtu changed fail, connection not exist, underlayer handle=%{public}d", underlayerHandle);
        return;
    }
...
{
    CONN_CHECK_AND_RETURN_RET_LOGE(SoftBusMutexLock(&g_serverState.lock) == SOFTBUS_OK, SOFTBUS_LOCK_ERR, CONN_BLE,
        "ATTENTION UNEXPECTED ERROR! ble server stop service fail, try to lock fail");
    enum GattServerState state = g_serverState.state;
    int32_t serviceHandle = g_serverState.serviceHandle;
    (void)SoftBusMutexUnlock(&g_serverState.lock);
...
static void BleServiceStopCallback(int32_t status, int32_t srvcHandle)
{
    ...
    CommonStatusMsgContext *ctx = (CommonStatusMsgContext *)SoftBusCalloc(sizeof(CommonStatusMsgContext));
    CONN_CHECK_AND_RETURN_LOGE(ctx != NULL, CONN_BLE,
        "receive gatt server callback, service stop handle fail: calloc service stop status msg fail");
    ctx->srvcHandle = srvcHandle;
    ctx->status = status;
...
static void BleServiceStopMsgHandler(CommonStatusMsgContext *ctx)
{
    ...
    do {
        if (ctx->status != SOFTBUS_OK) {
            ret = SOFTBUS_CONN_BLE_UNDERLAY_SERVICE_STOP_FAIL;
            CONN_LOGW(CONN_BLE, "underlayer return status is not success, status=%{public}d", ctx->status);
            break;
        }
        ret = SoftBusMutexLock(&g_serverState.lock);
        if (ret != SOFTBUS_OK) {
            CONN_LOGE(CONN_BLE, "try to lock fail, status=%{public}d", ctx->status);
            ret = SOFTBUS_LOCK_ERR;
            break;
        }
        int32_t serviceHandle = g_serverState.serviceHandle;
        ...
        ret = UpdateBleServerStateInOrder(BLE_SERVER_STATE_SERVICE_STOPPING, BLE_SERVER_STATE_SERVICE_STOPPED);
        if (ret != SOFTBUS_OK) {
            CONN_LOGE(CONN_BLE, "update server state fail, err=%{public}d", ret);
            break;
        }
        ret = SoftBusGattsDeleteService(serviceHandle);
        if (ret != SOFTBUS_OK) {
            ret = SOFTBUS_CONN_BLE_UNDERLAY_SERVICE_DELETE_ERR;
            CONN_LOGE(CONN_BLE, "underlay delete service fail, err=%{public}d", ret);
            break;
        }
        ret = UpdateBleServerStateInOrder(BLE_SERVER_STATE_SERVICE_STOPPED, BLE_SERVER_STATE_SERVICE_DELETING);
        if (ret != SOFTBUS_OK) {
            CONN_LOGE(CONN_BLE, "update server state fail, err=%{public}d", ret);
            break;
        }
...
static void BleServiceDeleteCallback(int32_t status, int32_t srvcHandle)
{
    ...
    CommonStatusMsgContext *ctx = (CommonStatusMsgContext *)SoftBusCalloc(sizeof(CommonStatusMsgContext));
    CONN_CHECK_AND_RETURN_LOGE(ctx != NULL, CONN_BLE,
        "gatt server callback, service deleted handle fail: calloc service stop status msg fail");
    ctx->srvcHandle = srvcHandle;
    ctx->status = status;
    if (ConnPostMsgToLooper(&g_bleGattServerAsyncHandler, MSG_SERVER_SERVICE_DELETED, 0, 0, ctx, 0) != SOFTBUS_OK) {
        CONN_LOGE(CONN_BLE, "gatt server callback, service deleted handle fail: post message to looper fail");
        SoftBusFree(ctx);
    }
}
...
static void BleServiceDeleteMsgHandler(const CommonStatusMsgContext *ctx)
{
    ...
    do {
        if (ctx->status != SOFTBUS_OK) {
            ret = SOFTBUS_CONN_BLE_UNDERLAY_SERVICE_DELETE_FAIL;
    ...
    if (isNotifyServerClosed) {
        g_serverEventListener.onServerClosed(BLE_GATT, ret);
    }
}
...
int32_t ConnGattServerSend(ConnBleConnection *connection, const uint8_t *data, uint32_t dataLen, int32_t module)
{
    CONN_CHECK_AND_RETURN_RET_LOGW(connection != NULL, SOFTBUS_INVALID_PARAM, CONN_BLE,
        "ble server send data fail, invalia param, connection is null");
    CONN_CHECK_AND_RETURN_RET_LOGW(data != NULL, SOFTBUS_INVALID_PARAM, CONN_BLE,
        "ble server send data fail, invalia param, data is null");
...
int32_t ConnGattServerConnect(ConnBleConnection *connection)
{
    ...
    if (underlayerHandle == INVALID_UNDERLAY_HANDLE) {
        CONN_LOGE(CONN_BLE, "ble server connect fail, underlay handle is invalid. connId=%{public}u",
...
static void BleServerWaitDisconnectTimeoutHandler(uint32_t connectionId)
{
    ...
    ConnBleConnection *connection = ConnBleGetConnectionById(connectionId);
    CONN_CHECK_AND_RETURN_LOGW(connection != NULL, CONN_BLE,
        "ble server wait disconnect timeout handler fail: connection not exist, connId=%{public}u", connectionId);
...
static int BleCompareGattServerLooperEventFunc(const SoftBusMessage *msg, void *args)
{
    ...
    if (ctx->arg1 != 0 || ctx->arg2 != 0 || ctx->obj != NULL) {
        CONN_LOGE(CONN_BLE,
            ...
            "compare fail to avoid fault silence, "
...
static bool BleIsConcernedAttrHandle(int32_t srvcHandle, int32_t attrHandle)
{
    int32_t ret = SoftBusMutexLock(&g_serverState.lock);
    if (ret != SOFTBUS_OK) {
        CONN_LOGE(CONN_BLE, "try to lock fail, attrHandle=%{public}d", attrHandle);
        return false;
    }
