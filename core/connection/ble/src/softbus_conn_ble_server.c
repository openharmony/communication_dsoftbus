...
static void BleServiceAddMsgHandler(const ServiceAddMsgContext *ctx)
{
    ...
    do {
        if (ctx->uuid.uuidLen != strlen(SOFTBUS_SERVICE_UUID) ||
            ...
            CONN_LOGW(CONN_BLE, "unkown service id, err=%d", rc);
            break;
        }
...
static void BleCharacteristicAddMsgHandler(const CharacteristicAddMsgContext *ctx)
{
    ...
    do {
        ...
        if (ctx->uuid.uuidLen == strlen(SOFTBUS_CHARA_BLENET_UUID) &&
            ...
            CONN_LOGW(CONN_BLE, "unkown characteristic, err=%d", rc);
            break;
        }
        if (ctx->status != SOFTBUS_OK) {
            rc = SOFTBUS_CONN_BLE_UNDERLAY_CHARACTERISTIC_ADD_FAIL;
            CONN_LOGW(CONN_BLE, "underlayer return status is not success, status=%d, err=%d", ctx->status, rc);
            break;
        }
        ...
        if (ctx->srvcHandle != g_serverState.serviceHandle) {
            ...
            CONN_LOGW(CONN_BLE, "srvcHandle is different, context srvcHandle=%d, server srvcHandle=%d, error=%d",
...
static void BleDescriptorAddMsgHandler(DescriptorAddMsgContext *ctx)
{
    ...
    do {
        if (ctx->uuid.uuidLen != strlen(SOFTBUS_DESCRIPTOR_CONFIGURE_UUID) ||
            ...
            CONN_LOGW(CONN_BLE, "unkown desciptor uuid, err=%d", rc);
            break;
        }
        rc = SoftBusMutexLock(&g_serverState.lock);
        if (rc != SOFTBUS_OK) {
...
static void BleConnectServerCallback(int32_t underlayerHandle, const SoftBusBtAddr *btAddr)
{
    ...
    if (connection != NULL) {
        CONN_LOGI(CONN_BLE, "server connected handle trace, connection exist, ignore, connection id=%u, address=%s",
    ...
    if (connection == NULL) {
        CONN_LOGE(CONN_BLE, "create connection failed, disconnect this connection, address=%s", anomizeAddress);
...
static void BleMtuChangeCallback(int32_t underlayerHandle, int32_t mtu)
{
    ...
    ConnBleConnection *connection = ConnBleGetConnectionByHandle(underlayerHandle, CONN_SIDE_SERVER, BLE_GATT);
    if (connection == NULL) {
        CONN_LOGE(CONN_BLE, "mtu changed failed, connection not exist, underlayer handle=%d", underlayerHandle);
        return;
    }
...
static void BleServerWaitMtuTimeoutHandler(uint32_t connectionId)
{
    ...
    CONN_CHECK_AND_RETURN_LOGW(connection != NULL, CONN_BLE,
        "ble server wait mtu timeout handle failed: connection not exist, connId=%u", connectionId);
    int32_t status = ConnGattServerDisconnect(connection);
    CONN_LOGI(CONN_BLE, "ble server wait mtu timeout, disconnect connection, connId=%u, status=%d", connectionId,
        status);
    ConnBleReturnConnection(&connection);
}
...
static void BleRequestWriteCallback(SoftBusGattWriteRequest writeCbPara)
{
    ...
    ConnBleConnection *connection = ConnBleGetConnectionByHandle(underlayerHandle, CONN_SIDE_SERVER, BLE_GATT);
    if (connection == NULL) {
        CONN_LOGE(CONN_BLE, "gatt server callback, request write failed: connection not exist,  underlayer handle=%d",
            underlayerHandle);
        return;
    }
    ...
    if (writeCbPara.attrHandle == g_serverState.netCharacteristicHandle) {
        ...
        CONN_LOGE(CONN_BLE, "request write failed: not NET or CONN characteristic, connId=%u, underlayer handle=%d, "
...
int32_t ConnGattInitServerModule(SoftBusLooper *looper, const ConnBleServerEventListener *listener)
{
    ...
    CONN_CHECK_AND_RETURN_RET_LOGE(status == SOFTBUS_OK, status, CONN_INIT,
