...
static int32_t ConvertCtxToDevice(ConnBrDevice **outDevice, const ConnBrConnectRequestContext *ctx)
{
    ...
    int32_t ret = NewRequest(&request, ctx->requestId, ctx->statistics, &ctx->result);
    if (ret != SOFTBUS_OK) {
        return ret;
    }
    ConnBrDevice *device = NULL;
    ret = NewDevice(&device, ctx->addr);
    if (ret != SOFTBUS_OK) {
        SoftBusFree(request);
        return ret;
    }
    ...
    const ConnBrDevice *device, ConnBrConnection *connection, bool isReuse, int32_t reason)
{
    ...
    ret = Convert2ConnectionInfo(connection, &info);
    if (ret != SOFTBUS_OK) {
        CONN_LOGE(CONN_BR, "convert br connection info fail, error=%{public}d", ret);
    }
...
static int32_t ConnectDeviceDirectly(ConnBrDevice *device, const char *anomizeAddress)
{
    ...
    do {
        if (address == NULL || strcpy_s(address, BT_MAC_LEN, device->addr) != EOK) {
            CONN_LOGW(CONN_BR, "copy br address fail, addr=%{public}s", anomizeAddress);
            ret = SOFTBUS_MEM_ERR;
            break;
        }
...
static bool BrReuseConnection(ConnBrDevice *device, ConnBrConnection *connection)
{
    CONN_CHECK_AND_RETURN_RET_LOGE(SoftBusMutexLock(&connection->lock) == SOFTBUS_OK, false, CONN_BR,
        "br reuse connection fail: lock fail, connId=%{public}u", connection->connectionId);
...
static bool CheckPending(const char *addr)
{
    CONN_CHECK_AND_RETURN_RET_LOGE(SoftBusMutexLock(&g_brManager.pendings->lock) == SOFTBUS_OK, false, CONN_BR,
        "check pending fail: lock pendings fail");
    bool pending = false;
    BrPending *it = NULL;
...
static void ConnectRequestOnAvailableState(const ConnBrConnectRequestContext *ctx)
{
    ...
    if (ret != SOFTBUS_OK) {
        CONN_LOGE(CONN_BR, "ConvertCtxToDevice fail, reqId=%{public}u, error=%{public}d", ctx->requestId, ret);
        DfxRecordBrConnectFail(ctx->requestId, DEFAULT_PID, device, &ctx->statistics, ret);
...
static void ServerAccepted(uint32_t connectionId)
{
    ...
    ConnectionInfo info = { 0 };
    int32_t ret = Convert2ConnectionInfo(connection, &info);
    if (ret != SOFTBUS_OK) {
        CONN_LOGE(CONN_BR, "convert connection info fail, error=%{public}d", ret);
    }
...
static bool IsNeedWaitCallbackError(uint32_t connectionId, int32_t *error)
{
    ...
    CONN_CHECK_AND_RETURN_RET_LOGE(connection != NULL, false, CONN_BR,
        "get conntion fail, connId=%{public}d", connectionId);

    if (SoftBusMutexLock(&connection->lock) != SOFTBUS_OK) {
        CONN_LOGE(CONN_BR, "lock fail, connId=%{public}u", connectionId);
        ConnBrReturnConnection(&connection);
        return false;
    }
    int32_t result = 0;
...
static void ReceivedControlData(ConnBrConnection *connection, const uint8_t *data, uint32_t dataLen)
{
    ...
    if (json == NULL) {
        CONN_LOGE(CONN_BR, "parse json fail, connId=%{public}u", connection->connectionId);
        return;
    }

    int32_t method = 0;
    if (!GetJsonObjectNumberItem(json, KEY_METHOD, &method)) {
        CONN_LOGE(CONN_BR, "parse method fail, connId=%{public}u", connection->connectionId);
        cJSON_Delete(json);
        return;
    }
    ...
    switch (method) {
        ...
        case BR_METHOD_NOTIFY_ACK:
            ret = ConnBrOnAckRequest(connection, json);
            break;
...
static void ConnectionResume(uint32_t connectionId)
{
    ...
    CONN_CHECK_AND_RETURN_LOGW(connection != NULL, CONN_BR,
        "br resume connection fail: connection not exist, connId=%{public}u", connectionId);
    ConnBrDevice *it = NULL;
    ConnBrDevice *next = NULL;
    LIST_FOR_EACH_ENTRY_SAFE(it, next, &g_brManager.waitings, ConnBrDevice, node) {
        if (StrCmpIgnoreCase(it->addr, connection->addr) == 0 && BrReuseConnection(it, connection)) {
...
static void DisconnectRequest(uint32_t connectionId)
{
    ConnBrConnection *connection = ConnBrGetConnectionById(connectionId);
    CONN_CHECK_AND_RETURN_LOGW(connection != NULL, CONN_BR,
        "br disconnect request fail: connection is not exist, connId=%{public}u", connectionId);
    ConnBrUpdateConnectionRc(connection, -1);
    ConnBrReturnConnection(&connection);
}
...
static void UnpendConnection(const char *addr)
{
    ...
    CONN_CHECK_AND_RETURN_LOGE(SoftBusMutexLock(&g_brManager.pendings->lock) == SOFTBUS_OK, CONN_BR,
        "unpend connection: lock fail, addr=%{public}s", anomizeAddress);
...
static void Reset(int32_t reason)
{
    ...
    int32_t ret = SoftBusMutexLock(&g_brManager.pendings->lock);
    if (ret != SOFTBUS_OK) {
        CONN_LOGE(CONN_BR, "lock pendings fail, error=%{public}d", ret);
        return;
    }
    BrPending *pendingIt = NULL;
    ...
    ret = SoftBusMutexLock(&g_brManager.connections->lock);
    if (ret != SOFTBUS_OK) {
        CONN_LOGE(CONN_BR, "lock connections fail, error=%{public}d", ret);
        return;
    }
...
static int BrCompareManagerLooperEventFunc(const SoftBusMessage *msg, void *args)
{
    ...
    if (ctx->arg1 != 0 || ctx->arg2 != 0 || ctx->obj != NULL) {
        CONN_LOGE(CONN_BR,
            "fail to avoid fault silence, what=%{public}d, arg1=%{public}" PRIu64 ", arg2=%{public}" PRIu64
            ", objIsNull=%{public}d",
...
static void OnClientConnectFailed(uint32_t connectionId, int32_t error)
{
    ...
    CONN_CHECK_AND_RETURN_LOGE(ctx != NULL, CONN_BR,
        "OnClientConnectFailed: calloc ctx fail, connId=%{public}u, error=%{public}d", connectionId, error);
...
static void OnDataReceived(uint32_t connectionId, uint8_t *data, uint32_t dataLen)
{
    ...
    if (ctx == NULL) {
        CONN_LOGE(
            CONN_BR, "calloc data received context fail, connId=%{public}u, len=%{public}u", connectionId, dataLen);
        SoftBusFree(data);
        return;
    }
    ctx->connectionId = connectionId;
    ...
    int32_t ret = ConnPostMsgToLooper(&g_brManagerAsyncHandler, MSG_DATA_RECEIVED, 0, 0, ctx, 0);
    if (ret != SOFTBUS_OK) {
        CONN_LOGE(CONN_BR, "post msg to looper fail, connId=%{public}u, len=%{public}u", connectionId, dataLen);
        SoftBusFree(data);
        SoftBusFree(ctx);
    }
}
...
static void OnConnectionException(uint32_t connectionId, int32_t error)
{
    ErrorContext *ctx = (ErrorContext *)SoftBusCalloc(sizeof(ErrorContext));
    CONN_CHECK_AND_RETURN_LOGE(ctx != NULL, CONN_BR,
        "br connection exception: calloc ctx fail, connId=%{public}u, error=%{public}d", connectionId, error);
    ctx->connectionId = connectionId;
    ctx->error = error;
...
int32_t ConnBrSaveConnection(ConnBrConnection *connection)
{
    ...
    if (ret != SOFTBUS_OK) {
        CONN_LOGE(CONN_BR, "lock manager connections fail, error=%{public}d", ret);
        return ret;
    }
    uint32_t connectionId = 0;
    do {
...
void ConnBrRemoveConnection(ConnBrConnection *connection)
{
    ...
    CONN_CHECK_AND_RETURN_LOGE(SoftBusMutexLock(&g_brManager.connections->lock) == SOFTBUS_OK, CONN_BR,
        "br remove connection: lock manager connections fail, connId=%{public}u", connection->connectionId);
...
ConnBrConnection *ConnBrGetConnectionByAddr(const char *addr, ConnSideType side)
{
    ...
    if (ret != SOFTBUS_OK) {
        CONN_LOGE(
            CONN_BR, "lock manager connections fail, addr=%{public}s, error=%{public}d", animizeAddress, ret);
        return NULL;
    }
    ...
    if (target != NULL) {
        ...
        if (ret != SOFTBUS_OK) {
            CONN_LOGE(CONN_BR, "lock connection fail, error=%{public}d", ret);
            SoftBusMutexUnlock(&g_brManager.connections->lock);
            return NULL;
        }
        target->objectRc += 1;
...
{
    int32_t ret = SoftBusMutexLock(&g_brManager.connections->lock);
    CONN_CHECK_AND_RETURN_RET_LOGE(ret == SOFTBUS_OK, NULL, CONN_BR,
        "br get connection by id: lock manager connections fail, connId=%{public}u, error=%{public}d", connectionId,
        ret);
    ...
    if (target != NULL) {
        ret = SoftBusMutexLock(&target->lock);
        if (ret != SOFTBUS_OK) {
            CONN_LOGE(CONN_BR, "lock connection fail, connId=%{public}u, error=%{public}d", connectionId, ret);
            SoftBusMutexUnlock(&g_brManager.connections->lock);
            return NULL;
        }
        target->objectRc += 1;
...
void ConnBrReturnConnection(ConnBrConnection **connectionPtr)
{
    ...
    CONN_CHECK_AND_RETURN_LOGE(SoftBusMutexLock(&connection->lock) == SOFTBUS_OK, CONN_BR,
        "br return connection: lock fail, connId=%{public}u", connection->connectionId);
    connection->objectRc -= 1;
...
static int32_t BrDisconnectDevice(uint32_t connectionId)
{
    ...
    ConnBrReturnConnection(&connection);
    int32_t ret = ConnPostMsgToLooper(&g_brManagerAsyncHandler, MGR_DISCONNECT_REQUEST, connectionId, 0, NULL, 0);
    CONN_LOGI(
        CONN_BR, "connId=%{public}u, address=%{public}s, status=%{public}d", connectionId, animizeAddress, ret);
    return ret;
}
...
static int32_t BrPendConnection(const ConnectOption *option, uint32_t time)
{
    ...
    do {
        ...
        if (copyAddr == NULL || strcpy_s(copyAddr, BT_MAC_LEN, option->brOption.brMac) != EOK) {
            CONN_LOGE(CONN_BR, "copy addr fail, addr=%s", animizeAddress);
        ...
        if (target != NULL) {
            ...
            ret = ConnPostMsgToLooper(&g_brManagerAsyncHandler, MSG_UNPEND, 0, 0, copyAddr,
                (time < BR_CONNECTION_PEND_TIMEOUT_MAX_MILLIS ? time : BR_CONNECTION_PEND_TIMEOUT_MAX_MILLIS));
            if (ret != SOFTBUS_OK) {
                CONN_LOGE(CONN_BR, "post msg fail, addr=%{public}s, error=%{public}d", animizeAddress, ret);
                SoftBusFree(copyAddr);
            }
            break;
        }
        ...
        if (pending == NULL) {
            CONN_LOGE(CONN_BR, "calloc pending object fail");
            ret = SOFTBUS_MALLOC_ERR;
            SoftBusFree(copyAddr);
            break;
        }
        ...
        ret = ConnPostMsgToLooper(&g_brManagerAsyncHandler, MSG_UNPEND, 0, 0, copyAddr,
            (time < BR_CONNECTION_PEND_TIMEOUT_MAX_MILLIS ? time : BR_CONNECTION_PEND_TIMEOUT_MAX_MILLIS));
        if (ret != SOFTBUS_OK) {
            CONN_LOGE(CONN_BR, "post msg to looper fail, addr=%{public}s, error=%{public}d", animizeAddress, ret);
...
static void DumpLocalBtMac(void)
{
    ...
    int32_t ret = SoftBusGetBtMacAddr(&addr);
    if (ret != SOFTBUS_OK) {
        CONN_LOGE(CONN_BR, "get bt Smac fail, error=%{public}d", ret);
        return;
    }
    char myBtMac[BT_MAC_LEN] = { 0 };
    ret = ConvertBtMacToStr(myBtMac, BT_MAC_LEN, addr.addr, sizeof(addr.addr));
    if (ret != SOFTBUS_OK) {
        CONN_LOGW(CONN_BR, "convert bt mac to str fail, error=%{public}d", ret);
        return;
    }
...
static void OnBtStateChanged(int listenerId, int state)
{
    ...
    if (state == SOFTBUS_BR_STATE_TURN_ON) {
        ...
        CONN_LOGI(CONN_BR, "recv bt on, start server, status=%{public}d", ret);
        return;
    }
    ...
    if (state == SOFTBUS_BR_STATE_TURN_OFF) {
        ...
        CONN_LOGI(CONN_BR, "recv bt off, stop server, status=%{public}d", ret);

        ErrorContext *ctx = (ErrorContext *)SoftBusCalloc(sizeof(ErrorContext));
        if (ctx == NULL) {
            CONN_LOGE(CONN_BR, "calloc ctx object fail");
            return;
        }
        ctx->error = SOFTBUS_CONN_BLUETOOTH_OFF;
        ret = ConnPostMsgToLooper(&g_brManagerAsyncHandler, MSG_RESET, 0, 0, ctx, 0);
        if (ret != SOFTBUS_OK) {
            CONN_LOGE(CONN_BR, "post msg to looper fail");
            SoftBusFree(ctx);
        }
        return;
    }
...
static int32_t InitBrManager(void)
{
    ...
    CONN_CHECK_AND_RETURN_RET_LOGE(
        ...
        CONN_INIT, "InitBrManager: create list fail");
    ...
    CONN_CHECK_AND_RETURN_RET_LOGW(ret == SOFTBUS_OK, SOFTBUS_CONN_BR_INTERNAL_ERR, CONN_INIT,
        "InitBrManager: add bt state change listener fail, invalid listenerId=%{public}d", listenerId);
    TransitionToState(BR_STATE_AVAILABLE);
    return SOFTBUS_OK;
}
...
static int32_t InitBrEventListener(void)
{
    ...
    CONN_CHECK_AND_RETURN_RET_LOGE(ret == SOFTBUS_OK, SOFTBUS_CONN_BR_INTERNAL_ERR, CONN_INIT,
        "ConnInitBr: init looper fail, error=%{public}d", ret);
    ...
    CONN_CHECK_AND_RETURN_RET_LOGE(sppDriver != NULL, SOFTBUS_CONN_BR_INTERNAL_ERR, CONN_INIT,
        "ConnInitBr: init spp socket driver fail");
    ...
    CONN_CHECK_AND_RETURN_RET_LOGE(
        ret == SOFTBUS_OK, ret, CONN_INIT, "ConnInitBr: init connection fail, error=%{public}d ", ret);
    ...
    CONN_CHECK_AND_RETURN_RET_LOGE(
        ret == SOFTBUS_OK, ret, CONN_INIT, "ConnInitBr: init trans fail, error=%{public}d", ret);
    return SOFTBUS_OK;
}
...
{
    CONN_CHECK_AND_RETURN_RET_LOGW(
        CheckConnCallbackPara(callback) == SOFTBUS_OK, NULL, CONN_INIT, "ConnInitBr callback para fail");
    CONN_CHECK_AND_RETURN_RET_LOGE(
        InitBrEventListener() == SOFTBUS_OK, NULL, CONN_INIT, "InitBrEventListener init fail");

    int32_t ret = InitBrManager();
    CONN_CHECK_AND_RETURN_RET_LOGE(
        ret == SOFTBUS_OK, NULL, CONN_INIT, "ConnInitBr: init manager fail, error=%{public}d", ret);
    ret = ConnBrInitBrPendingPacket();
    CONN_CHECK_AND_RETURN_RET_LOGE(ret == SOFTBUS_OK, NULL, CONN_INIT,
        "conn init br fail: init pending packet fail, error=%{public}d", ret);
...
int32_t ConnBrDumper(ListNode *connectionSnapshots)
{
    ...
    int32_t ret = SoftBusMutexLock(&g_brManager.connections->lock);
    CONN_CHECK_AND_RETURN_RET_LOGE(
        ret == SOFTBUS_OK, ret, CONN_BLE, "lock br connections fail, error=%{public}d", ret);

    ConnBrConnection *it = NULL;
    LIST_FOR_EACH_ENTRY(it, &g_brManager.connections->list, ConnBrConnection, node) {
        ConnBrConnectionSnapshot *snapshot = ConnBrCreateConnectionSnapshot(it);
        if (snapshot == NULL) {
            CONN_LOGE(CONN_BLE, "br hidumper construct snapshot fail");
            continue;
        }
