                                ...
                                int32_t fd)
{
    ...
    if (fd != 0) {
        ...
        CONN_LOGI(CONN_WIFI_DIRECT, "%{public}s=%{public}s", keyProperty->content, buffer);
    }
}
                                ...
                                int32_t fd)
{
    ...
    if (fd != 0) {
        ...
        CONN_LOGI(CONN_WIFI_DIRECT,
            "%{public}s=%{public}s", keyProperty->content, WifiDirectAnonymizeIp(ipString));
    }
}
                        ...
                        void *item, size_t totalSize)
{
    switch (keyProperty->type) {
        case BOOLEAN:
            CONN_LOGI(CONN_WIFI_DIRECT, "%{public}s=%{public}d",
                      keyProperty->content, self->getBoolean(self, key, false));
            break;
        case INT:
            CONN_LOGI(CONN_WIFI_DIRECT, "%{public}s=%{public}d",
        ...
        case STRING:
            ...
            break;
        case BYTE_ARRAY:
            CONN_LOGI(CONN_WIFI_DIRECT, "%{public}s=%{public}zd", keyProperty->content, totalSize);
            break;
        case INT_ARRAY:
        ...
        case IPV4_INFO_ARRAY:
            ...
            break;
        case AUTH_CONNECTION:
            CONN_LOGI(CONN_WIFI_DIRECT, "%{public}s=%{public}s", keyProperty->content, item ? "not null" : "null");
            break;
        default:
            break;
    }
...
static void Dump(struct InfoContainer *self, int32_t fd)
{
    ...
    CONN_LOGI(CONN_WIFI_DIRECT, "-- %{public}s --", self->getContainerName());
    size_t keyMax = self->getKeySize();
    for (size_t key = 0; key < keyMax; key++) {
        struct InfoContainerKeyProperty *keyProperty = self->getKeyProperty(self, key);
        if (self->dumpFilter && !(keyProperty->flag & DUMP_FLAG)) {
        ...
        if (keyProperty->flag == CONTAINER_FLAG) {
            ...
            CONN_LOGI(CONN_WIFI_DIRECT, ">> %{public}s=%{public}zu", keyProperty->content, itemCount);
            size_t itemSize = totalSize / itemCount;
            for (size_t i = 0; i < itemCount; i++) {
                struct InfoContainer *container = (struct InfoContainer *)((uint8_t *) item + itemSize * i);
                container->dump(container, fd);
