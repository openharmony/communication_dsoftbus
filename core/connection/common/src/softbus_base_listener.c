...
static SoftbusListenerNode *GetListenerNodeCommon(ListenerModule module, bool create)
{
    ...
    CONN_CHECK_AND_RETURN_RET_LOGE(
        status == SOFTBUS_OK, NULL, CONN_COMMON, "lock listener lists failed, module=%d, error=%d", module, status);
    ...
    do {
        ...
        if (status != SOFTBUS_OK) {
            CONN_LOGE(CONN_COMMON, "lock listener failed, module=%d, error=%d", module, status);
            node = NULL;
            break;
        }
        node->objectRc += 1;
...
static void RemoveListenerNode(SoftbusListenerNode *node)
{
    ...
    CONN_CHECK_AND_RETURN_LOGE(
        status == SOFTBUS_OK, CONN_COMMON, "lock listener lists failed, module=%d, error=%d", node->module, status);
    do {
        if (g_listenerList[node->module] != node) {
            CONN_LOGW(CONN_COMMON, "listener node is not in listener list, just skip, module=%d", node->module);
            break;
        }
        status = SoftBusMutexLock(&node->lock);
        if (status != SOFTBUS_OK) {
            CONN_LOGE(CONN_COMMON, "lock listener node failed, module=%d", node->module);
            break;
        }
...
static void ReturnListenerNode(SoftbusListenerNode **nodePtr)
{
    ...
    do {
        ...
        CONN_LOGI(CONN_COMMON, "object reference count <= 0, free listener node, module=%d, object reference=%d",
...
static SoftbusListenerNode *CreateSpecifiedListenerModule(ListenerModule module)
{
    ...
    CONN_CHECK_AND_RETURN_RET_LOGE(
        node != NULL, NULL, CONN_COMMON, "calloc listener node object failed, module=%d", module);
    ...
    if (status != SOFTBUS_OK) {
        CONN_LOGE(CONN_COMMON, "init lock failed, module=%d, error=%d", module, status);
        SoftBusFree(node);
        return NULL;
    }
...
int32_t InitBaseListener(void)
{
    ...
    if (status != SOFTBUS_OK) {
        CONN_LOGE(CONN_INIT, "init select thread lock failed, error=%d", status);
    ...
    if (status != SOFTBUS_OK) {
        ...
        CONN_LOGE(CONN_INIT, "init listener list lock failed, error=%d", status);
        return SOFTBUS_LOCK_ERR;
    }

    status = SoftBusMutexLock(&g_listenerListLock);
    if (status != SOFTBUS_OK) {
        CONN_LOGE(CONN_INIT, "lock listener list failed, error=%d", status);
        SoftBusMutexDestroy(&g_selectThreadStateLock);
    ...
    (void)SoftBusMutexUnlock(&g_listenerListLock);
    if (status != SOFTBUS_OK) {
        CONN_LOGE(CONN_INIT, "create static module listener failed, error=%d", status);
        SoftBusMutexDestroy(&g_selectThreadStateLock);
        SoftBusMutexDestroy(&g_listenerListLock);
        return status;
    }
...
uint32_t CreateListenerModule(void)
{
    int32_t status = SoftBusMutexLock(&g_listenerListLock);
    CONN_CHECK_AND_RETURN_RET_LOGE(
        status == SOFTBUS_OK, UNUSE_BUTT, CONN_COMMON, "lock listener list failed, error=%d", status);
    ...
    for (; module <= LISTENER_MODULE_DYNAMIC_END; module++) {
        ...
        if (node == NULL) {
            CONN_LOGE(CONN_COMMON, "create specified listener module failed, module=%d", module);
            module = UNUSE_BUTT;
        } else {
...
void DestroyBaseListener(ListenerModule module)
{
    CONN_CHECK_AND_RETURN_LOGW(module >= LISTENER_MODULE_DYNAMIC_START && module <= LISTENER_MODULE_DYNAMIC_END,
        CONN_COMMON, "only dynamic module support destroy, module=%d", module);

    CONN_LOGI(CONN_COMMON, "receive request, module=%d", module);
    SoftbusListenerNode *node = GetListenerNode(module);
    if (node == NULL) {
        CONN_LOGW(CONN_COMMON, "listener not exist, module=%d", module);
        return;
    }
    RemoveListenerNode(node);
    ReturnListenerNode(&node);
...
int32_t StartBaseClient(ListenerModule module, const SoftbusBaseListener *listener)
{
    CONN_CHECK_AND_RETURN_RET_LOGW(module >= 0 && module < UNUSE_BUTT, SOFTBUS_INVALID_PARAM, CONN_COMMON,
        "invalid module, module=%d", module);
    CONN_CHECK_AND_RETURN_RET_LOGW(listener != NULL, SOFTBUS_INVALID_PARAM, CONN_COMMON,
        "listener is null, module=%d", module);
    CONN_CHECK_AND_RETURN_RET_LOGW(listener->onConnectEvent != NULL, SOFTBUS_INVALID_PARAM, CONN_COMMON,
        "listener onConnectEvent is null, module=%d", module);
    CONN_CHECK_AND_RETURN_RET_LOGW(listener->onDataEvent != NULL, SOFTBUS_INVALID_PARAM, CONN_COMMON,
        "listener onDataEvent is null, module=%d", module);

    CONN_LOGI(CONN_COMMON, "receive request, module=%d", module);

    SoftbusListenerNode *node = GetOrCreateListenerNode(module);
    CONN_CHECK_AND_RETURN_RET_LOGW(
        node != NULL, SOFTBUS_NOT_FIND, CONN_COMMON, "get listener node failed, module=%d", module);

    int32_t status = SoftBusMutexLock(&node->lock);
    if (status != SOFTBUS_OK) {
        CONN_LOGE(CONN_COMMON, "lock listener node failed, module=%d, error=%d", module, status);
        ReturnListenerNode(&node);
        return SOFTBUS_LOCK_ERR;
    }
    do {
        if (node->info.status != LISTENER_IDLE) {
            CONN_LOGE(CONN_COMMON, "listener is not idle status, module=%d, status=%d", module, node->info.status);
            status = SOFTBUS_ERR;
            break;
        }
        ...
        if (status != SOFTBUS_OK) {
            CONN_LOGE(CONN_COMMON, "start select thread failed, module=%d, "
...
static int32_t StartServerListenUnsafe(SoftbusListenerNode *node, const LocalListenerInfo *info)
{
    ...
    if (socketIf == NULL) {
        CONN_LOGE(CONN_COMMON, "not find protocal implement, module=%d, protocal=%d", module, protocol);
...
int32_t StartBaseListener(const LocalListenerInfo *info, const SoftbusBaseListener *listener)
{
    ...
    CONN_CHECK_AND_RETURN_RET_LOGW(info->type == CONNECT_TCP || info->type == CONNECT_P2P, SOFTBUS_INVALID_PARAM,
        CONN_COMMON, "only CONNECT_TCP(%d) and CONNECT_P2P(%d) is permitted, type=%d",
    ...
    CONN_CHECK_AND_RETURN_RET_LOGW(info->socketOption.port >= 0, SOFTBUS_INVALID_PARAM, CONN_COMMON,
        "port is invalid, port=%d", info->socketOption.port);
    CONN_CHECK_AND_RETURN_RET_LOGW(info->socketOption.moduleId >= 0 && info->socketOption.moduleId < UNUSE_BUTT,
        SOFTBUS_INVALID_PARAM, CONN_COMMON, "invalid module, module=%d", info->socketOption.moduleId);
    CONN_CHECK_AND_RETURN_RET_LOGW(listener != NULL, SOFTBUS_INVALID_PARAM, CONN_COMMON,
        "listener is null, module=%d", info->socketOption.moduleId);
    CONN_CHECK_AND_RETURN_RET_LOGW(listener->onConnectEvent != NULL, SOFTBUS_INVALID_PARAM, CONN_COMMON,
        "listener onConnectEvent is null, module=%d", info->socketOption.moduleId);
    CONN_CHECK_AND_RETURN_RET_LOGW(listener->onDataEvent != NULL, SOFTBUS_INVALID_PARAM, CONN_COMMON,
        "listener onDataEvent is null, module=%d", info->socketOption.moduleId);
    ...
    do {
        ...
        node->listener.onDataEvent = listener->onDataEvent;
        if (memcpy_s(&node->info.listenerInfo, sizeof(LocalListenerInfo), info, sizeof(LocalListenerInfo)) != EOK) {
            CONN_LOGE(CONN_COMMON, "memcpy_s listener info failed, module=%d", node->module);
        ...
        status = StartSelectThread();
        if (status != SOFTBUS_OK) {
            CONN_LOGE(CONN_COMMON, "start listener thread failed, module=%d, status=%d",
                module, status);
...
{
    CONN_CHECK_AND_RETURN_RET_LOGW(
        module >= 0 && module < UNUSE_BUTT, SOFTBUS_INVALID_PARAM, CONN_COMMON, "invalid module, module=%d", module);

    CONN_LOGI(CONN_COMMON, "receive request, module=%d", module);
    SoftbusListenerNode *node = GetListenerNode(module);
    CONN_CHECK_AND_RETURN_RET_LOGW(
        node != NULL, SOFTBUS_NOT_FIND, CONN_COMMON, "listener node not exist, module=%d", module);

    int32_t status = ShutdownBaseListener(node);
    if (status != SOFTBUS_OK) {
        CONN_LOGE(CONN_COMMON, "stop listen thread failed, module=%d, error=%d", module, status);
    }
    ReturnListenerNode(&node);
    return status;
}
...
static int32_t ShutdownBaseListener(SoftbusListenerNode *node)
{
    ...
    CONN_CHECK_AND_RETURN_RET_LOGE(status == SOFTBUS_OK, SOFTBUS_LOCK_ERR, CONN_COMMON,
        "lock listener node failed, module=%d, error=%d", node->module, status);

    do {
        if (node->info.status != LISTENER_RUNNING) {
            CONN_LOGW(CONN_COMMON, "listener is not running, just skip, module=%d, error=%d", node->module,
                node->info.status);
            break;
        }
        status = StopSelectThread();
        if (status != SOFTBUS_OK) {
            CONN_LOGE(CONN_COMMON, "stop select thread failed, module=%d, error=%d", node->module, status);
            break;
        }
        ...
        LIST_FOR_EACH_ENTRY_SAFE(it, next, &node->info.waitEventFds, FdNode, node) {
            CONN_LOGE(CONN_COMMON, "listener node there is fd not close, module=%d, fd=%d, trigger set=%u",
        ...
        if (node->info.modeType == SERVER_MODE && listenFd > 0) {
            CONN_LOGE(
                CONN_COMMON, "close server, module=%d, listen fd=%d, port=%d", node->module, listenFd, listenPort);
...
int32_t AddTrigger(ListenerModule module, int32_t fd, TriggerType trigger)
{
    CONN_CHECK_AND_RETURN_RET_LOGW(module >= 0 && module < UNUSE_BUTT, SOFTBUS_INVALID_PARAM, CONN_COMMON,
        "invalid module, module=%d", module);
    CONN_CHECK_AND_RETURN_RET_LOGW(
        fd > 0, SOFTBUS_INVALID_PARAM, CONN_COMMON, "invalid fd, module=%d, fd=%d", module, fd);
    CONN_CHECK_AND_RETURN_RET_LOGW(IsValidTriggerType(trigger), SOFTBUS_INVALID_PARAM, CONN_COMMON,
        "invalid trigger, module=%d, fd=%d, trigger=%d", module, fd, trigger);

    CONN_LOGI(CONN_COMMON, "receive request, module=%d, fd=%d, trigger=%d", module, fd, trigger);
    SoftbusListenerNode *node = GetListenerNode(module);
    CONN_CHECK_AND_RETURN_RET_LOGW(node != NULL, SOFTBUS_NOT_FIND, CONN_COMMON,
        "listener node not exist, module=%d, fd=%d, trigger=%d", module, fd, trigger);

    int32_t status = SoftBusMutexLock(&node->lock);
    if (status != SOFTBUS_OK) {
        CONN_LOGE(CONN_COMMON, "lock listener node failed, module=%d, fd=%d, trigger=%d, error=%d", module, fd, trigger,
            status);
        ReturnListenerNode(&node);
        return SOFTBUS_LOCK_ERR;
    }
    ...
    do {
        if (node->info.status != LISTENER_RUNNING) {
            CONN_LOGE(CONN_COMMON, "module is not running, module=%d, fd=%d, trigger=%d", module, fd, trigger);
            status = SOFTBUS_ERR;
            break;
        }
        ...
        if (node->info.waitEventFdsLen > MAX_LISTEN_EVENTS) {
            CONN_LOGE(CONN_COMMON,
                "can not trigger more, fd exceed more than %d, module=%d, fd=%d, trigger=%d, wait event fds len=%d",
        ...
        if (fdNode == NULL) {
            CONN_LOGE(CONN_COMMON, "calloc fd node object failed, module=%d, fd=%d, trigger=%d", module, fd, trigger);
...
int32_t DelTrigger(ListenerModule module, int32_t fd, TriggerType trigger)
{
    ...
    CONN_CHECK_AND_RETURN_RET_LOGW(fd > 0, SOFTBUS_INVALID_PARAM, CONN_COMMON,
        "invalid fd, module=%d, fd=%d", module, fd);
    CONN_CHECK_AND_RETURN_RET_LOGW(IsValidTriggerType(trigger), SOFTBUS_INVALID_PARAM, CONN_COMMON,
        "invalid trigger, module=%d, fd=%d, trigger=%d", module, fd, trigger);
    ...
    do {
        ...
        if (target == NULL) {
            CONN_LOGW(CONN_COMMON, "fd node not exist, module=%d, fd=%d, trigger=%d", module, fd, trigger);
        ...
        if ((target->triggerSet & trigger) == 0) {
            CONN_LOGW(CONN_COMMON,
                "without add trigger before, repeat delete trigger or mismatch module. module=%d, fd=%d,  want delete "
                "trigger=%d, exist trigger set=%u",
                module, fd, trigger, it->triggerSet);
            // consider delete trigger success,
            status = SOFTBUS_OK;
            break;
        ...
        CONN_LOGI(
            CONN_COMMON, "delete trigger success, free fd node now, module=%d, fd=%d, trigger=%d", module, fd, trigger);
    ...
    while (true) {
        ...
        if (len < 0) {
            ...
            if (status == EINTR) {
                ...
                CONN_LOGE(
                    CONN_COMMON, "wakeup trace=%d, fd=%d, invalid read len=%d, error=%d", wakeupTrace, fd, len, status);
                break;
            }
        }
        CONN_LOGI(CONN_COMMON, "wakeup ctrl message received, wakeup trace=%d, fd=%d, ctrl trace=%d, read length=%d",
    ...
    const SocketInterface *socketIf, const SoftbusBaseListener *listener, int32_t wakeupTrace)
{
    CONN_CHECK_AND_RETURN_RET_LOGW(socketIf != NULL, SOFTBUS_INVALID_PARAM, CONN_COMMON,
        "socket interface implement is null, wakeup trace=%d module=%d", wakeupTrace, module);
    CONN_CHECK_AND_RETURN_RET_LOGW(socketIf->AcceptClient != NULL, SOFTBUS_INVALID_PARAM, CONN_COMMON,
        "socket interface implement not support AcceptClient method, wakeup trace=%d, module=%d", wakeupTrace, module);
    ...
    while (true) {
        ...
        if (listener->onConnectEvent != NULL) {
            CONN_LOGI(CONN_COMMON,
                "trigger ACCEPT event, wakeup trace=%d, module=%d, listen fd=%d, client ip=%s, client fd=%d",
                wakeupTrace, module, listenFd, animizedIp, clientFd);
            listener->onConnectEvent(module, clientFd, &clientAddr);
        } else {
            CONN_LOGE(CONN_COMMON,
                "trigger ACCEPT event, but event listener not registered, wakeup trace=%d, module=%d, listen fd=%d, "
                "client ip=%s, client fd=%d",
                wakeupTrace, module, listenFd, animizedIp, clientFd);
...
static int32_t CopyWaitEventFdsUnsafe(const SoftbusListenerNode *node, FdNode **outArray, uint32_t *outArrayLen)
{
    ...
    CONN_CHECK_AND_RETURN_RET_LOGE(fdArray != NULL, SOFTBUS_MALLOC_ERR, CONN_COMMON,
        "calloc fd node array object failed, module=%d, wait event len=%u", node->module, fdArrayLen);
    ...
    LIST_FOR_EACH_ENTRY(item, &node->info.waitEventFds, FdNode, node) {
        if (i >= fdArrayLen) {
            ...
            if (tmp == NULL) {
                CONN_LOGE(CONN_COMMON, "expand calloc fd node array object failed, module=%d, wait event len=%u",
                    node->module, tmpLen);
                SoftBusFree(fdArray);
    ...
    if (expand) {
        CONN_LOGE(CONN_COMMON,
            "listener node 'waitEventFdsLen' field is unexpected, actual wait event fd size larger than it, module=%d, "
            "wait event fds len=%u, actual wait event fds len=%u",
            node->module, node->info.waitEventFdsLen, i);
    } else if (i != fdArrayLen) {
        CONN_LOGE(CONN_COMMON,
            "listener node 'waitEventFdsLen' field is unexpected, actual wait event fd size lower than it, module=%d, "
            "wait event fds len=%u, actual wait event fds len=%u",
            node->module, node->info.waitEventFdsLen, i);
    }
...
{
    CONN_CHECK_AND_RETURN_LOGE(
        SoftBusMutexLock(&node->lock) == SOFTBUS_OK, CONN_COMMON, "lock listener node failed, module=%d", node->module);
    do {
        if (node->info.status != LISTENER_RUNNING || node->info.modeType != SERVER_MODE ||
            node->info.listenFd != listenFd) {
            break;
        }
        CONN_LOGW(CONN_COMMON, "forcely close to prevent repeat wakeup select, module=%d, invalid listen "
    ...
    SoftBusFdSet *exceptSet, int32_t wakeupTrace)
{
    CONN_CHECK_AND_RETURN_LOGE(SoftBusMutexLock(&node->lock) == SOFTBUS_OK, CONN_COMMON,
        "lock listener node failed, wakeup trace=%d, module=%d", wakeupTrace, node->module);
    if (node->info.status != LISTENER_RUNNING) {
        SoftBusMutexUnlock(&node->lock);
        return;
    }
    ...
    SoftBusMutexUnlock(&node->lock);
    if (status != SOFTBUS_OK) {
        CONN_LOGE(CONN_COMMON, "copy wait event fds failed, wakeup trace=%d, module=%d, error=%d", wakeupTrace,
            node->module, status);
        return;
    }
    ...
    for (uint32_t i = 0; i < fdArrayLen; i++) {
        if ((fdArray[i].triggerSet & READ_TRIGGER) != 0 && SoftBusSocketFdIsset(fdArray[i].fd, readSet)) {
            CONN_LOGD(CONN_COMMON, "trigger IN event, wakeup trace=%d, module=%d, fd=%d, trigger set=%u", wakeupTrace,
                node->module, fdArray[i].fd, fdArray[i].triggerSet);
        ...
        if ((fdArray[i].triggerSet & WRITE_TRIGGER) != 0 && SoftBusSocketFdIsset(fdArray[i].fd, writeSet)) {
            CONN_LOGD(CONN_COMMON, "trigger OUT event, wakeup trace=%d, module=%d, fd=%d, trigger set=%u", wakeupTrace,
                node->module, fdArray[i].fd, fdArray[i].triggerSet);
        ...
        if ((fdArray[i].triggerSet & EXCEPT_TRIGGER) != 0 && SoftBusSocketFdIsset(fdArray[i].fd, exceptSet)) {
            CONN_LOGW(CONN_COMMON,
                "trigger EXCEPTION(out-of-band data) event, wakeup trace=%d, module=%d, fd=%d, trigger set=%u",
    ...
    SoftbusListenerNode *node, SoftBusFdSet *readSet, SoftBusFdSet *writeSet, SoftBusFdSet *exceptSet)
{
    CONN_CHECK_AND_RETURN_RET_LOGE(SoftBusMutexLock(&node->lock) == SOFTBUS_OK, SOFTBUS_LOCK_ERR, CONN_COMMON,
        "lock listener node failed, module=%d", node->module);
...
static void *SelectTask(void *arg)
{
    ...
    while (true) {
        ...
        if (status != SOFTBUS_OK) {
            CONN_LOGE(CONN_COMMON, "lock select thread state self failed, retry after %d ms, select trace=%d, error=%d",
        ...
        if (maxFdOrStatus < 0) {
            CONN_LOGE(CONN_COMMON, "collect wait event fd set failed, retry after %d ms, select trace=%d, error=%d",
        ...
        if (nEvents <= 0) {
            CONN_LOGE(CONN_COMMON, "unexpect wakeup, retry after %d ms, wakeup trace id=%d, events=%d",
...
static int32_t StartSelectThread(void)
{
    ...
    do {
        if (g_selectThreadState != NULL) {
            ...
            if (status != SOFTBUS_OK) {
                CONN_LOGE(CONN_COMMON, "lock select thread state self failed, error=%d", status);
                status = SOFTBUS_LOCK_ERR;
                break;
            }
            ...
            CONN_LOGW(CONN_COMMON,
                "select thread is already start, select trace=%d, ctrl read fd=%d, ctrl write "
...
#endif
        if (rc != 0) {
            CONN_LOGE(CONN_COMMON, "create ctrl pipe failed, error=%s", strerror(errno));
            SoftBusFree(state);
            status = SOFTBUS_ERR;
            break;
        }
        ...
        if (status != SOFTBUS_OK) {
            CONN_LOGE(CONN_COMMON, "start select task async failed, error=%d", status);
        ...
        if (status != SOFTBUS_OK) {
            CONN_LOGE(CONN_COMMON, "init lock failed, error=%d", status);
            CleanupSelectThreadState(&state);
            break;
        }
...
static void WakeupSelectThread(void)
{
    ...
    int32_t status = SoftBusMutexLock(&g_selectThreadStateLock);
    CONN_CHECK_AND_RETURN_LOGE(
        status == SOFTBUS_OK, CONN_COMMON, "lock global select thread state failed, error=%d", status);
    do {
        if (g_selectThreadState == NULL) {
        ...
        CONN_LOGI(CONN_COMMON, "wakeup ctrl message sent, write length=%d, ctrl trace=%d", len, ctrlTraceId);
    } while (false);
