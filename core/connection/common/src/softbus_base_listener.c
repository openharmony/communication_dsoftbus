...
static SoftBusMutex g_removeAbnormalFdLock = { 0 };
...
static SoftbusListenerNode *GetListenerNodeCommon(ListenerModule module, bool create)
{
    ...
    CONN_CHECK_AND_RETURN_RET_LOGE(
        ret == SOFTBUS_OK, NULL, CONN_COMMON, "lock fail, module=%{public}d, error=%{public}d",
        module, ret);
    ...
    do {
        ...
        if (ret != SOFTBUS_OK) {
            CONN_LOGE(CONN_COMMON, "lock listener fail, module=%{public}d, error=%{public}d", module, ret);
            node = NULL;
            break;
        }
        node->objectRc += 1;
...
static void RemoveListenerNode(SoftbusListenerNode *node)
{
    ...
    CONN_CHECK_AND_RETURN_LOGE(
        ret == SOFTBUS_OK, CONN_COMMON, "lock listener lists fail, module=%{public}d, error=%{public}d",
        node->module, ret);
    do {
...
static SoftbusListenerNode *CreateSpecifiedListenerModule(ListenerModule module)
{
    ...
    int32_t ret = SoftBusMutexInit(&node->lock, NULL);
    if (ret != SOFTBUS_OK) {
        CONN_LOGE(CONN_COMMON, "init lock fail, module=%{public}d, error=%{public}d", module, ret);
        SoftBusFree(node);
        return NULL;
    }
...
static int32_t InitBaseListenerLock(void)
{
    ...
    int32_t ret = SoftBusMutexInit(&g_watchThreadStateLock, &attr);
    if (ret != SOFTBUS_OK) {
        CONN_LOGE(CONN_INIT, "init watch thread lock fail, error=%{public}d", ret);
    ...
    if (ret != SOFTBUS_OK) {
        ...
        CONN_LOGE(CONN_INIT, "init listener list lock fail, error=%{public}d", ret);
        return SOFTBUS_LOCK_ERR;
    }
    ret = SoftBusMutexInit(&g_removeAbnormalFdLock, &attr);
    if (ret != SOFTBUS_OK) {
        SoftBusMutexDestroy(&g_watchThreadStateLock);
        SoftBusMutexDestroy(&g_listenerListLock);
        CONN_LOGE(CONN_INIT, "init remove abnormal fd lock fail, error=%{public}d", ret);
        return SOFTBUS_LOCK_ERR;
    }
    return SOFTBUS_OK;
}
...
int32_t InitBaseListener(void)
{
    ...
    int32_t ret = SoftBusMutexLock(&g_listenerListLock);
    if (ret != SOFTBUS_OK) {
        CONN_LOGE(CONN_INIT, "lock listener list fail, error=%{public}d", ret);
        SoftBusMutexDestroy(&g_watchThreadStateLock);
        SoftBusMutexDestroy(&g_listenerListLock);
        SoftBusMutexDestroy(&g_removeAbnormalFdLock);
        return SOFTBUS_LOCK_ERR;
    }
...
uint32_t CreateListenerModule(void)
{
    int32_t ret = SoftBusMutexLock(&g_listenerListLock);
    CONN_CHECK_AND_RETURN_RET_LOGE(
        ret == SOFTBUS_OK, UNUSE_BUTT, CONN_COMMON, "lock fail, error=%{public}d", ret);
...
int32_t StartBaseClient(ListenerModule module, const SoftbusBaseListener *listener)
{
    ...
    int32_t ret = SoftBusMutexLock(&node->lock);
    if (ret != SOFTBUS_OK) {
        CONN_LOGE(CONN_COMMON, "lock listener node fail, module=%{public}d, error=%{public}d", module, ret);
        ReturnListenerNode(&node);
        return SOFTBUS_LOCK_ERR;
    }
    do {
...
int32_t StartBaseListener(const LocalListenerInfo *info, const SoftbusBaseListener *listener)
{
    ...
    int32_t ret = SoftBusMutexLock(&g_removeAbnormalFdLock);
    if (ret != SOFTBUS_OK) {
        CONN_LOGE(CONN_COMMON, "lock fail, module=%{public}d, error=%{public}d", module, ret);
    ...
    SoftbusListenerNode *node = GetOrCreateListenerNode(module);
    if (node == NULL) {
        CONN_LOGE(CONN_COMMON, "get listener node fail, module=%{public}d", module);
        SoftBusMutexUnlock(&g_removeAbnormalFdLock);
        FillConnEventExtra(info, &extra, SOFTBUS_NOT_FIND);
        CONN_EVENT(EVENT_SCENE_START_BASE_LISTENER, EVENT_STAGE_TCP_COMMON_ONE, extra);
        return SOFTBUS_NOT_FIND;
    }
    ret = SoftBusMutexLock(&node->lock);
    if (ret != SOFTBUS_OK) {
        CONN_LOGE(CONN_COMMON, "lock node fail, module=%{public}d, error=%{public}d", module, ret);
        SoftBusMutexUnlock(&g_removeAbnormalFdLock);
        ReturnListenerNode(&node);
        FillConnEventExtra(info, &extra, SOFTBUS_LOCK_ERR);
        CONN_EVENT(EVENT_SCENE_START_BASE_LISTENER, EVENT_STAGE_TCP_COMMON_ONE, extra);
    ...
    do {
        ...
        ret = AddEvent(g_eventWatcher, node->info.listenFd, READ_TRIGGER);
        if (ret != SOFTBUS_OK) {
    ...
    (void)SoftBusMutexUnlock(&node->lock);
    (void)SoftBusMutexUnlock(&g_removeAbnormalFdLock);
    ReturnListenerNode(&node);
    FillConnEventExtra(info, &extra, ret);
    CONN_EVENT(EVENT_SCENE_START_BASE_LISTENER, EVENT_STAGE_TCP_COMMON_ONE, extra);
    return ret == SOFTBUS_OK ? listenPort : ret;
}
...
int32_t StopBaseListener(ListenerModule module)
{
    ...
    int32_t ret = ShutdownBaseListener(node);
    if (ret != SOFTBUS_OK) {
        CONN_LOGE(CONN_COMMON, "stop listen thread fail, module=%{public}d, error=%{public}d", module, ret);
    }
    ReturnListenerNode(&node);
    extra.errcode = ret;
...
{
    int32_t ret = SoftBusMutexLock(&g_removeAbnormalFdLock);
    CONN_CHECK_AND_RETURN_RET_LOGE(ret == SOFTBUS_OK, SOFTBUS_LOCK_ERR, CONN_COMMON,
        "lock fail, module=%{public}d, error=%{public}d", node->module, ret);
    ret = SoftBusMutexLock(&node->lock);
    if (ret != SOFTBUS_OK) {
        CONN_LOGE(CONN_COMMON, "lock node fail, module=%{public}d, error=%{public}d", node->module, ret);
        SoftBusMutexUnlock(&g_removeAbnormalFdLock);
        return SOFTBUS_LOCK_ERR;
    }
...
int32_t AddTrigger(ListenerModule module, int32_t fd, TriggerType trigger)
{
    ...
    int32_t ret = SoftBusMutexLock(&g_removeAbnormalFdLock);
    CONN_CHECK_AND_RETURN_RET_LOGW(ret == SOFTBUS_OK, SOFTBUS_LOCK_ERR, CONN_COMMON, "lock fail, "
        "module=%{public}d, fd=%{public}d, trigger=%{public}d, error=%{public}d", module, fd, trigger, ret);
    SoftbusListenerNode *node = GetListenerNode(module);
    if (node == NULL) {
        CONN_LOGE(CONN_COMMON, "listener node not exist, module=%{public}d, fd=%{public}d, trigger=%{public}d",
            module, fd, trigger);
        (void)SoftBusMutexUnlock(&g_removeAbnormalFdLock);
        return SOFTBUS_NOT_FIND;
    }
    ret = SoftBusMutexLock(&node->lock);
    if (ret != SOFTBUS_OK) {
        CONN_LOGE(CONN_COMMON, "lock node fail, module=%{public}d, fd=%{public}d, trigger=%{public}d, "
                               "error=%{public}d", module, fd, trigger, ret);
        ReturnListenerNode(&node);
        (void)SoftBusMutexUnlock(&g_removeAbnormalFdLock);
        return SOFTBUS_LOCK_ERR;
    }
    ...
    do {
        ...
        if (target != NULL) {
            ...
            ret = ModifyEvent(g_eventWatcher, fd, target->triggerSet | trigger);
            if (ret == SOFTBUS_OK) {
        ...
        ret = AddEvent(g_eventWatcher, fd, trigger);
        if (ret == SOFTBUS_OK) {
            ListInit(&fdNode->node);
            fdNode->fd = fd;
...
static void ProcessSpecifiedListenerNodeEvent(SoftbusListenerNode *node, ListNode *fdNode, int32_t wakeupTrace)
{
    ...
    int32_t ret = CopyWaitEventFdsUnsafe(node, &fdArray, &fdArrayLen);
    SoftBusMutexUnlock(&node->lock);
    if (ret != SOFTBUS_OK) {
        CONN_LOGE(CONN_COMMON,
            "copy wait event fds fail, wakeupTrace=%{public}d, module=%{public}d, error=%{public}d",
            wakeupTrace, node->module, ret);
        return;
    }
...
static bool NeedRemoveFdFromEpoll(int32_t fd)
{
    for (ListenerModule module = 0; module < UNUSE_BUTT; module++) {
        SoftbusListenerNode *node = GetListenerNode(module);
        if (node == NULL) {
        ...
        if (fd == node->info.listenFd) {
            CONN_LOGW(CONN_COMMON, "fd has been reassigned, fd=%{public}d, module=%{public}d", fd, module);
            SoftBusMutexUnlock(&node->lock);
            ReturnListenerNode(&node);
            return false;
        }
...
static void RemoveUnprocessedFdFromEpoll(int32_t fd)
{
    CONN_CHECK_AND_RETURN_LOGW(SoftBusMutexLock(&g_removeAbnormalFdLock) == SOFTBUS_OK, CONN_COMMON, "lock fail");
    if (NeedRemoveFdFromEpoll(fd)) {
        CONN_LOGE(CONN_COMMON, "need remove fd, fd=%{public}d", fd);
        if (SoftBusSocketGetError(fd) != SOFTBUS_CONN_BAD_FD) {
            (void)RemoveEvent(g_eventWatcher, fd);
        }
    }
    SoftBusMutexUnlock(&g_removeAbnormalFdLock);
...
static void CheckAndRemoveFdFromEpoll(ListNode *fdNode)
{
    if (IsListEmpty(fdNode)) {
        return;
    }
    struct FdNode *it = NULL;
    struct FdNode *next = NULL;
    LIST_FOR_EACH_ENTRY_SAFE(it, next, fdNode, struct FdNode, node) {
        RemoveUnprocessedFdFromEpoll(it->fd);
    }
}
...
static void ProcessEvent(ListNode *fdNode, const WatchThreadState *watchState, int32_t wakeupTrace)
{
    ...
    CheckAndRemoveFdFromEpoll(fdNode);
}
...
static int32_t StartWatchThread(void)
{
    ...
    int32_t ret = SoftBusMutexLock(&g_watchThreadStateLock);
    CONN_CHECK_AND_RETURN_RET_LOGE(
        ret == SOFTBUS_OK, SOFTBUS_LOCK_ERR, CONN_COMMON, "lock global watch thread state fail");

    do {
        if (g_watchThreadState != NULL) {
            ret = SoftBusMutexLock(&g_watchThreadState->lock);
            if (ret != SOFTBUS_OK) {
                CONN_LOGE(CONN_COMMON, "lock watch thread state self fail, error=%{public}d", ret);
                ret = SOFTBUS_LOCK_ERR;
                break;
            }
        ...
        ret = SoftBusMutexInit(&state->lock, NULL);
        if (ret != SOFTBUS_OK) {
        ...
        ret = ConnStartActionAsync(state, WatchTask, NULL);
        if (ret != SOFTBUS_OK) {
            CONN_LOGE(CONN_COMMON, "start watch task async fail, error=%{public}d", ret);
            CleanupWatchThreadState(&state);
            break;
        }
