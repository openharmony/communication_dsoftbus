...
static int SetReusePort(int fd, int on)
{
    ...
    if (rc != 0) {
        CONN_LOGE(CONN_COMMON, "set SO_REUSEPORT fail");
        return -1;
    }
    return 0;
}
...
static int SetReuseAddr(int fd, int on)
{
    ...
    if (rc != 0) {
        CONN_LOGE(CONN_COMMON, "set SO_REUSEADDR fail");
        return -1;
    }
    return 0;
}
...
static int SetNoDelay(int fd, int on)
{
    ...
    if (rc != 0) {
        CONN_LOGE(CONN_COMMON, "set TCP_NODELAY fail");
        return -1;
    }
    return 0;
}
...
static int SetQuickStart(int fd, int quick)
{
    ...
    if (rc != 0) {
        CONN_LOGE(CONN_COMMON, "set TCP_QUICK_START fail. rc=%{public}d, errno=%{public}d(%{public}s)",
...
static int SetSendBufFix(int fd, int val)
{
    ...
    if (rc != 0) {
        CONN_LOGE(CONN_COMMON, "set SOFTBUS_SO_SNDBUF fail. rc=%{public}d, errno=%{public}d(%{public}s)",
            rc, errno, strerror(errno));
        return -1;
    }
    return 0;
...
static int SetRcvBufFix(int fd, int val)
{
    ...
    if (rc != 0) {
        CONN_LOGE(CONN_COMMON, "set SOFTBUS_SO_RCVBUF fail. rc=%{public}d, errno=%{public}d(%{public}s)",
            rc, errno, strerror(errno));
        return -1;
    }
    return 0;
...
int BindLocalIP(int32_t domain, int fd, const char *localIP, uint16_t port)
{
    ...
    if (domain == SOFTBUS_AF_INET6) {
        ...
        if (rc != SOFTBUS_OK) {
            CONN_LOGE(CONN_COMMON, "pack ipv6 addr fail");
    ...
    if (rc != SOFTBUS_OK) {
        CONN_LOGE(CONN_COMMON, "pack ipv4 addr fail");
        return SOFTBUS_SOCKET_ADDR_ERR;
    }
    return SOFTBUS_TEMP_FAILURE_RETRY(
...
int32_t SetIpTos(int fd, uint32_t tos)
{
    ...
    if (rc != 0) {
        CONN_LOGE(CONN_COMMON, "set tos fail, fd=%{public}d", fd);
...
int32_t SocketConnect(int32_t fd, int32_t domain, const ConnectOption *option)
{
    ...
    if (domain == SOFTBUS_AF_INET6) {
        ...
        if (rc != SOFTBUS_OK) {
            CONN_LOGW(CONN_COMMON, "pack ipv6 addr fail");
            return rc;
        }
        return SOFTBUS_TEMP_FAILURE_RETRY(
...
static int32_t OpenTcpClientSocket(const ConnectOption *option, const char *myIp, bool isNonBlock)
{
    ...
    CONN_CHECK_AND_RETURN_RET_LOGE(ret == SOFTBUS_OK, ret, CONN_COMMON,
        "create socket fail, serverIp=%{public}s, serverPort=%{public}d, error=%{public}d", animizedIp,
...
int32_t ConnSetTcpUserTimeOut(int32_t fd, uint32_t millSec)
{
    ...
    if (SoftBusSocketSetOpt(fd, SOFTBUS_IPPROTO_TCP, SOFTBUS_TCP_USER_TIMEOUT, &millSec, sizeof(millSec)) !=
        SOFTBUS_ADAPTER_OK) {
        CONN_LOGE(CONN_COMMON, "set SOFTBUS_TCP_USER_TIMEOUT fail");
        return SOFTBUS_ADAPTER_ERR;
    }
    return SOFTBUS_OK;
}
...
static int32_t AcceptTcpClient(int32_t fd, ConnectOption *clientAddr, int32_t *cfd)
{
    ...
    if (addr.saFamily == SOFTBUS_AF_INET6) {
        ...
        if (ret < 0) {
            CONN_LOGE(CONN_COMMON, "get ipv6 addr fail");
            return ret;
        }
        return SOFTBUS_OK;
    }
