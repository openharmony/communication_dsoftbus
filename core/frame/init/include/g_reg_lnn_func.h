...
typedef bool (*IsPotentialTrustedDeviceFunc)(TrustedRelationIdType idType, const char *deviceId, bool isPrecise, bool isPointToPoint);
...
typedef ConnBleConnection *(*ConnBleGetConnectionByAddrFunc)(const char *addr, ConnSideType side, BleProtocolType protocol);
...
typedef int32_t (*LnnGetRemoteStrInfoByIfnameIdxFunc)(const char *networkId, InfoKey key, char *info, uint32_t len, int32_t ifIdx);
