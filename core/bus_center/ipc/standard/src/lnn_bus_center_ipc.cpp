...
NO_SANITIZE("cfi") int32_t MetaNodeIpcServerJoin(const char *pkgName, int32_t callingPid, void *addr, CustomData *customData,
    ...
    int32_t retCode)
{
    ...
    for (iter = g_joinLNNRequestInfo.begin(); iter != g_joinLNNRequestInfo.end();) {
        ...
        ClientOnJoinLNNResult((*iter)->pkgName, (*iter)->pid, addr, addrTypeLen, networkId, retCode);
        delete *iter;
    ...
    int32_t retCode)
{
    ...
    for (iter = g_joinMetaNodeRequestInfo.begin(); iter != g_joinMetaNodeRequestInfo.end();) {
        ...
        ClientOnJoinMetaNodeResult((*iter)->pkgName, (*iter)->pid, addr, addrTypeLen, networkId, retCode);
        delete *iter;
