...
static uint16_t ConvertStringToInt(const char *deviceType, uint16_t *typeId)
{
    *typeId = 0;
    uint16_t tmp;
    for (uint16_t i = 0; i < strlen(deviceType); i++) {
...
static uint16_t InterceptTypeId(uint16_t typeId, int i)
{
...
{
    int j = 0;
    for (int i = DEVICE_TYPE_MAX_LENGTH - 1; i >= 0; i--) {
...
int32_t LnnConvertDeviceTypeToId(const char *deviceType, uint16_t *typeId)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (mstRet != EOK) {
            *typeId = TYPE_UNKNOW_ID;
                    LOG_ERR("LnnConvertDeviceTypeToId memset_s fail.");
            return SOFTBUS_ERR;
        }
        *typeId = ConvertStringToInt(deviceType, typeId);
        ...
        LOG_ERR("convert string to int fail.");
    }
    *typeId = TYPE_UNKNOW_ID;
    return SOFTBUS_ERR;
}
