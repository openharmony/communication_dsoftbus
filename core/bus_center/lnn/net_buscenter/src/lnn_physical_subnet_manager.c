...
void DoNotifyAddressChange(const char *ifName, ProtocolType protocolType, void *status)
{
...
void LnnNotifyPhysicalSubnetAddressChanged(const char *ifName, ProtocolType protocolType, void *status)
{
    CALL_VOID_FUNC_WITH_LOCK(&g_physicalSubnetsLock, DoNotifyAddressChange(ifName, protocolType, status));
}
