...
static int32_t TryUpdateStartTimeSyncReq(TimeSyncReqInfo *info, const StartTimeSyncReqMsgPara *startReq)
{
    ...
    if (SoftBusMutexLock(&g_startReqListLock) != SOFTBUS_OK) {
        SoftBusLog(SOFTBUS_LOG_LNN, SOFTBUS_LOG_ERROR, "lock list err");
...
NO_SANITIZE("cfi") int32_t LnnStopTimeSync(const char *pkgName, const char *targetNetworkId, int32_t callingPid)
{
