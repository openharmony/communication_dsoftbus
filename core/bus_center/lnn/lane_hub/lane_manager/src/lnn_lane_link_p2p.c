...
static void DfxRecordLnnLaneSelectEnd(int32_t laneId, int32_t connReqId)
{
    LnnEventExtra extra = { 0 };
    LnnEventExtraInit(&extra);
    extra.result = EVENT_STAGE_RESULT_OK;
    extra.laneId = laneId;
    extra.connReqId = connReqId;
...
static void HandleNotSupportP2pError(AsyncResultType type, uint32_t p2pRequestId)
{
    P2pLinkReqList reqInfo;
    (void)memset_s(&reqInfo, sizeof(P2pLinkReqList), 0, sizeof(P2pLinkReqList));
    if (GetP2pLinkReqByReqId(type, p2pRequestId, &reqInfo) != SOFTBUS_OK) {
        LNN_LOGE(LNN_LANE, "get p2pLinkReq fail, type=%{public}d, requestId=%{public}u", type, p2pRequestId);
        return;
    }
...
static int32_t OpenBleTriggerToConn(const LinkRequest *request, uint32_t laneReqId, const LaneLinkCb *callback)
{
    ...
    DfxRecordLnnLaneSelectEnd((int32_t)laneReqId, (int32_t)wifiDirectInfo.requestId);
    LNN_LOGI(LNN_LANE, "wifidirect connectDevice. p2pRequestId=%{public}u, connectType=%{public}d",
        wifiDirectInfo.requestId, wifiDirectInfo.connectType);
...
static int32_t OpenActionToConn(const LinkRequest *request, uint32_t laneLinkReqId, const LaneLinkCb *callback)
{
    ...
    DfxRecordLnnLaneSelectEnd((int32_t)laneLinkReqId, (int32_t)wifiDirectInfo.requestId);
