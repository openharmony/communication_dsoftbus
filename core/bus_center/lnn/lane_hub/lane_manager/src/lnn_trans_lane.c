...
static bool IsLaneResourceNeedDelay(const char *peerUuid, LaneLinkType linkType)
{
    int32_t laneNum = 0;
    int32_t ret = CheckLaneResourceNumByLinkType(peerUuid, linkType, &laneNum);
    if (ret != SOFTBUS_OK) {
        LNN_LOGE(LNN_LANE, "check lane resource num by linkType fail, ret=%{public}d", ret);
        return false;
    }
    LNN_LOGI(LNN_LANE, "get hml lane resource num=%{public}d", laneNum);
    if (laneNum < WIFI_DIRECET_NUM_LIMIT) {
        return true;
    }
    return false;
}
...
static void IsNeedDelayFreeLane(uint32_t laneReqId, uint64_t laneId, bool *isDelayFree)
{
    ...
    if (resourceItem.link.type == LANE_HML && resourceItem.clientRef == 1 &&
        IsLaneResourceNeedDelay(resourceItem.link.peerUdid, resourceItem.link.type) && !isMeta) {
