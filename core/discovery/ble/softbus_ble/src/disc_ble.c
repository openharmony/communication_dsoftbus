...
{
    int32_t type = 0;
    DISC_CHECK_AND_RETURN_RET_LOGE(SoftBusMutexLock(&g_bleInfoLock) == SOFTBUS_OK, SOFTBUS_LOCK_ERR, DISC_BLE, "lock failed");
...
static void ProcessDisConPacket(const BroadcastReportInfo *reportInfo, DeviceInfo *foundInfo)
{
    ...
    ret = SoftBusGenerateStrHash(reportInfo->packet.bcData.payload, reportInfo->packet.bcData.payloadLen,
        (uint8_t *)key);
    if (ret != SOFTBUS_OK) {
        DISC_LOGE(DISC_BLE, "GenerateStrHash failed, ret=%{public}d", ret);
        return;
    }
...
static bool ProcessHashAccount(DeviceInfo *foundInfo)
{
    ...
    for (uint32_t pos = 0; pos < CAPABILITY_MAX_BITNUM; pos++) {
        ...
        if (ret != SOFTBUS_OK) {
            DISC_LOGE(DISC_BLE, "DiscBleGetShortUserIdHash error, ret=%{public}d", ret);
            return false;
        }
...
static void ProcessDisNonPacket(const BroadcastReportInfo *reportInfo, char rssi, DeviceInfo *foundInfo)
{
    ...
    if (ret != SOFTBUS_OK) {
        DISC_LOGE(DISC_BLE, "convert ble address failed, ret=%{public}d", ret);
        return;
    }
...
static void UpdateScannerFilter(bool isStopScan)
{
    if (isStopScan) {
        ...
        if (ret != SOFTBUS_OK) {
            DISC_LOGE(DISC_BLE, "StopScaner failed, ret=%{public}d", ret);
            return;
        }
    }
...
{
    if (type <= 0 || type > (CON_FILTER_TYPE | NON_FILTER_TYPE)) {
        DISC_LOGE(DISC_BLE, "start scanner type invaild");
        return;
    }
    if (!CheckScanner()) {
