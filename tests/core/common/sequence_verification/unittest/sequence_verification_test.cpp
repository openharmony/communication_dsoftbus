...
HWTEST_F(SequenceVerificationTest, Softbus_SeqVerifyTest_Test_DisorderCase_004, TestSize.Level0)
{
    ...
    seqInfo.minSeq = -100;
    seqInfo.maxSeq = -100;
    int32_t recvSeq[MAX_RECEIVE_SEQUENCE] = {-100, -99, -39, -96, -90};
    for (int i = 0; i < MAX_RECEIVE_SEQUENCE; i++) {
        bool ret = IsPassSeqCheck(&seqInfo, recvSeq[i]);
        EXPECT_EQ(ret, true);
    }
...
HWTEST_F(SequenceVerificationTest, Softbus_SeqVerifyTest_Test_DisorderCase_006, TestSize.Level0)
{
    ...
    seqInfo.minSeq = INT32_MAX - 2;
    seqInfo.maxSeq = INT32_MAX - 2;
    int32_t recvSeq[MAX_RECEIVE_SEQUENCE] = {INT32_MAX - 2, INT32_MAX - 1, INT32_MIN + 58, INT32_MIN, INT32_MIN + 7};
    for (int i = 0; i < MAX_RECEIVE_SEQUENCE; i++) {
        bool ret = IsPassSeqCheck(&seqInfo, recvSeq[i]);
        EXPECT_EQ(ret, true);
    }
...
HWTEST_F(SequenceVerificationTest, Softbus_SeqVerifyTest_Test_DisorderCase_007, TestSize.Level0)
{
    ...
    seqInfo.minSeq = INT32_MAX - 2;
    seqInfo.maxSeq = INT32_MAX - 2;
    int32_t recvSeq[MAX_RECEIVE_SEQUENCE] = {INT32_MAX - 2, INT32_MAX - 1, INT32_MIN + 59, INT32_MIN, INT32_MIN + 7};
    for (int i = 0; i < MAX_RECEIVE_SEQUENCE; i++) {
        bool ret = IsPassSeqCheck(&seqInfo, recvSeq[i]);
        if (i < 2) {
            EXPECT_EQ(ret, true);
...
HWTEST_F(SequenceVerificationTest, Softbus_SeqVerifyTest_Test_DisorderCase_008, TestSize.Level0)
{
    ...
    seqInfo.minSeq = -30;
    seqInfo.maxSeq = -30;
    int32_t recvSeq[MAX_RECEIVE_SEQUENCE] = {-30, -29, 31, 0, -3};
    for (int i = 0; i < MAX_RECEIVE_SEQUENCE; i++) {
        bool ret = IsPassSeqCheck(&seqInfo, recvSeq[i]);
        EXPECT_EQ(ret, true);
    }
...
HWTEST_F(SequenceVerificationTest, Softbus_SeqVerifyTest_Test_RepeatCase_003, TestSize.Level0)
{
    ...
    seqInfo.minSeq = INT32_MAX - 2;
    seqInfo.maxSeq = INT32_MAX - 2;
    int32_t recvSeq[MAX_RECEIVE_SEQUENCE] = {INT32_MAX - 2, INT32_MAX - 1, INT32_MIN + 5, INT32_MAX - 1, INT32_MIN + 3};
    for (int i = 0; i < MAX_RECEIVE_SEQUENCE; i++) {
        bool ret = IsPassSeqCheck(&seqInfo, recvSeq[i]);
        if (i < 3 || i == 4) {
            EXPECT_EQ(ret, true);
...
HWTEST_F(SequenceVerificationTest, Softbus_SeqVerifyTest_Test_RepeatCase_004, TestSize.Level0)
{
    ...
    seqInfo.minSeq = INT32_MAX - 2;
    seqInfo.maxSeq = INT32_MAX - 2;
    int32_t recvSeq[MAX_RECEIVE_SEQUENCE] = {INT32_MAX - 2, INT32_MIN, INT32_MIN + 5, INT32_MIN, INT32_MIN + 3};
    for (int i = 0; i < MAX_RECEIVE_SEQUENCE; i++) {
        bool ret = IsPassSeqCheck(&seqInfo, recvSeq[i]);
        if (i < 3 || i == 4) {
            EXPECT_EQ(ret, true);
...
HWTEST_F(SequenceVerificationTest, Softbus_SeqVerifyTest_Test_LessThanMinCase_004, TestSize.Level0)
{
    ...
    int32_t recvSeq[MAX_RECEIVE_SEQUENCE] = {INT32_MAX - 10, INT32_MAX - 5, INT32_MIN, INT32_MIN + 1, INT32_MIN + 3};
    for (int i = 0; i < MAX_RECEIVE_SEQUENCE; i++) {
        bool ret = IsPassSeqCheck(&seqInfo, recvSeq[i]);
        if (i >= 2) {
            EXPECT_EQ(ret, true);
