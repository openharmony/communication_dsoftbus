...
HWTEST_F(TransClientProxyTest, InformPermissionChangeTest001, TestSize.Level0)
{
    ...
    ret = InformPermissionChange(TEST_STATE, nullptr);
    EXPECT_EQ(SOFTBUS_ERR, ret);

    ret = InformPermissionChange(TEST_STATE, g_pkgName);
    EXPECT_EQ(SOFTBUS_ERR, ret);
    ...
    ret = SoftbusClientInfoManager::GetInstance().SoftbusAddService(g_pkgName, remoteObject, abilityDeath);
...
HWTEST_F(TransClientProxyTest, ClientIpcOnChannelOpenFailedTest001, TestSize.Level0)
{
    ...
    ret = ClientIpcOnChannelOpenFailed(g_pkgName, TEST_CHANNELID, TEST_CHANNELTYPE, TEST_ERR_CODE);
    EXPECT_EQ(SOFTBUS_OK, ret);
...
HWTEST_F(TransClientProxyTest, ClientIpcOnChannelLinkDownTest001, TestSize.Level0)
{
    ...
    ret = ClientIpcOnChannelLinkDown(g_pkgName, networkId, TEST_REMOTE_TYPE);
    EXPECT_EQ(SOFTBUS_OK, ret);
...
HWTEST_F(TransClientProxyTest, ClientIpcOnChannelClosedTest001, TestSize.Level0)
{
    ...
    ret = ClientIpcOnChannelClosed(g_pkgName, TEST_CHANNELID, TEST_CHANNELTYPE);
    EXPECT_EQ(SOFTBUS_OK, ret);
...
HWTEST_F(TransClientProxyTest, ClientIpcOnChannelMsgReceivedTest001, TestSize.Level0)
{
    ...
    ret = ClientIpcOnChannelMsgReceived(g_pkgName, TEST_CHANNELID, TEST_CHANNELTYPE, data, TEST_LEN, TEST_DATA_TYPE);
    EXPECT_EQ(SOFTBUS_OK, ret);
