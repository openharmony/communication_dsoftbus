...
char* TestGetMsgInfo(void)
{
    ...
    UkIdInfo ukIdInfo;
    if (TransPackRequestUdpInfo(requestMsg, &info, &ukIdInfo) != SOFTBUS_OK) {
        cJSON_Delete(requestMsg);
        return nullptr;
    }
...
HWTEST_F(TransUdpNegoTest, TransUnpackReplyErrInfo001, TestSize.Level1)
{
    ...
    int32_t ret = TransUnpackReplyErrInfo(nullptr, nullptr);
...
HWTEST_F(TransUdpNegoTest, TransPackReplyErrInfo001, TestSize.Level1)
{
    ...
    int32_t ret = TransPackReplyErrInfo(nullptr, errCode, nullptr);
    EXPECT_EQ(ret, SOFTBUS_INVALID_PARAM);
...
HWTEST_F(TransUdpNegoTest, SendUdpInfo001, TestSize.Level1)
{
    ...
    int32_t ret = SendUdpInfo(nullptr, authHandle, seq, false);
    EXPECT_EQ(ret, SOFTBUS_PARSE_JSON_ERR);
    authHandle.authId = 0;
...
HWTEST_F(TransUdpNegoTest, SendReplyErrInfo001, TestSize.Level1)
{
    ...
    int32_t ret = SendReplyErrInfo(errCode, nullptr, authHandle, 0, false);
    EXPECT_EQ(ret, SOFTBUS_INVALID_PARAM);

    ret = SendReplyErrInfo(errCode, (char *)errDesc.c_str(), authHandle, 0, false);
...
HWTEST_F(TransUdpNegoTest, StartExchangeUdpInfo001, TestSize.Level1)
{
    ...
    int32_t ret = StartExchangeUdpInfo(&channel, authHandle, 0, false, nullptr);
    EXPECT_EQ(ret, SOFTBUS_NOT_IMPLEMENT);
...
HWTEST_F(TransUdpNegoTest, StartExchangeUdpInfo002, TestSize.Level1)
{
    ...
    StartExchangeUdpInfo(channel, authHandle, channel->seq, false, nullptr);
    EXPECT_EQ(ret, SOFTBUS_OK);
}
...
HWTEST_F(TransUdpNegoTest, UdpModuleCb001, TestSize.Level1)
{
    ...
    EXPECT_EQ(ret, SOFTBUS_OK);
    UdpModuleCb(authHandle, nullptr);
...
HWTEST_F(TransUdpNegoTest, UdpOpenAuthConn001, TestSize.Level1)
{
    ...
    bool isMeta = false;
    int32_t ret = UdpOpenAuthConn(peerUdid.c_str(), requestId, isMeta, 0, true);
    EXPECT_NE(ret, SOFTBUS_OK);

    isMeta = true;
    ret = UdpOpenAuthConn(peerUdid.c_str(), requestId, isMeta, 0, false);
    EXPECT_EQ(ret, SOFTBUS_NOT_IMPLEMENT);
}
...
HWTEST_F(TransUdpNegoTest, AcceptUdpChannelAsServer001, TestSize.Level1)
{
    ...
    int32_t ret = AcceptUdpChannelAsServer(appInfo, authHandle, seq, nullptr);
...
HWTEST_F(TransUdpNegoTest, ProcessUdpChannelState001, TestSize.Level1)
{
    ...
    isServerSide = false;
    ret = ProcessUdpChannelState(appInfo, isServerSide, &authHandle, seq, nullptr);
    EXPECT_EQ(ret, SOFTBUS_LOCK_ERR);
...
HWTEST_F(TransUdpNegoTest, ParseRequestAppInfo001, TestSize.Level1)
{
    ...
    UkIdInfo ukIdInfo;
    ret = TransPackRequestUdpInfo(msg, appInfo, &ukIdInfo);
    EXPECT_EQ(ret, SOFTBUS_OK);
    memset_s(appInfo, sizeof(AppInfo), 0, sizeof(AppInfo));
    ret = ParseRequestAppInfo(authHandle, msg, appInfo, &ukIdInfo);
...
HWTEST_F(TransUdpNegoTest, TransPackRequestUdpInfo001, TestSize.Level1)
{
    ...
    cJSON *msg = cJSON_Parse(msgStr.c_str());
    UkIdInfo ukIdInfo;
    int32_t ret = TransPackRequestUdpInfo(msg, appInfo, &ukIdInfo);
    EXPECT_EQ(ret, SOFTBUS_INVALID_PARAM);
