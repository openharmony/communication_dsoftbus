...
HWTEST_F(TransUdpNegotiationExchangeTest, TransUdpNegotiationExchangeTest001, TestSize.Level1)
{
    ...
    UkIdInfo ukIdInfo;
    int32_t ret = TransPackRequestUdpInfo(nullptr, appInfo, &ukIdInfo);
    EXPECT_EQ(ret, SOFTBUS_INVALID_PARAM);
    ret = TransUnpackRequestUdpInfo(nullptr, appInfo, &ukIdInfo);
    EXPECT_EQ(ret, SOFTBUS_INVALID_PARAM);
    ret = TransPackRequestUdpInfo(msg, nullptr, &ukIdInfo);
    EXPECT_EQ(ret, SOFTBUS_INVALID_PARAM);
    ret = TransUnpackRequestUdpInfo(msg, nullptr, &ukIdInfo);
    EXPECT_EQ(ret, SOFTBUS_INVALID_PARAM);
    cJSON_Delete(msg);
    SoftBusFree(appInfo);
}
...
HWTEST_F(TransUdpNegotiationExchangeTest, TransUdpNegotiationExchangeTest002, TestSize.Level1)
{
    ...
    memset_s(appInfo, sizeof(AppInfo), 0, sizeof(AppInfo));
    ret = TransUnpackRequestUdpInfo(msg, appInfo, &ukIdInfo);
    EXPECT_EQ(ret, SOFTBUS_OK);
    cJSON_Delete(msg);
