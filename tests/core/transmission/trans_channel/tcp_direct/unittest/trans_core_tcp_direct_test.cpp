...
HWTEST_F(TransCoreTcpDirectTest, TransTdcDeathCallbackTest002, TestSize.Level1)
{
    ...
    TransTdcDeathCallback(g_pkgName);

    TransTdcDeathCallback(nullptr);
    TransTcpDirectDeinit();
}
...
HWTEST_F(TransCoreTcpDirectTest, TransTdcOnChannelOpenedTest0014, TestSize.Level1)
{
    ...
    int32_t ret = TransTdcOnChannelOpened(nullptr, g_sessionName, nullptr);
    EXPECT_TRUE(ret != SOFTBUS_OK);

    info->isServer = true;
    info->channelType = CHANNEL_TYPE_AUTH;
    ret = TransTdcOnChannelOpened(nullptr, g_sessionName, info);
    EXPECT_TRUE(ret != SOFTBUS_OK);

    ret = TransTdcOnChannelOpened(g_pkgName, g_sessionName, info);
    EXPECT_TRUE(ret != SOFTBUS_OK);
...
HWTEST_F(TransCoreTcpDirectTest, TransTdcGetUidAndPidTest0015, TestSize.Level1)
{
    ...
    ret = TransTdcOnChannelOpenFailed(g_pkgName, channelId, errCode);
    EXPECT_TRUE(ret == SOFTBUS_OK);
