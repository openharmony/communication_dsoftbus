...
HWTEST_F(AuthManagerTest, FIND_AUTH_MANAGER_TEST_001, TestSize.Level1)
{
    ...
    EXPECT_EQ(GetAuthConnInfoByUuid(UUID_TEST, (AuthLinkType)type, &connInfo), SOFTBUS_AUTH_NOT_FOUND);
...
HWTEST_F(AuthManagerTest, START_VERIFY_DEVICE_TEST_001, TestSize.Level1)
{
    ...
    EXPECT_TRUE(StartVerifyDevice(&authVerifyParam, &connInfo, &verifyCb) == SOFTBUS_AUTH_INIT_FAIL);
    g_regDataChangeListener = true;
    EXPECT_TRUE(StartVerifyDevice(&authVerifyParam, &connInfo, &verifyCb) == SOFTBUS_AUTH_CONN_FAIL);
    EXPECT_TRUE(StartVerifyDevice(&authVerifyParam, &connInfo, nullptr) == SOFTBUS_INVALID_PARAM);
    AuthSessionInfo info;
    SetAuthSessionInfo(&info, CONN_ID, false, AUTH_LINK_TYPE_WIFI);
    EXPECT_TRUE(NewAuthManager(AUTH_SEQ, &info) != nullptr);
    AuthHandle authHandle = { .authId = AUTH_SEQ_1, .type = AUTH_LINK_TYPE_WIFI };
