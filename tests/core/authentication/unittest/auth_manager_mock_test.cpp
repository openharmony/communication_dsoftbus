...
HWTEST_F(AuthManagerMockTest, DFX_RECORD_SERVER_RECV_PASSIVE_CONN_TIME_TEST_001, TestSize.Level1)
{
    ...
    AuthManagerInterfaceMock authManagerMock;
    EXPECT_CALL(authManagerMock, GetLnnTriggerInfo).WillRepeatedly(Return());
...
HWTEST_F(AuthManagerMockTest, HANDLE_DEVICE_ID_DATA_TEST_001, TestSize.Level1)
{
    ...
    EXPECT_CALL(authManagerMock, ConnGetNewRequestId).WillRepeatedly(Return(REQ_ID));
    EXPECT_CALL(authManagerMock, HandleRepeatDeviceIdDataDelay).WillRepeatedly(Return());
    ...
    EXPECT_CALL(authManagerMock, AuthSessionStartAuth)
        .WillOnce(Return(SOFTBUS_INVALID_PARAM))
        .WillRepeatedly(Return(SOFTBUS_OK));
    EXPECT_CALL(authManagerMock, AuthSessionProcessDevIdData).WillRepeatedly(Return(SOFTBUS_INVALID_PARAM));
    EXPECT_NO_FATAL_FAILURE(HandleDeviceIdData(connId, &connInfo, false, &head, nullptr));
...
HWTEST_F(AuthManagerMockTest, HANDLE_DEVICE_INFO_DATA_TEST_002, TestSize.Level1)
{
    ...
    EXPECT_CALL(authManagerMock, IsDeviceMessagePacket)
        .WillRepeatedly(DoAll(SetArgPointee<4>(messageParse), Return(true)));
    EXPECT_CALL(authManagerMock, RequireAuthLock).WillRepeatedly(Return(false));
    EXPECT_CALL(authManagerMock, AuthSetTcpKeepaliveOption).WillRepeatedly(Return(SOFTBUS_INVALID_PARAM));
    EXPECT_CALL(authManagerMock, DestroySessionKeyList).WillRepeatedly(Return());
    EXPECT_CALL(authManagerMock, CheckAuthConnInfoType).WillRepeatedly(Return(true));
    EXPECT_NO_FATAL_FAILURE(HandleDeviceInfoData(connId, &connInfo, false, &head, nullptr));
...
HWTEST_F(AuthManagerMockTest, AUTH_DEVICE_GET_CONN_INFO_BY_TYPE_TEST_001, TestSize.Level1)
{
    ...
    AuthManagerInterfaceMock authManagerMock;
    EXPECT_CALL(authManagerMock, RequireAuthLock).WillRepeatedly(Return(false));
    EXPECT_CALL(authManagerMock, LnnGetNetworkIdByUuid).WillRepeatedly(Return(SOFTBUS_INVALID_PARAM));
...
HWTEST_F(AuthManagerMockTest, AUTH_MANAGER_SET_AUTH_FAILED_TEST_001, TestSize.Level1)
{
    ...
    EXPECT_CALL(authManagerMock, CheckAuthConnInfoType).WillRepeatedly(Return(true));
    EXPECT_CALL(authManagerMock, GetAuthSideStr).WillRepeatedly(Return("server"));
    EXPECT_CALL(authManagerMock, AuthNotifyDeviceDisconnect).WillRepeatedly(Return());
    EXPECT_CALL(authManagerMock, GetAuthRequest).WillRepeatedly(Return(SOFTBUS_INVALID_PARAM));
    EXPECT_CALL(authManagerMock, GetConnType).WillRepeatedly(Return(AUTH_LINK_TYPE_WIFI));
    EXPECT_CALL(authManagerMock, UpdateAuthDevicePriority).WillRepeatedly(Return());
    EXPECT_CALL(authManagerMock, DisconnectAuthDevice).WillRepeatedly(Return());
    EXPECT_CALL(authManagerMock, AuthAddNodeToLimitMap).WillRepeatedly(Return());
...
HWTEST_F(AuthManagerMockTest, AUTH_PROCESS_EMPTY_SESSION_KEY_TEST_001, TestSize.Level1)
{
    ...
    EXPECT_CALL(authManagerMock, CancelUpdateSessionKey).WillRepeatedly(Return());
    EXPECT_CALL(authManagerMock, DestroySessionKeyList).WillRepeatedly(Return());
    EXPECT_CALL(authManagerMock, GetSessionKeyByIndex).WillRepeatedly(Return(SOFTBUS_INVALID_PARAM));
    EXPECT_CALL(authManagerMock, ClearOldKey).WillRepeatedly(Return(SOFTBUS_OK));
    EXPECT_CALL(authManagerMock, SetSessionKeyAuthLinkType).WillRepeatedly(Return(SOFTBUS_OK));
    AuthSessionInfo info;
    (void)memset_s(&info, sizeof(info), 0, sizeof(info));
    EXPECT_EQ(strcpy_s(info.udid, UDID_BUF_LEN, UDID_TEST), EOK);
    EXPECT_EQ(strcpy_s(info.uuid, UUID_BUF_LEN, UUID_TEST), EOK);
...
HWTEST_F(AuthManagerMockTest, REPORT_AUTH_REQUEST_PASSED_TEST_001, TestSize.Level1)
{
    ...
    EXPECT_CALL(authManagerMock, CheckAuthConnCallback)
        ...
        .WillRepeatedly(Return(true));
    EXPECT_CALL(authManagerMock, AuthNotifyDeviceVerifyPassed).WillRepeatedly(Return());
    EXPECT_CALL(authManagerMock, PerformAuthConnCallback).WillRepeatedly(Return());
    EXPECT_CALL(authManagerMock, DelAuthRequest).WillRepeatedly(Return());
    EXPECT_CALL(authManagerMock, PerformVerifyCallback).WillRepeatedly(Return());
    EXPECT_CALL(authManagerMock, FindAuthRequestByConnInfo).WillRepeatedly(Return(SOFTBUS_INVALID_PARAM));
    AuthRequest request;
    (void)memset_s(&request, sizeof(AuthRequest), 0, sizeof(AuthRequest));
    request.connInfo.type = AUTH_LINK_TYPE_WIFI;
    EXPECT_CALL(authManagerMock, GetAuthRequest).WillOnce(DoAll(SetArgPointee<1>(request), Return(SOFTBUS_OK)));
...
{
    AuthManagerInterfaceMock authManagerMock;
    EXPECT_CALL(authManagerMock, GetAuthRequest).WillRepeatedly(Return(SOFTBUS_OK));
    EXPECT_CALL(authManagerMock, CheckAuthConnCallback)
        .WillOnce(Return(false))
        .WillRepeatedly(Return(true));
    EXPECT_CALL(authManagerMock, DelAuthRequest).WillRepeatedly(Return());
    EXPECT_CALL(authManagerMock, PerformVerifyCallback).WillRepeatedly(Return());
    EXPECT_CALL(authManagerMock, PerformAuthConnCallback).WillRepeatedly(Return());
    EXPECT_CALL(authManagerMock, FindAuthRequestByConnInfo).WillRepeatedly(Return(SOFTBUS_OK));
    EXPECT_CALL(authManagerMock, ConnectAuthDevice).WillRepeatedly(Return(SOFTBUS_OK));
    uint32_t requestId = REQ_ID;
    int32_t reason = SOFTBUS_AUTH_TIMEOUT;
...
HWTEST_F(AuthManagerMockTest, AUTH_NOTIFY_AUTH_PASSED_TEST_001, TestSize.Level1)
{
    ...
    EXPECT_CALL(authManagerMock, CancelUpdateSessionKey).WillRepeatedly(Return());
    EXPECT_CALL(authManagerMock, DestroySessionKeyList).WillRepeatedly(Return());
    EXPECT_CALL(authManagerMock, DelAuthNormalizeRequest).WillRepeatedly(Return());
    uint64_t time = TIME_TEST;
    EXPECT_CALL(authManagerMock, GetCurrentTimeMs).WillRepeatedly(Return(time));
    AuthSessionInfo info;
    (void)memset_s(&info, sizeof(info), 0, sizeof(info));
...
HWTEST_F(AuthManagerMockTest, AUTH_NOTIFY_AUTH_PASSED_TEST_002, TestSize.Level1)
{
    ...
    EXPECT_CALL(authManagerMock, ReleaseAuthLock).WillRepeatedly(Return());
    EXPECT_CALL(authManagerMock, GetAuthSideStr).WillRepeatedly(Return("server"));
    EXPECT_CALL(authManagerMock, PrintAuthConnInfo).WillRepeatedly(Return());
    int64_t authSeq = AUTH_SEQ;
    AuthSessionInfo info;
    (void)memset_s(&info, sizeof(info), 0, sizeof(info));
    EXPECT_NO_FATAL_FAILURE(AuthNotifyAuthPassed(authSeq, &info));
...
HWTEST_F(AuthManagerMockTest, AUTH_MANAGER_SET_AUTH_FINISHED_TEST_001, TestSize.Level1)
{
    ...
    AuthManagerInterfaceMock authManagerMock;
    EXPECT_CALL(authManagerMock, LnnGetLocalNumU64Info).WillRepeatedly(Return(SOFTBUS_OK));
    EXPECT_CALL(authManagerMock, IsFeatureSupport).WillRepeatedly(Return(true));
    EXPECT_CALL(authManagerMock, DisconnectAuthDevice).WillRepeatedly(Return());
    EXPECT_CALL(authManagerMock, SoftBusGenerateStrHash).WillRepeatedly(Return(SOFTBUS_OK));
    EXPECT_CALL(authManagerMock, AuthDeleteLimitMap).WillRepeatedly(Return());
    EXPECT_CALL(authManagerMock, PostAuthEvent).WillRepeatedly(Return(SOFTBUS_OK));
    int64_t authSeq = AUTH_SEQ;
...
HWTEST_F(AuthManagerMockTest, AUTH_MANAGER_SET_AUTH_FINISHED_TEST_002, TestSize.Level1)
{
    ...
    AuthManagerInterfaceMock authManagerMock;
    EXPECT_CALL(authManagerMock, LnnGetLocalNumU64Info).WillRepeatedly(Return(SOFTBUS_OK));
    EXPECT_CALL(authManagerMock, IsFeatureSupport).WillRepeatedly(Return(false));
    EXPECT_CALL(authManagerMock, DisconnectAuthDevice).WillRepeatedly(Return());
    EXPECT_CALL(authManagerMock, SoftBusGenerateStrHash).WillRepeatedly(Return(SOFTBUS_OK));
    EXPECT_CALL(authManagerMock, AuthDeleteLimitMap).WillRepeatedly(Return());
    int64_t authSeq = AUTH_SEQ;
...
HWTEST_F(AuthManagerMockTest, ON_CONNECT_RESULT_TEST_001, TestSize.Level1)
{
    ...
    EXPECT_CALL(authManagerMock, GetAuthRequest).WillRepeatedly(DoAll(SetArgPointee<1>(request), Return(SOFTBUS_OK)));
    EXPECT_CALL(authManagerMock, AuthSessionStartAuth).WillRepeatedly(Return(SOFTBUS_OK));
...
HWTEST_F(AuthManagerMockTest, HANDLE_UK_CONNECTION_DATA_TEST_001, TestSize.Level1)
{
    ...
    (void)memset_s(&head, sizeof(AuthDataHead), 0, sizeof(AuthDataHead));
    AuthManagerInterfaceMock authManagerMock;
    EXPECT_CALL(authManagerMock, GenConnId).WillRepeatedly(Return(connId));
    int32_t fd = FD_TEST;
    EXPECT_CALL(authManagerMock, GetFd).WillRepeatedly(Return(fd));
    uint32_t len = UK_ENCRYPT_INDEX_LEN + 1;
    EXPECT_CALL(authManagerMock, AuthGetUkDecryptSize).WillRepeatedly(Return(len));
    EXPECT_CALL(authManagerMock, AuthDecryptByUkId).WillRepeatedly(Return(SOFTBUS_OK));
    EXPECT_CALL(authManagerMock, RequireAuthLock).WillRepeatedly(Return(true));
    EXPECT_CALL(authManagerMock, PrintAuthConnInfo).WillRepeatedly(Return());
    EXPECT_CALL(authManagerMock, GetConnType).WillRepeatedly(Return(AUTH_LINK_TYPE_WIFI));
    EXPECT_CALL(authManagerMock, ReleaseAuthLock).WillRepeatedly(Return());
    EXPECT_CALL(authManagerMock, GetAuthSideStr).WillRepeatedly(Return("server"));
    EXPECT_CALL(authManagerMock, GetConnTypeStr).WillRepeatedly(Return("wifi/eth"));
    uint32_t connIdTest = REQ_ID;
    EXPECT_CALL(authManagerMock, GetConnId).WillRepeatedly(Return(connIdTest));
...
HWTEST_F(AuthManagerMockTest, HANDLE_UK_CONNECTION_DATA_TEST_003, TestSize.Level1)
{
    ...
    uint32_t len = UK_ENCRYPT_INDEX_LEN + 1;
    EXPECT_CALL(authManagerMock, AuthGetUkDecryptSize).WillRepeatedly(Return(len));
    EXPECT_CALL(authManagerMock, AuthDecryptByUkId).WillRepeatedly(Return(SOFTBUS_INVALID_PARAM));
    EXPECT_CALL(authManagerMock, RequireAuthLock).WillRepeatedly(Return(true));
    EXPECT_CALL(authManagerMock, GetConnType).WillRepeatedly(Return(AUTH_LINK_TYPE_WIFI));
    EXPECT_CALL(authManagerMock, ReleaseAuthLock).WillRepeatedly(Return());
    EXPECT_CALL(authManagerMock, GetAuthSideStr).WillRepeatedly(Return("server"));
...
HWTEST_F(AuthManagerMockTest, POST_DEVICE_MESSAGE_BY_UUID_TEST_001, TestSize.Level1)
{
    ...
    EXPECT_CALL(authManagerMock, CancelUpdateSessionKey).WillRepeatedly(Return());
    EXPECT_CALL(authManagerMock, DestroySessionKeyList).WillRepeatedly(Return());
    EXPECT_CALL(authManagerMock, PostDeviceMessage).WillRepeatedly(Return(SOFTBUS_OK));
    AuthSessionInfo info;
    (void)memset_s(&info, sizeof(info), 0, sizeof(info));
    EXPECT_EQ(strcpy_s(info.udid, UDID_BUF_LEN, UDID_TEST), EOK);
    EXPECT_EQ(strcpy_s(info.uuid, UUID_BUF_LEN, UUID_TEST), EOK);
