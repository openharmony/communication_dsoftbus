...
HWTEST_F(LinkManagerTest, ProcessIfXXXByRemoteDeviceId, TestSize.Level1)
{
    ...
    bool execute = false;
    auto result = LinkManager::GetInstance().ProcessIfPresent(InnerLink::LinkType::HML, remoteDeviceId, [&execute](InnerLink &innerLink){
        execute = true;
    });
    ...
    execute = false;
    result = LinkManager::GetInstance().ProcessIfAbsent(InnerLink::LinkType::HML, remoteDeviceId, [&execute, remoteDeviceId](InnerLink &innerLink){
    ...
    execute = false;
    result = LinkManager::GetInstance().ProcessIfPresent(InnerLink::LinkType::HML, remoteDeviceId, [&execute](InnerLink &innerLink){
        execute = true;
    });
    EXPECT_TRUE(execute);
    EXPECT_TRUE(result);
...
HWTEST_F(LinkManagerTest, ProcessIfXXXByRemoteMac, TestSize.Level1)
{
    ...
    bool execute = false;
    bool result = LinkManager::GetInstance().ProcessIfPresent(remoteMac, [&execute](InnerLink &innerLink){
        execute = true;
    });
    EXPECT_FALSE(execute);
    EXPECT_FALSE(result);
    ...
    execute = false;
    result = LinkManager::GetInstance().ProcessIfAbsent(remoteMac, [&execute, remoteMac](InnerLink &innerLink){
    ...
    execute = false;
    result = LinkManager::GetInstance().ProcessIfPresent(remoteMac, [&execute](InnerLink &innerLink){
        execute = true;
    });
    EXPECT_TRUE(execute);
    EXPECT_TRUE(result);
    ...
    execute = false;
    result = LinkManager::GetInstance().ProcessIfPresent(link.linkId, [&execute](InnerLink &innerLink) {
        execute = true;
    });
    EXPECT_TRUE(execute);
    EXPECT_TRUE(result);
