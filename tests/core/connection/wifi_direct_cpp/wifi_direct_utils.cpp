...
static int32_t LnnGetRecommendChannelPacked(const char *udid, int32_t *preferChannel)
{
    ...
    if (LnnCheckFuncPointer(reinterpret_cast<void *>pfnLnnEnhanceFuncList->lnnGetRecommendChannel) != SOFTBUS_OK) {
...
{
    LnnEnhanceFuncList *pfnLnnEnhanceFuncList = LnnEnhanceFuncListGet();
    if (LnnCheckFuncPointer(reinterpret_cast<void *>pfnLnnEnhanceFuncList->lnnGetLocalPtkByUuid) != SOFTBUS_OK) {
        return SOFTBUS_OK;
    }
...
{
    LnnEnhanceFuncList *pfnLnnEnhanceFuncList = LnnEnhanceFuncListGet();
    if (LnnCheckFuncPointer(reinterpret_cast<void *>pfnLnnEnhanceFuncList->lnnGetLocalDefaultPtkByUuid) != SOFTBUS_OK) {
        return SOFTBUS_OK;
    }
...
std::vector<uint8_t> WifiDirectUtils::GetLocalPtk(const std::string &remoteNetworkId)
{
    ...
    if (ret == SOFTBUS_NOT_FIND) {
        ret = LnnGetLocalDefaultPtkByUuidPacked(remoteUuid.c_str(), reinterpret_cast<char *>ptkBytes, sizeof(ptkBytes));
    }
...
{
    LnnEnhanceFuncList *pfnLnnEnhanceFuncList = LnnEnhanceFuncListGet();
    if (LnnCheckFuncPointer(reinterpret_cast<void *>pfnLnnEnhanceFuncList->lnnGetRemoteDefaultPtkByUuid) != SOFTBUS_OK) {
        return SOFTBUS_OK;
    }
...
std::vector<uint8_t> WifiDirectUtils::GetRemotePtk(const std::string &remoteNetworkId)
{
    ...
    if (ret == SOFTBUS_OK && memcmp(ptkBytes, zeroPtkBytes, PTK_DEFAULT_LEN) == 0) {
        ret = LnnGetRemoteDefaultPtkByUuidPacked(remoteUuid.c_str(), reinterpret_cast<char *>ptkBytes, sizeof(ptkBytes));
    }
...
std::vector<uint8_t> WifiDirectUtils::GetInterfaceMacAddr(const std::string &interface)
{
    ...
    int32_t fd = 0;
    CONN_CHECK_AND_RETURN_RET_LOGW(fd > 0, macArray, CONN_WIFI_DIRECT, "open socket fail");
    ret = ioctl(fd, SIOCGIFHWADDR, &ifr);
    close(fd);

    CONN_CHECK_AND_RETURN_RET_LOGW(ret == 0, macArray, CONN_WIFI_DIRECT, "get hw addr fail ret=%{public}d", ret);
...
std::string WifiDirectUtils::GetInterfaceIpv6Addr(const std::string &name)
{
    ...
    CONN_CHECK_AND_RETURN_RET_LOGE(ret == 0, "", CONN_WIFI_DIRECT, "getifaddrs fail, errno=%{public}d", errno);
...
std::vector<Ipv4Info> WifiDirectUtils::GetLocalIpv4Infos()
{
    ...
    if (getifaddrs(&ifAddr) == -1) {
        CONN_LOGE(CONN_WIFI_DIRECT, "getifaddrs fail, errno=%{public}d", errno);
        return ipv4Infos;
    }
...
int32_t WifiDirectUtils::GetInterfaceIpString(const std::string &interface, std::string &ip)
{
    ...
    CONN_CHECK_AND_RETURN_RET_LOGW(
        socketFd >= 0, SOFTBUS_CONN_OPEN_SOCKET_FAILED, CONN_WIFI_DIRECT, "open socket fail");

    struct ifreq request { };
    (void)memset_s(&request, sizeof(request), 0, sizeof(request));
    int32_t ret = strcpy_s(request.ifr_name, sizeof(request.ifr_name), (const char *)interface.c_str());
    if (ret != EOK) {
        CONN_LOGW(CONN_WIFI_DIRECT, "copy interface name fail");
        close(socketFd);
    ...
    close(socketFd);
    CONN_CHECK_AND_RETURN_RET_LOGW(
        ret >= 0, SOFTBUS_CONN_GET_IFR_CONF_FAILED, CONN_WIFI_DIRECT, "get ifr conf fail ret=%{public}d", ret);
...
int32_t WifiDirectUtils::IpStringToIntArray(const char *addrString, uint32_t *addrArray, size_t addrArraySize)
{
    ...
    CONN_CHECK_AND_RETURN_RET_LOGW(
        ret > 0, SOFTBUS_CONN_SCAN_IP_NUMBER_FAILED, CONN_WIFI_DIRECT, "scan ip number fail");
    return SOFTBUS_OK;
}
...
{
    int32_t metaType = 0;
    auto ret = LnnGetRemoteNumInfo(networkId, NUM_KEY_META_TYPE, &metaType);
