...
{
    return GetBrConnectionInterface()->ConnStartActionAsync(arg, runnable, taskName);
}
...
{
    return GetBrConnectionInterface()->SoftBusMutexLockInner(mutex);
}

int32_t SoftBusMutexUnlockInner(SoftBusMutex *mutex)
{
    return GetBrConnectionInterface()->SoftBusMutexUnlockInner(mutex);
}

int32_t SoftBusMutexInit(SoftBusMutex *mutex, SoftBusMutexAttr *attr)
{
...
{
    return GetBrConnectionInterface()->SoftBusMutexDestroy(mutex);
}

int32_t SoftBusSleepMs(uint32_t milliseconds)
{
    return GetBrConnectionInterface()->SoftBusSleepMs(milliseconds);
}
