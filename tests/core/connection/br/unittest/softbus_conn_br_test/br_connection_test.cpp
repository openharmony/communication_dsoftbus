...
{
    NiceMock<BrConnectionInterfaceMock> brMock;
    EXPECT_CALL(brMock, SoftBusMutexInit).WillRepeatedly(Return(SOFTBUS_OK));
    EXPECT_CALL(brMock, CreateSoftBusList).WillOnce(Return((SoftBusList *)SoftBusCalloc(sizeof(SoftBusList))));
    EXPECT_CALL(brMock, SoftBusMutexDestroy).WillRepeatedly(Return());
...
HWTEST_F(BrConnectionTest, ConnBrFreeConnectionTest001, TestSize.Level1)
{
    ConnBrFreeConnection(NULL);
}
...
HWTEST_F(BrConnectionTest, ConnBrUpdateConnectionRcTest003, TestSize.Level1)
{
    ...
    EXPECT_CALL(brMock, ConnBrPackCtlMessage).WillOnce(Return(1));
    EXPECT_CALL(brMock, ConnBrPostBytes).WillOnce(Return(SOFTBUS_OK));
    EXPECT_CALL(brMock, ConnPostMsgToLooper).WillOnce(Return(SOFTBUS_OK));
...
HWTEST_F(BrConnectionTest, ConnBrUpdateConnectionRcTest004, TestSize.Level1)
{
    ...
    char addr[BT_MAC_LEN] = "11:22:33:44:55:66";
    ConnBrConnectionBrConnection *connection = ConnBrCreateConnection(addr, CONN_SIDE_CLIENT, INVALID_SOCKET_HANDLE);
    ASSERT_NE(nullptr, connection);
    int32_t ret = ConnBrUpdateConnectionRc(connection, -1);
    EXPECT_EQ(SOFTBUS_OK, ret);
    ConnBrFreeConnection(connection);
...
HWTEST_F(BrConnectionTest, ConnBrOnReferenceRequestTest001, TestSize.Level1)
{
    int32_t ret = ConnBrOnReferenceRequest(NULL, NULL);
    EXPECT_EQ(SOFTBUS_INVALID_PARAM, ret);
}
...
HWTEST_F(BrConnectionTest, ConnBrOnReferenceRequestTest007, TestSize.Level1)
{
    ...
    EXPECT_CALL(brMock, ConnBrPackCtlMessage).WillOnce(Return(1));
    EXPECT_CALL(brMock, ConnBrPostBytes).WillOnce(Return(SOFTBUS_OK));
    EXPECT_CALL(brMock, ConnPostMsgToLooper).WillRepeatedly(Return(SOFTBUS_OK));
    ...
    int32_t ret = ConnBrOnReferenceRequest(connection, json);
    EXPECT_EQ(SOFTBUS_OK, ret);
    EXPECT_EQ(BR_CONNECTION_STATE_CONNECTED, testConnection->state);
    cJSON_Delete(json);
    ConnBrFreeConnection(connection);
    SoftBusFree(testConnection);
}

HWTEST_F(BrConnectionTest, ConnBrOnReferenceRequestTest008, TestSize.Level1)
{
    NiceMock<BrConnectionInterfaceMock> brMock;
    EXPECT_CALL(brMock, SoftBusMutexInit).WillRepeatedly(Return(SOFTBUS_OK));
    EXPECT_CALL(brMock, CreateSoftBusList).WillOnce(Return((SoftBusList *)SoftBusCalloc(sizeof(SoftBusList))));
...
HWTEST_F(BrConnectionTest, ConnBrOnReferenceResponseTest007, TestSize.Level1)
{
    NiceMock<BrConnectionInterfaceMock> brMock;
    EXPECT_CALL(brMock, SoftBusMutexInit).WillOnce(Return(SOFTBUS_LOCK_ERR));
...
HWTEST_F(BrConnectionTest, ConnBrDisconnectNowTest001, TestSize.Level1)
{
    ...
    EXPECT_CALL(brMock, SoftBusMutexLock).WillRepeatedly(Return(SOFTBUS_OK));
    EXPECT_CALL(brMock, SoftBusMutexUnlock).WillRepeatedly(Return(SOFTBUS_OK));
    EXPECT_CALL(brMock, ConnRemoveMsgFromLooper).WillRepeatedly(Return());
    ...
    EXPECT_EQ(SOFTBUS_OK, ret);
    EXPECT_EQ(BR_CONNECTION_STATE_CLOSED, connection->state);
    ConnBrFreeConnection(connection);
}
...
HWTEST_F(BrConnectionTest, ConnBrDisconnectNowTest002, TestSize.Level1)
{
    ...
    EXPECT_CALL(brMock, SoftBusMutexUnlock).WillRepeatedly(Return(SOFTBUS_OK));
    EXPECT_CALL(brMock, ConnRemoveMsgFromLooper).WillRepeatedly(Return());
    EXPECT_CALL(brMock, SoftBusSleepMs).WillRepeatedly(Return());
...
HWTEST_F(BrConnectionTest, ConnBrDisconnectNowTest003, TestSize.Level1)
{
    ...
    EXPECT_CALL(brMock, SoftBusMutexDestroy).WillRepeatedly(Return()));
    EXPECT_CALL(brMock, DestroySoftBusList).WillRepeatedly(Return()));
    EXPECT_CALL(brMock, SoftBusMutexLock).WillOnce(Return(SOFTBUS_LOCK_ERR));
...
HWTEST_F(BrConnectionTest, ConnBrStopServerTest001, TestSize.Level1)
{
    ...
    EXPECT_CALL(brMock, SoftBusMutexInit).WillRepeatedly(Return(SOFTBUS_OK));
    EXPECT_CALL(brMock, SoftBusMutexLock).WillRepeatedly(Return(SOFTBUS_OK));
    EXPECT_CALL(brMock, SoftBusMutexUnlock).WillRepeatedly(ReturnOFTBUS_OK));
...
HWTEST_F(BrConnectionTest, ConnBrRefreshIdleTimeoutTest001, TestSize.Level1)
{
    ConnBrRefreshIdleTimeout(NULL);
}
...
HWTEST_F(BrConnectionTest, ConnBrRefreshIdleTimeoutTest003, TestSize.Level1)
{
    ...
    EXPECT_CALL(brMock, SoftBusMutexDestroy).WillRepeatedly(Return());
    EXPECT_CALL(brMock, DestroySoftBusList).WillRepeatedly(Return()));
    EXPECT_CALL(brMock, SoftBusMutexLock).WillRepeatedly(ReturnOFTBUS_OK));
    EXPECT_CALL(brMock, SoftBusMutexUnlock).WillRepeatedly(ReturnOFTBUS_OK));
    EXPECT_CALL(brMock, ConnRemoveMsgFromLooper).WillRepeatedly(Return());
...
HWTEST_F(BrConnectionTest, ConnBrSetIdleCheckTest002, TestSize.Level1)
{
    ...
    EXPECT_EQ(SOFTBUS_OK, ret);
    EXPECT_EQ(false, connection->enableIdleCheck);
    ConnBrFreeConnection(connection);
}
...
HWTEST_F(BrConnectionTest, ConnBrOccupyTest001, TestSize.Level1)
{
    ConnBrOccupy(NULL);
}
...
HWTEST_F(BrConnectionTest, ConnBrOccupyTest002, TestSize.Level1)
{
    ...
    EXPECT_EQ(true, connection->isOccupied);
    ConnBrFreeConnection(connection);
}
...
HWTEST_F(BrConnectionTest, ConnBrOccupyTest004, TestSize.Level1)
{
    ...
    EXPECT_CALL(brmock, SoftBusMutexUnlock).WillRepeatedly(ReturnOFTBUS_OK));
    EXPECT_CALL(brmock, ConnRemoveMsgFromLooper).WillRepeatedly(Return()));
    EXPECT_CALL(brmock, ConnPostMsgToLooper).WillOnce(Return(SOFTBUS_LOCK_ERR));
...
HWTEST_F(BrConnectionTest, ConnBrConnectionMuduleInitTest001, TestSize.Level1)
{
    ...
    SoftBusLooper *looper = LooperGetLooper();
...
HWTEST_F(BrConnectionTest, ConnBrConnectionMuduleInitTest005, TestSize.Level1)
{
    NiceMock<BrConnectionInterfaceMock> brMock;
    EXPECT_CALL(brmock, SoftBusMutexInit).WillOnce(Return(SOFTBUS_LOCK_ERR));
...
HWTEST_F(BrConnectionTest, ConnBrConnectionMuduleInitTest008, TestSize.Level1)
{
    NiceMock<BrConnectionInterfaceMock> brMock;
    EXPECT_CALL(brmock, SoftBusMutexInit).WillRepeatedly(Return(SOFTBUS_OK));
    EXPECT_CALL(brmock, SoftbusGetConfig)
    ...
    ret = ConnBrConnectionMuduleInit(looper, &g_sppDriver, &g_eventListener);
    EXPECT_EQ(SOFTBUS_OK, ret);
}
}