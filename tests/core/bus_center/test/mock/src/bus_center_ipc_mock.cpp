...
int32_t ClientOnJoinLNNResult(
    const char *pkgName, int32_t pid,  void *addr, uint32_t addrTypeLen, const char *networkId, int32_t retCode)
{
    return BusCenterIpcInterfaceInstance()->ClientOnJoinLNNResult(pkgName, pid, addr, addrTypeLen, networkId, retCode);
}

int32_t ClientOnJoinMetaNodeResult(
    const char *pkgName, int32_t pid, void *addr, uint32_t addrTypeLen, const char *networkId, int32_t retCode)
{
    return BusCenterIpcInterfaceInstance()->ClientOnJoinMetaNodeResult(
        pkgName, pid, addr, addrTypeLen, networkId, retCode);
}
...
int32_t ClientOnTimeSyncResult(const char *pkgName, int32_t pid, const void *info, uint32_t infoTypeLen, int32_t retCode)
{
