...
#include "lnn_data_cloud_sync_mock.h"
...
{
    return GetNetBuilderDepsInterface()->LnnConvertDLidToUdid(id, type, udid, len);
}
...
{
    return GetNetBuilderDepsInterface()->LnnPutDBData(dbId, putKey, putKeyLen, putValue, putValueLen);
}

int32_t LnnCloudSync(int32_t dbId)
{
    return GetNetBuilderDepsInterface()->LnnCloudSync(dbId);
}
} // extern "C"
} // namespace OHOS
