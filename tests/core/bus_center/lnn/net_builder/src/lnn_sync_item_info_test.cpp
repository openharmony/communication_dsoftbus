...
HWTEST_F(LNNSyncInfoItemTest, WIFI_CONNECT_TO_TARGET_AP_TEST_001, TestSize.Level1)
{
    ...
    EXPECT_CALL(LnnServiceMock, SoftBusGetWifiDeviceConfig)
        .WillOnce(DoAll(SetArgPointee<0>(*result1), SetArgPointee<1>(INVALID_WIFI_MAX_CONFIG_SIZE),
            Return(SOFTBUS_ERR)))
        .WillOnce(DoAll(SetArgPointee<0>(*result2), SetArgPointee<1>(INVALID_WIFI_MAX_CONFIG_SIZE),
            Return(SOFTBUS_OK)))
        .WillOnce(DoAll(SetArgPointee<0>(*result3), SetArgPointee<1>(WIFI_MAX_CONFIG_SIZE),
            Return(SOFTBUS_OK)))
        .WillOnce(DoAll(SetArgPointee<0>(*result4), SetArgPointee<1>(WIFI_MAX_CONFIG_SIZE),
            Return(SOFTBUS_OK)))
        .WillOnce(DoAll(SetArgPointee<0>(*result5), SetArgPointee<1>(WIFI_MAX_CONFIG_SIZE),
            Return(SOFTBUS_OK)));
...
HWTEST_F(LNNSyncInfoItemTest, LNN_SEND_TRANS_REQ_TEST_001, TestSize.Level1)
{
    ...
    EXPECT_CALL(LedgerMock, LnnSetDLBssTransInfo).WillOnce(Return(SOFTBUS_ERR))
        .WillRepeatedly(Return(SOFTBUS_OK));
    NiceMock<LnnSyncInfoInterfaceMock> SyncInfoMock;
    EXPECT_CALL(SyncInfoMock, LnnSendSyncInfoMsg).WillOnce(Return(SOFTBUS_ERR))
        .WillRepeatedly(Return(SOFTBUS_OK));
...
HWTEST_F(LNNSyncInfoItemTest, LNN_ONRECEIVE_TRANS_REQ_MSG_TEST_001, TestSize.Level1)
{
    NiceMock<LnnNetLedgertInterfaceMock> LedgerMock;
    NiceMock<LnnSyncInfoInterfaceMock> SyncInfoMock;
    EXPECT_CALL(LedgerMock, LnnConvertDlId).WillOnce(Return(SOFTBUS_ERR))
        .WillRepeatedly(Return(SOFTBUS_OK));
    EXPECT_CALL(LedgerMock, LnnSetDLDeviceInfoName).WillOnce(Return(SOFTBUS_ERR))
        .WillRepeatedly(Return(SOFTBUS_OK));
...
HWTEST_F(LNNSyncInfoItemTest, LNN_ONRECEIVE_BR_OFFLINE_TEST_001, TestSize.Level1)
{
    ...
    EXPECT_CALL(LedgerMock, LnnConvertDlId).WillOnce(Return(SOFTBUS_ERR))
        .WillRepeatedly(Return(SOFTBUS_OK));
    EXPECT_CALL(LedgerMock, LnnGetCnnCode).WillOnce(Return(INVALID_CONNECTION_CODE_VALUE))
        .WillRepeatedly(Return(SOFTBUS_OK));
...
{
    SoftBusWifiDevConf conWifiConf = { .securityType = TEST_INT_VALUE, .isHiddenSsid = TEST_INT_VALUE, };
