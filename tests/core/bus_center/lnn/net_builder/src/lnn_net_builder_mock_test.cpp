...
{
    void *para = reinterpret_cast<void *>(SoftBusMalloc(sizeof(ConnectionAddr)));
    void *para1 = reinterpret_cast<void *>(SoftBusMalloc(sizeof(ConnectionAddr)));
    void *para2 = reinterpret_cast<void *>(SoftBusMalloc(sizeof(ConnectionAddr)));
    ...
    EXPECT_TRUE(ProcessNodeStateChanged(para1) == SOFTBUS_ERR);
    EXPECT_TRUE(ProcessNodeStateChanged(para2) == SOFTBUS_ERR);
...
{
    JoinLnnMsgPara *para = reinterpret_cast<JoinLnnMsgPara *>(SoftBusMalloc(sizeof(JoinLnnMsgPara)));
    LnnConnectionFsm *connFsm = reinterpret_cast<LnnConnectionFsm *>(SoftBusMalloc(sizeof(LnnConnectionFsm)));
    ListInit(&connFsm->node);
    connFsm->connInfo.version = SOFTBUS_NEW_V1;
    connFsm->isDead = false;
    ...
    NiceMock<NetBuilderDepsInterfaceMock> NetBuilderMock;
    EXPECT_CALL(NetBuilderMock, LnnIsSameConnectionAddr(_, _)).WillRepeatedly(Return(false));
    para->isNeedConnect = false;
    (void)strcpy_s(para->pkgName, PKG_NAME_SIZE_MAX, "pkgName");
    EXPECT_TRUE(TrySendJoinLNNRequest(nullptr, true) == SOFTBUS_INVALID_PARAM);
    EXPECT_TRUE(TrySendJoinLNNRequest(para, true) == SOFTBUS_ERR);
...
HWTEST_F(LNNNetBuilderMockTest, TRY_SEND_JOIN_LNN_REQUEST_TEST_002, TestSize.Level1)
{
    ...
    (void)strcpy_s(para->pkgName, PKG_NAME_SIZE_MAX, "pkgName");
    EXPECT_TRUE(TrySendJoinLNNRequest(para, true) == SOFTBUS_OK);
...
HWTEST_F(LNNNetBuilderMockTest, TRY_SEND_JOIN_LNN_REQUEST_TEST_003, TestSize.Level1)
{
    ...
    NiceMock<NetBuilderDepsInterfaceMock> NetBuilderMock;
    EXPECT_CALL(NetBuilderMock, LnnIsSameConnectionAddr(_, _)).WillRepeatedly(Return(true));
    para->isNeedConnect = true;
    (void)strcpy_s(para->pkgName, PKG_NAME_SIZE_MAX, "pkgName");
    EXPECT_TRUE(TrySendJoinLNNRequest(para, true) == SOFTBUS_OK);
...
HWTEST_F(LNNNetBuilderMockTest, LNN_PROCESS_COMPLETE_NOT_TRUSTED_MSG_TEST_001, TestSize.Level1)
{
    ...
    LnnProcessCompleteNotTrustedMsg(LNN_INFO_TYPE_NOT_TRUSTED, NODE_NETWORK_ID, jsonMsg, MSG_ERR_LEN0);
    LnnProcessCompleteNotTrustedMsg(LNN_INFO_TYPE_NOT_TRUSTED, NODE_NETWORK_ID, jsonMsg, MSG_ERR_LEN0);
    LnnProcessCompleteNotTrustedMsg(LNN_INFO_TYPE_NOT_TRUSTED, NODE_NETWORK_ID, jsonMsg, strlen(jsonStr) + 1);
    LnnProcessCompleteNotTrustedMsg(LNN_INFO_TYPE_NOT_TRUSTED, NODE_NETWORK_ID, jsonMsg, strlen(jsonStr) + 1);
}
...
HWTEST_F(LNNNetBuilderMockTest, ON_RE_AUTH_VERIFY_PASSED_TEST_002, TestSize.Level1)
{
    ...
    EXPECT_CALL(NetBuilderMock, LnnIsSameConnectionAddr(_, _))
        .WillOnce(Return(false))
        .WillRepeatedly(Return(true));
    OnReAuthVerifyPassed(REQUEST_ID, AUTH_ID, &info);
    OnReAuthVerifyPassed(REQUEST_ID, AUTH_ID, &info);
...
{
    NiceMock<NetBuilderDepsInterfaceMock> NetBuilderMock;
    EXPECT_CALL(NetBuilderMock, LnnIsSameConnectionAddr).WillOnce(Return(false)).WillRepeatedly(Return(true));
    LnnConnectionFsm *connFsm = reinterpret_cast<LnnConnectionFsm *>(SoftBusMalloc(sizeof(LnnConnectionFsm)));
    ListInit(&connFsm->node);
    connFsm->connInfo.requestId = REQUEST_ID;
    connFsm->isDead = false;
...
HWTEST_F(LNNNetBuilderMockTest, ON_RECEIVE_NODE_ADDR_CHANGED_MSG_TEST_001, TestSize.Level1)
{
    ...
    uint8_t *jsonMsg1 = reinterpret_cast<uint8_t *>(jsonStr1);
    char jsonStr2[] = "{\"NODE_ADDR\":\"127.0.0.1\",\"PROXY_PORT\":1000,\"SESSION_PORT\":1001}";
    uint8_t *jsonMsg2 = reinterpret_cast<uint8_t *>(jsonStr2);
    char jsonStr3[] = "{\"NODE_CODE\":1111,\"PROXY_PORT\":1000,\"SESSION_PORT\":1001}";
    uint8_t *jsonMsg3 = reinterpret_cast<uint8_t *>(jsonStr3);
    char jsonStr4[] = "{\"NODE_CODE\":1111,\"NODE_ADDR\":\"127.0.0.1\",\"SESSION_PORT\":1001}";
    uint8_t *jsonMsg4 = reinterpret_cast<uint8_t *>(jsonStr4);
    char jsonStr5[] = "{\"NODE_CODE\":1111,\"NODE_ADDR\":\"127.0.0.1\",\"PROXY_PORT\":1000}";
    uint8_t *jsonMsg5 = reinterpret_cast<uint8_t *>(jsonStr5);
    uint8_t jsonMsg6[] = {0};
    NiceMock<NetBuilderDepsInterfaceMock> NetBuilderMock;
    EXPECT_CALL(NetBuilderMock, SoftbusGetConfig(_, _, _)).WillRepeatedly(Return(SOFTBUS_OK));
    OnReceiveNodeAddrChangedMsg(LNN_INFO_TYPE_DEVICE_NAME, NODE_NETWORK_ID, jsonMsg1, MSG_ERR_LEN0);
    OnReceiveNodeAddrChangedMsg(LNN_INFO_TYPE_NODE_ADDR, NODE_NETWORK_ID, jsonMsg1, MSG_ERR_LEN0);
    OnReceiveNodeAddrChangedMsg(LNN_INFO_TYPE_NODE_ADDR, NODE_NETWORK_ID, jsonMsg6, MSG_ERR_LEN1);
    OnReceiveNodeAddrChangedMsg(LNN_INFO_TYPE_NODE_ADDR, NODE_NETWORK_ID, jsonMsg2, strlen(jsonStr2) + 1);
    OnReceiveNodeAddrChangedMsg(LNN_INFO_TYPE_NODE_ADDR, NODE_NETWORK_ID, jsonMsg3, strlen(jsonStr3) + 1);
    OnReceiveNodeAddrChangedMsg(LNN_INFO_TYPE_NODE_ADDR, NODE_NETWORK_ID, jsonMsg4, strlen(jsonStr4) + 1);
    OnReceiveNodeAddrChangedMsg(LNN_INFO_TYPE_NODE_ADDR, NODE_NETWORK_ID, jsonMsg5, strlen(jsonStr5) + 1);
    OnReceiveNodeAddrChangedMsg(LNN_INFO_TYPE_NODE_ADDR, NODE_NETWORK_ID, jsonMsg1, strlen(jsonStr1) + 1);
}
