...
HWTEST_F(LNNDisctributedLedgerTest, LNN_CONVERT_DL_ID_Test_001, TestSize.Level1)
{
    ...
    EXPECT_EQ(SOFTBUS_OK, LnnConvertDlId(const_cast<char *>(srcId), CATEGORY_UDID,
        CATEGORY_UDID, dstIdBuf, UDID_BUF_LEN));
    const char *srcId1 = "123456ABCDEFGHI";
    char dstIdBuf1[UUID_BUF_LEN] = { 0 };
    EXPECT_EQ(SOFTBUS_NOT_FIND, LnnConvertDlId(const_cast<char *>(srcId1), CATEGORY_UUID,
...
HWTEST_F(LNNDisctributedLedgerTest, LnnSetDLBleDirectTimestamp_Test_001, TestSize.Level1)
{
    ...
    NodeInfo deviceInfo;
    if (LnnSaveRemoteDeviceInfo(&deviceInfo) == SOFTBUS_NOT_IMPLEMENT) {
        EXPECT_EQ(LnnSetDLConnCapability(networkId, connCapability), SOFTBUS_NOT_IMPLEMENT);
    } else {
        EXPECT_EQ(LnnSetDLConnCapability(networkId, connCapability), SOFTBUS_OK);
    }
}
...
HWTEST_F(LNNDisctributedLedgerTest, LnnSetDLConnUserIdCheckSum_Test_001, TestSize.Level1)
{
    ...
    NodeInfo deviceInfo;
    if (LnnSaveRemoteDeviceInfo(&deviceInfo) == SOFTBUS_NOT_IMPLEMENT) {
        EXPECT_EQ(LnnSetDLConnUserIdCheckSum(networkId, userIdCheckSum), SOFTBUS_NOT_IMPLEMENT);
    } else {
        EXPECT_EQ(LnnSetDLConnUserIdCheckSum(networkId, userIdCheckSum), SOFTBUS_OK);
    }
}
...
HWTEST_F(LNNDisctributedLedgerTest, LnnSetDLConnUserId_Test_001, TestSize.Level1)
{
    ...
    NodeInfo deviceInfo;
    if (LnnSaveRemoteDeviceInfo(&deviceInfo) == SOFTBUS_NOT_IMPLEMENT) {
        EXPECT_EQ(LnnSetDLConnUserId(networkId, userId), SOFTBUS_NOT_IMPLEMENT);
    } else {
        EXPECT_EQ(LnnSetDLConnUserId(networkId, userId), SOFTBUS_OK);
    }
}
} // namespace OHOS
