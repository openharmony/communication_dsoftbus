...
void SocketTestWithPeerName(const uint8_t *data, size_t size)
{
    ...
    std::string socketPeerName(bufSize, '\0');
    if (!GenerateString(socketPeerName)) {
        DataGenerator::Clear();
        return;
    }
    DataGenerator::Clear();
    ...
    SocketInfo info = {
        ...
        .peerName = const_cast<char *>(socketPeerName.c_str()),
        .peerNetworkId = const_cast<char *>(DEFAULT_SOCKET_PEER_NETWORKID),
