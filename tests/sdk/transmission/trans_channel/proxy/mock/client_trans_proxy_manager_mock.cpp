...
uint32_t SoftBusHtoL1(uint32_t value)
{
    return GetClientTransProxyManagerInterface()->SoftBusHtoL1(value);
}
uint32_t SoftBusHtoN1(uint32_t value)
{
    return GetClientTransProxyManagerInterface()->SoftBusHtoN1(value);
}
...
int32_t TransProxyPackTlvBytes(ProxyDataInfo *dataInfo, const char *sessionKey,
...
uint8_t *TransProxyPackData(ProxyDataInfo *dataInfo, uint32_t sliceNum,
