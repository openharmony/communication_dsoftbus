...
HWTEST_F(ClientTransProxyManagerMockTest, TransProxyChannelAsyncSendBytes001, TestSize.Level1)
{
    ...
    EXPECT_CALL(ClientProxyManagerMock, TransProxyPackData).WillRepeatedly(Return(temSliceData));
    EXPECT_CALL(ClientProxyManagerMock, ServerIpcSendMessage).WillRepeatedly(Return(SOFTBUS_OK));
    EXPECT_CALL(ClientProxyManagerMock, ClientGetSessionIdByChannelId).WillRepeatedly(Return(SOFTBUS_OK));
    EXPECT_CALL(ClientProxyManagerMock, DataSeqInfoListAddItem).WillRepeatedly(Return(SOFTBUS_OK));
...
/**
 * @tc.name: TransProxyChannelAsyncSendBytes002
 * @tc.desc: TransProxyChannelAsyncSendBytes, enter the normal branch
 * @tc.type: FUNC
 * @tc.require:
 */
...
/**
 * @tc.name: TransProxyChannelAsyncSendBytes003
 * @tc.desc: TransProxyChannelAsyncSendBytes, enter the normal branch
 * @tc.type: FUNC
 * @tc.require:
 */
HWTEST_F(ClientTransProxyManagerMockTest, TransProxyChannelAsyncSendBytes003, TestSize.Level1)
{
    ...
    EXPECT_CALL(ClientProxyManagerMock, ServerIpcSendMessage).WillRepeatedly(Return(SOFTBUS_OK));
    EXPECT_CALL(ClientProxyManagerMock, ClientGetSessionIdByChannelId).WillOnce(Return(SOFTBUS_OK));
    EXPECT_CALL(ClientProxyManagerMock, DataSeqInfoListAddItem).WillOnce(Return(SOFTBUS_INVALID_PARAM));
    ret = TransProxyChannelAsyncSendBytes(channelId, data, len, dataSeq);
    EXPECT_EQ(SOFTBUS_INVALID_PARAM, ret);
    ...
    EXPECT_CALL(ClientProxyManagerMock, ServerIpcSendMessage).WillRepeatedly(Return(SOFTBUS_OK));
    EXPECT_CALL(ClientProxyManagerMock, ClientGetSessionIdByChannelId).WillOnce(Return(SOFTBUS_INVALID_PARAM));
    ret = TransProxyChannelAsyncSendBytes(channelId, newdata, len, dataSeq);
    EXPECT_EQ(SOFTBUS_INVALID_PARAM, ret);
