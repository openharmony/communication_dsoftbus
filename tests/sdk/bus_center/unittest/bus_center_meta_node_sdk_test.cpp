...
HWTEST_F(BusCenterMetaNodeSdkTest, BUS_CENTER_SDK_META_NODE_Test_001, TestSize.Level0)
{
    ...
    (void)memset_s(&infos, sizeof(MetaNodeInfo) * MAX_META_NODE_NUM, 0,
        sizeof(MetaNodeInfo) * MAX_META_NODE_NUM);
    ...
    EXPECT_EQ(ActiveMetaNode(TEST_PKG_NAME, &configInfo, metaNodeId), SOFTBUS_IPC_ERR);
    EXPECT_EQ((int32_t)strlen(metaNodeId), 0);
    EXPECT_EQ(ActiveMetaNode(TEST_PKG_NAME, &configInfo, metaNodeId), SOFTBUS_IPC_ERR);
    EXPECT_EQ(GetAllMetaNodeInfo(TEST_PKG_NAME, infos, &infoNum), SOFTBUS_PERMISSION_DENIED);
    ...
    EXPECT_EQ(infos[0].configInfo.addrNum, 0);
    EXPECT_EQ(DeactiveMetaNode(TEST_PKG_NAME, metaNodeId), SOFTBUS_PERMISSION_DENIED);
    EXPECT_EQ(GetAllMetaNodeInfo(TEST_PKG_NAME, infos, &infoNum), SOFTBUS_PERMISSION_DENIED);
    EXPECT_EQ(infoNum, MAX_META_NODE_NUM);
}
...
HWTEST_F(BusCenterMetaNodeSdkTest, BUS_CENTER_SDK_META_NODE_Test_002, TestSize.Level0)
{
    ...
    for (i = 0; i < MAX_META_NODE_NUM; ++i) {
        ...
        EXPECT_EQ(DeactiveMetaNode(TEST_PKG_NAME, infos[i].metaNodeId), SOFTBUS_PERMISSION_DENIED);
    }
}
