...
HWTEST_F(BusCenterSdkTest, PublishLNNTest001, TestSize.Level0)
{
    ...
    ret = PublishLNN("MODULE_LNN", &g_pInfo, &g_publishCb);
    EXPECT_TRUE(ret != 0);
...
HWTEST_F(BusCenterSdkTest, RefreshLNNTest001, TestSize.Level0)
{
    ...
    ret = RefreshLNN("MODULE_LNN", &g_sInfo, &g_refreshCb);
    EXPECT_TRUE(ret != 0);
...
HWTEST_F(BusCenterSdkTest, BUS_CENTER_SDK_TRIGGER_RANGE_Test001, TestSize.Level1)
{
    ...
    EXPECT_EQ(RegisterRangeCallbackForMsdp(TEST_MSDP_NAME, &g_bleRangeCb), SOFTBUS_PERMISSION_DENIED);
    ...
    EXPECT_EQ(UnregisterRangeCallbackForMsdp(TEST_MSDP_NAME), SOFTBUS_PERMISSION_DENIED);
