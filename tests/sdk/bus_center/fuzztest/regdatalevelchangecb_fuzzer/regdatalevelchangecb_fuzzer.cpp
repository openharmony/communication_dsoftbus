...
#include "regdatalevelchangecb_fuzzer.h"
#include <cstddef>
#include <securec.h>
...
namespace OHOS {
    void FuzzOnDataLevelChanged(const char *networkId, const DataLevel dataLevel)
    {
        (void)networkId;
        (void)dataLevel;
    }
    ...
    bool DoSomethingInterestingWithMyAPI(const uint8_t* data, size_t size)
    {
        ...
        RegDataLevelChangeCb(reinterpret_cast<const char *>(tmp), &dataLevelCb);
        free(tmp);
        return true;
    }
}
