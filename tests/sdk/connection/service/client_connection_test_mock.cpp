...
HWTEST_F(ClientConnectionTest, CreateServerTest, TestSize.Level0)
{
    ...
    ASSERT_EQ(ret, SOFTBUS_INVALID_PARAM);
    ret = GeneralCreateServer("dms", nullptr);
    ASSERT_EQ(ret, SOFTBUS_INVALID_PARAM);
    ...
    ret = GeneralCreateServer("dms", "1234");
    ASSERT_NE(ret, SOFTBUS_OK);
...
HWTEST_F(ClientConnectionTest, RemoveServerTest, TestSize.Level0)
{
    ...
    ret = GeneralRemoveServer("dms", "1234");
    ASSERT_NE(ret, SOFTBUS_OK);
...
HWTEST_F(ClientConnectionTest, ConnectTest, TestSize.Level0)
{
    ...
    ASSERT_EQ(ret, SOFTBUS_INVALID_PARAM);
    ret = GeneralConnect("dms", "1234", nullptr);
    ASSERT_EQ(ret, SOFTBUS_INVALID_PARAM);

    testing::NiceMock<ClientConnectionInterfaceMock> mock;
    EXPECT_CALL(mock, InitSoftBus).WillOnce(testing::Return(-1));
    Address addr;
    ret = GeneralConnect("dms", "1234", &addr);
    ASSERT_NE(ret, SOFTBUS_OK);
...
{
    int32_t ret = GeneraConnGetPeerDeviceId(-1, nullptr, 0);
    ASSERT_EQ(ret, SOFTBUS_INVALID_PARAM);
    ret = GeneraConnGetPeerDeviceId(1, nullptr, 0);
    ASSERT_EQ(ret, SOFTBUS_INVALID_PARAM);
    char deviceId[10] = { 0 };
    ret = GeneraConnGetPeerDeviceId(1, deviceId, 0);
    ASSERT_EQ(ret, SOFTBUS_INVALID_PARAM);
    ret = GeneraConnGetPeerDeviceId(1, deviceId, BT_MAC_LEN + 1);
    ASSERT_EQ(ret, SOFTBUS_INVALID_PARAM);

    testing::NiceMock<ClientConnectionInterfaceMock> mock;
    EXPECT_CALL(mock, ServerIpcGetPeerDeviceId).WillOnce(testing::Return(-1));
    ret = GeneraConnGetPeerDeviceId(1, deviceId, 10);
    ASSERT_NE(ret, SOFTBUS_OK);
