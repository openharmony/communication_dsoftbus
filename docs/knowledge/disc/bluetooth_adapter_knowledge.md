...
> 路径：`adapter/common/net/bluetooth` | 源码 ~7056 行 | 8 个源文件
...
```
bluetooth/
├── common/       bt_common.c (346行) — 蓝牙状态管理、GAP 回调、监听器分发
│                 bt_common_virtual.c — 不支持 BT 平台的桩实现
├── ble/          gatt_client.c (614行) — GATT Client 连接/写/流控
│                 gatt_server.c (864行) — GATT Server 注册/服务/通知/流控
├── broadcast/
│   ├── interface/  broadcast_manager.h — 广播管理对外接口（~30 API）
│   ├── adapter/ble/ ble_gatt.c (1293行) — BLE 广播/扫描适配器、通道池
│   │                 ble_utils.c (564行) — TLV 解析、类型转换
│   └── manager/    broadcast_mgr.c (3191行) — 广播/扫描管理核心
│                   broadcast_mgr_utils.c (110行) — 异步回调辅助
└── net_bluetooth.gni — 编译配置
```

**代码量分布**：广播/扫描管理 47% | 广播适配 18% | GATT Server 12% | GATT Client 9% | 工具 8% | 其他 6%
